...
//when ylog root dir changed,move all dst and log file to new  dir.
static int ydst_root_new(struct ydst_root *root, char *root_new)
{
	...
	root->refs_cur_move_root = root->refs;//need to  be moved  dst num.
	root->ydst_change_seq_move_root++;//count move times
...
static void ylog_all_thread_exit(void)
{
	...
	for_each_ylog(i, y, NULL)
	{
		if (y->name == NULL)
		{
			ylog_info("ylog_all_thread_exit continue\n");
			continue;
		}
	...
	for (;;)
	{
		...
		if (yp_isset(YLOG_POLL_INDEX_PIPE, yp))
		{
			...
			case 'A':
				...
				break;
			case 'B':
				state_curr = YLOG_SUSPEND;
				break;
			case 'C':
				state_curr = YLOG_RESUME;
				break;
			case 'D':
				state_curr = YLOG_STOP;
				break;
			case 'E':
				state_curr = YLOG_EXIT;
				break;
			case 'F':
				state_curr = YLOG_RESTART;
				break;
			case 'G':
				state_curr = YLOG_MOVE_ROOT;
				break;
			case 'H':
				state_curr = YLOG_RESIZE_SEGMENT;
				break;
			case 'I':
				state_curr = YLOG_FLUSH;
				break;
			case 'J':
				state_curr = YLOG_RESET;
				break;
			default:
				state_curr = YLOG_NOP;
				break;
...
__state_control:
		...
		if (yp_isset(YLOG_POLL_INDEX_DATA, yp))
		{
			...
			if (count > 0)
			{
				...
				{
					if(strcmp(y->name,"android_crash")==0)
					{
						char *x = y->rbuf;
						char *q = y->rbuf;
						char line[1024] ;
						...
						while( *x)
						{
							...
							case '\n':
								...
								strncpy(line+2,q,line_count);
								y->write_handler(line, line_count+2, y, NULL);
								line_count = 0;
