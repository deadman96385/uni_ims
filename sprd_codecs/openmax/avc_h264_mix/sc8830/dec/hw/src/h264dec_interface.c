...
PUBLIC void H264Dec_ReleaseRefBuffers(AVCHandle *avcHandle)
{
    ...
    SCI_TRACE_LOW("%s, %d, display_array_len: %d", __FUNCTION__, __LINE__, vo->display_array_len);
    for(i =0; i<vo->display_array_len; i++)
    {
        //OR_VSP_UNBIND(vo->display_array_BH[i]);
        (*(avcHandle->VSP_unbindCb))(avcHandle->userdata,vo->display_array_BH[i]);
    }

    vo->display_array_len = 0;
}
