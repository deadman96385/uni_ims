 ...
 **	Note:
 *****************************************************************************/
void Mp4Dec_InitDecoderPara(Mp4DecObject *vd)
{
...
PUBLIC void Mp4Dec_InitGlobal (Mp4DecObject *vd)
{
...
LOCAL MMDecRet Mp4Dec_MallocFrmBfr(Mp4DecObject *vd)
{
//	uint32 size;

    if( MMDEC_OK != Mp4Dec_InitYUVBfr(vd) )
    {
    ...
    vd->vop_mode_ptr->pCurRecFrame->pDecFrame = &(vd->g_FrmYUVBfr[0]);
 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_InitSessionDecode(Mp4DecObject *vd)
{
    DEC_VOP_MODE_T *vop_mode_ptr = vd->vop_mode_ptr;
    ...
    vd->mp4Handle->VSP_extMemCb(vd->mp4Handle->userdata, vop_mode_ptr->FrameWidth, vop_mode_ptr->FrameHeight, vop_mode_ptr->bDataPartitioning);

    /*MB mode for a frame*/
    vop_mode_ptr->pMbMode = (DEC_MB_MODE_T *)Mp4Dec_ExtraMemAlloc(vd, sizeof(DEC_MB_MODE_T) * total_mb_num, 4);
    if(NULL == vop_mode_ptr->pMbMode)
    {
        return MMDEC_MEMORY_ERROR;
    }

    vop_mode_ptr->pMbMode_prev = (DEC_MB_MODE_T *)Mp4Dec_ExtraMemAlloc(vd, sizeof(DEC_MB_MODE_T) * total_mb_num, 4);
    if(NULL == vop_mode_ptr->pMbMode)
    {
        return MMDEC_MEMORY_ERROR;
    }
    ...
    if (vop_mode_ptr->bDataPartitioning)
    {
        ...
        vop_mode_ptr->g_dec_dc_store = (int32 **)Mp4Dec_ExtraMemAlloc(vd, sizeof (int32 *) * total_mb_num, 4);
        ...
        {
            vop_mode_ptr->g_dec_dc_store[i] = (int32 *) Mp4Dec_ExtraMemAlloc(vd, sizeof (int32) * 6, 4);
    ...
    vop_mode_ptr->pTopCoeff = (int16 *)Mp4Dec_ExtraMemAlloc( vd, sizeof(int16) * (uint8)(vop_mode_ptr->MBNumX) * 4 * 8, 4);
    SCI_ASSERT(NULL != vop_mode_ptr->pTopCoeff);

    vop_mode_ptr->mb_cache_ptr->pMBBfrY = (int8 *)Mp4Dec_ExtraMemAlloc(vd, sizeof(int8) * BLOCK_SQUARE_SIZE*4, 256);
    SCI_ASSERT(NULL != vop_mode_ptr->mb_cache_ptr->pMBBfrY);

    vop_mode_ptr->mb_cache_ptr->pMBBfrU = (int8 *)Mp4Dec_ExtraMemAlloc(vd, sizeof(int8) * BLOCK_SQUARE_SIZE, 256);
    SCI_ASSERT(NULL != vop_mode_ptr->mb_cache_ptr->pMBBfrU);

    vop_mode_ptr->mb_cache_ptr->pMBBfrV = (int8 *)Mp4Dec_ExtraMemAlloc(vd, sizeof(int8) * BLOCK_SQUARE_SIZE, 256);
    SCI_ASSERT(NULL != vop_mode_ptr->mb_cache_ptr->pMBBfrV);

    vop_mode_ptr->pLeftCoeff = (int16 *)Mp4Dec_ExtraMemAlloc(vd, sizeof(int16) * 4 * 8, 4);
    SCI_ASSERT(NULL != vop_mode_ptr->pLeftCoeff );

    for (i = 0; i < BLOCK_CNT; i++)
    {
        vop_mode_ptr->coef_block[i] = (int16 *)Mp4Dec_ExtraMemAlloc(vd, sizeof(int16) * BLOCK_SQUARE_SIZE, 4);
    ...
    if( MMDEC_OK != Mp4Dec_MallocFrmBfr(vd) )
    {
        return MMDEC_MEMORY_ERROR;
    }
    ...
    {
        vd->g_Mp4Dec_IVOP = Mp4Dec_DecIVOP;
        vd->g_Mp4Dec_PVOP = Mp4Dec_DecPVOP;
        vd->g_Mp4Dec_BVOP = Mp4Dec_DecBVOP;
    }
