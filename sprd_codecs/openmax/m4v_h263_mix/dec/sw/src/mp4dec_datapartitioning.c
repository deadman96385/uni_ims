 ...
 **	Note:			vld, de-scan
 *****************************************************************************/
PUBLIC int32 Mp4Dec_RvlcIntraTCOEF(DEC_VOP_MODE_T *vop_mode_ptr, int16 * iDCTCoefQ, int32 iCoefStart, int8 *pNonCoeffPos)
{
    int32 i = iCoefStart;
    int32 last = 0;
...
LOCAL MMDecRet Mp4Dec_ResyncDataPartitioning (DEC_VOP_MODE_T *vop_mode_ptr, uint32 uAddBits)
{
    /*error concealment*/
    int32 rsc_found;
...
PUBLIC MMDecRet Mp4Dec_DecIVOPErrResDataPartitioning(DEC_VOP_MODE_T *vop_mode_ptr)
{
    ...
    while(!vop_mode_ptr->stop_decoding)
    {
        ...
        {
            /*error concealment*/
            ret = Mp4Dec_ResyncDataPartitioning (vop_mode_ptr, 0);
            continue;
        }
...
PUBLIC MMDecRet Mp4Dec_DecPVOPErrResDataPartitioning(DEC_VOP_MODE_T *vop_mode_ptr)
{
    ...
    while(!vop_mode_ptr->stop_decoding)
    {
        ...
        {
            /*error concealment*/
            ret = Mp4Dec_ResyncDataPartitioning (vop_mode_ptr, uAddBits);
            continue;
        }
