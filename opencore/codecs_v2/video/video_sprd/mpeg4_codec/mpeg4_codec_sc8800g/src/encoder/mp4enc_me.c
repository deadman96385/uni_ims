...
#else
	four_mv_cost = 22*g_lambda[pVop_mode->StepSize];
...
// 	uint32 vp_mb_num = pVop_mode->MBNumOneVP;
	...
	if(mb_pos_y != 0)
	{
		if (!is_top_bndry)
		{			
			candidate_mv[1].x = pVop_mode->pMbModeAbv[mb_pos_x].mv[0].x;
			candidate_mv[1].y = pVop_mode->pMbModeAbv[mb_pos_x].mv[0].y;
			is_in_bound[1] = TRUE;
			num_in_Bound++;
		}
		if (!is_right_bndry)
		{			
			candidate_mv[2].x = pVop_mode->pMbModeAbv[mb_pos_x+1].mv[0].x;
			candidate_mv[2].y = pVop_mode->pMbModeAbv[mb_pos_x+1].mv[0].y;
			is_in_bound[2] = TRUE;
			num_in_Bound++;
		}
	}
	...
	if(num_in_Bound == 1)
	{
		for (i = 0; i < 3; i++)
		{
			if (is_in_bound [i] == TRUE)
			{
				...
				{
					int32 ref_pos_x = ((int32)(mb_pos_x<<5) + (pMb_mode->mvPred.x));
					int32 ref_pos_y = ((int32)(mb_pos_y<<5) + (pMb_mode->mvPred.y));
