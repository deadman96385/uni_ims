...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType0MbMap(DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 iGroup;
	int16 j;
	int32 i = 0;
...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType1MbMap (DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 i;
...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType2MbMap (DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 iGroup;
	int32 i, x, y;
	...
	for (i = 0; i < mb_num; i++)
	{
		g_MbToSliceGroupMap[i] = pps_ptr->num_slice_groups_minus1;
	}
	...
	for (iGroup = pps_ptr->num_slice_groups_minus1-1; iGroup >= 0; iGroup--)
	{
		...
		for (y = yTopLeft; y <= yBottomRight; y++)
		{
			...
			{
				g_MbToSliceGroupMap[y*img_ptr->frame_width_in_mbs+x] = iGroup;
			}
		}
	}
...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType3MbMap (DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 i, k;
	...
	{
		mapUnitVacant = (g_MbToSliceGroupMap[y*img_ptr->frame_width_in_mbs+x] == 2) ? 1 : 0;

		if (mapUnitVacant)
		{
			g_MbToSliceGroupMap[y*img_ptr->frame_width_in_mbs+x] = (k >= mapUnitsInSliceGroup0) ? 1 : 0;
		}
...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType4MbMap (DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 mapUnitsInSliceGroup0 = mmin((int32)(pps_ptr->slice_group_change_rate_minus1+1)*img_ptr->slice_group_change_cycle, mb_num);
...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType5MbMap (DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 mapUnitsInSliceGroup0 = mmin((int32)(pps_ptr->slice_group_change_rate_minus1+1)*img_ptr->slice_group_change_cycle, mb_num);
	...
	for (j = 0; j < (int32)img_ptr->frame_width_in_mbs; j++)
	{
		for (i = 0; i < (int32)img_ptr->pic_height_in_map_units; i++)
		{
			...
			{
				g_MbToSliceGroupMap[i*img_ptr->frame_width_in_mbs+j] = (int8)pps_ptr->slice_group_change_direction_flag;
			}else
			{
				g_MbToSliceGroupMap[i*img_ptr->frame_width_in_mbs+j] = (int8)(1-pps_ptr->slice_group_change_direction_flag);
			}
		}
	}
...
************************************************************************
*/
LOCAL void H264Dec_FmoGenerateType6MbMap (DEC_IMAGE_PARAMS_T *img_ptr, DEC_PPS_T *pps_ptr, DEC_SPS_T *sps_ptr, int32 mb_num)
{
	int32 i;
...
LOCAL BOOLEAN H264Dec_FmoGenerateMbToSliceGroupMap(DEC_IMAGE_PARAMS_T *img_ptr)
{
	int32 mb_num;
	DEC_PPS_T *pps_ptr = g_active_pps_ptr;
	...
	{
#ifndef _VSP_LINUX_	
		g_MbToSliceGroupMap = (int8 *)H264Dec_ExtraMemAlloc((uint32)(mb_num)*sizeof(int8));
#else
	...
	{
		memset(g_MbToSliceGroupMap, 0, (uint32)(mb_num)*sizeof(int8));
		return TRUE;
	}
...
PUBLIC BOOLEAN H264Dec_FMO_init(DEC_IMAGE_PARAMS_T *img_ptr)
{
	return H264Dec_FmoGenerateMbToSliceGroupMap (img_ptr);
}

PUBLIC int32 H264Dec_Fmo_get_next_mb_num (int32 curr_mb_nr, DEC_IMAGE_PARAMS_T *img_ptr)
{
	int32 slice_group = g_MbToSliceGroupMap[curr_mb_nr];

	while ((++curr_mb_nr < (int32)img_ptr->frame_size_in_mbs) && g_MbToSliceGroupMap[curr_mb_nr] != slice_group)
		;
