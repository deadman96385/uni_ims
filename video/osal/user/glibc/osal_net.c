    ...
    OSAL_NetAddress *address_ptr)
{
    ...
    if (1 == inet_pton(AF_INET, str_ptr, &address_ptr->ipv4)) {
        address_ptr->type = OSAL_NET_SOCK_UDP;
        return (OSAL_SUCCESS);
    }

    /* Not an ipv4 address, try ipv6 */
    if (1 == inet_pton(AF_INET6, str_ptr, address_ptr->ipv6)) {
        address_ptr->type = OSAL_NET_SOCK_UDP_V6;
        return (OSAL_SUCCESS);
    }
    ...
    OSAL_NetAddress *address_ptr)
{
    if ((OSAL_NET_SOCK_TCP_V6 == address_ptr->type) ||
            OSAL_NET_SOCK_UDP_V6 == address_ptr->type) {
        /* ipv6 */
        inet_ntop(AF_INET6, address_ptr->ipv6, str_ptr, OSAL_NET_IPV6_STR_MAX);
    }
    else {
        /* ipv4 */
        inet_ntop(AF_INET, &address_ptr->ipv4, str_ptr, OSAL_NET_IPV4_STR_MAX);
    }
...
# else
    ...
    /* Get address info */
    if (0 != getaddrinfo(name_ptr, NULL, &hints, &res)) {
        return (OSAL_FAIL);
    }
        ...
        char*   mac)
{
    ...
    OSAL_strncpy(ifr.ifr_name, infcName, IFNAMSIZ-1);

    ioctl(fd, SIOCGIFHWADDR, &ifr);
    close(fd);
    OSAL_memCpy(mac, ifr.ifr_hwaddr.sa_data, 6);
}
