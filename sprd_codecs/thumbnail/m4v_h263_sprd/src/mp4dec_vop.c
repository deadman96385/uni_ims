...
PUBLIC void Mp4Dec_exit_picture(Mp4DecObject *vd)
{
    ...
    if(vop_mode_ptr->VopPredType != BVOP)
    {
        ...
        vop_mode_ptr->pCurRecFrame->pDecFrame->need_extend = TRUE;
        ...
        if(vop_mode_ptr->VopPredType == IVOP)
        {
            if(vop_mode_ptr->pBckRefFrame->pDecFrame != NULL)
            {
                ...
                vop_mode_ptr->pBckRefFrame->pDecFrame->need_extend= FALSE;
        ...
        if(vop_mode_ptr->pFrdRefFrame->pDecFrame != NULL)
        {
            ...
            vop_mode_ptr->pFrdRefFrame->pDecFrame->need_extend= FALSE;
...
PUBLIC MMDecRet Mp4Dec_InitVop(DEC_VOP_MODE_T *vop_mode_ptr, MMDecInput *dec_input_ptr)
{
    ...
    if(IVOP != vop_mode_ptr->VopPredType)
    {
        ...
        {
            ret =  MMDEC_ERROR;
        } else if (pDecFrame->need_extend)
        {
            Mp4Dec_ExtendFrame(vop_mode_ptr, pDecFrame->imgYUV);
...
PUBLIC void Mp4Dec_output_one_frame (Mp4DecObject *vd, MMDecOutput *dec_output_ptr)
{
    DEC_VOP_MODE_T *vop_mode_ptr = vd->vop_mode_ptr;
