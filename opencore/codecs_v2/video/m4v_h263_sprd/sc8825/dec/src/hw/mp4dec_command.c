...
PUBLIC void Mp4Dec_init_frame_VSP(DEC_VOP_MODE_T *vop_mode_ptr, MMDecInput *dec_input_ptr)
{
	...
	Mp4Dec_InitBitstream(
		...
		//( ((uint32)(vop_mode_ptr->frame_bistrm_ptr) -(uint32)s_CMD_mem_cache_bfr_ptr+(uint32)s_CMD_mem_cache_bfr_phy_ptr))
#endif		
...
LOCAL MMDecRet Mp4Dec_VSPInit ()
{
	...
	VSP_WRITE_REG(VSP_AHBM_REG_BASE+AHBM_ENDAIN_SEL_OFFSET, 0x5, "ENDAIN_SEL: 0x5 for little endian system");	
    	...
    	cmd = (1 << 31) |(1 << 30) |((uint32)0xfffffff);
...
PUBLIC MMDecRet Mp4Dec_VspFrameInit(DEC_VOP_MODE_T *vop_mode_ptr, uint32 bitstm_addr)
{
	...
	VSP_WRITE_REG(VSP_MEMO10_ADDR+20*4, ((uint32)CMD_CONTROL_INFO_ADDR)>>8, "config CMD control info buffer address");
	VSP_WRITE_REG(VSP_MEMO10_ADDR+21*4, ((uint32)CMD_CONTROL_DATA_ADDR)>>8, "config CMD control data buffer address");
#else
	VSP_WRITE_REG(VSP_MEMO10_ADDR+20*4, ((uint32)Mp4Dec_ExtraMem_V2P(g_cmd_info_base, HW_CACHABLE))>>2, "config CMD control info buffer address");
	VSP_WRITE_REG(VSP_MEMO10_ADDR+21*4, ((uint32)Mp4Dec_ExtraMem_V2P(g_cmd_data_base, HW_CACHABLE))>>2, "config CMD control data buffer address");
     //VSP_WRITE_REG(VSP_MEMO10_ADDR+20*4, ((uint32)((uint32)g_cmd_info_base -(uint32)s_CMD_mem_cache_bfr_ptr+(uint32)s_CMD_mem_cache_bfr_phy_ptr))>>2, "config CMD control info buffer address");
	 //VSP_WRITE_REG(VSP_MEMO10_ADDR+21*4, ((uint32)((uint32)g_cmd_data_base -(uint32)s_CMD_mem_cache_bfr_ptr+(uint32)s_CMD_mem_cache_bfr_phy_ptr))>>2, "config CMD control data buffer address");
	...
	VSP_WRITE_REG(VSP_MEMO10_ADDR+ 8, bitstm_addr>>2, "AHBM_FRM_ADDR_6: source bitstream buffer0 address");
#endif
...
SCI_TRACE_LOW("configure reconstruct frame Y %x",vop_mode_ptr->pCurRecFrame->pDecFrame->imgYAddr);
	if (vop_mode_ptr->post_filter_en)
	{
		VSP_WRITE_REG(pTableAddr+ 4, MAPaddr(vop_mode_ptr->pCurDispFrame->pDecFrame->imgYAddr), "configure display frame Y");
	}else
	{
		VSP_WRITE_REG(pTableAddr+ 4, MAPaddr(vop_mode_ptr->pCurRecFrame->pDecFrame->imgYAddr), "configure display frame Y");
	}

#if _CMODEL_
	VSP_WRITE_REG(pTableAddr+ 12, ((uint32)DC_AC_PREDICTION_ADDR)>>8, "config top coeff address");
#else
	VSP_WRITE_REG(pTableAddr+ 12, ((uint32)Mp4Dec_ExtraMem_V2P(vop_mode_ptr->pTopCoeff, HW_NO_CACHABLE))>>2, "config top coeff address");	
#endif 
	...
	if(IVOP != vop_mode_ptr->VopPredType)
	{
		if (vop_mode_ptr->pBckRefFrame->pDecFrame == NULL)
		{
			if (!vop_mode_ptr->post_filter_en)
			{
				VSP_WRITE_REG(pTableAddr+ 20, MAPaddr(vop_mode_ptr->pCurRecFrame->pDecFrame->imgYAddr), "configure backward reference frame Y");
			}else
			{
				SCI_TRACE_LOW("MMDEC_ERROR  %s %d",__FUNCTION__,__LINE__);
				return MMDEC_ERROR;
			}
		}else
		{
			VSP_WRITE_REG(pTableAddr+ 20, MAPaddr(vop_mode_ptr->pBckRefFrame->pDecFrame->imgYAddr), "configure backward reference frame Y");
		}
		...
		if(BVOP == vop_mode_ptr->VopPredType)
		{
			if (vop_mode_ptr->pFrdRefFrame->pDecFrame == NULL)
			{
				if (!vop_mode_ptr->post_filter_en)
				{			
					VSP_WRITE_REG(pTableAddr+ 16, MAPaddr(vop_mode_ptr->pCurRecFrame->pDecFrame->imgYAddr), "configure forward reference frame Y");
				}else
				{	SCI_TRACE_LOW("MMDEC_ERROR  %s %d",__FUNCTION__,__LINE__);
					return MMDEC_ERROR;
				}
			...
			{
				VSP_WRITE_REG(pTableAddr+ 16, MAPaddr(vop_mode_ptr->pFrdRefFrame->pDecFrame->imgYAddr), "configure forward reference frame Y");
			}
		}
	}
...
PUBLIC void Mp4Dec_InitBitstream(void *pOneFrameBitstream, int32 length)
{
	...
	VSP_WRITE_REG_CQM(VSP_BSM_REG_BASE+BSM_CFG1_OFF, 0, "BSM_CFG1: configure the bitstream address");
