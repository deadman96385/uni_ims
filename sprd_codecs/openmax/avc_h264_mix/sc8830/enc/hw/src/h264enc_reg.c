...
//	OR1200_WRITE_REG(ORSC_VSP_GLB_OFF + 0x64, cmd, "ORSC: BSM1_FRM_ADDR");
	...
	cmd	= H264ENC_GetPhyAddr(g_vlc_hw_ptr)>>3;		// Frm_addr3[31:0], VLC Table set by Fixed Table Address
	OR1200_WRITE_REG(ORSC_FMADD_TBL_OFF + 0xC, cmd, "ORSC: Frm_addr3: Start address of VLC table");
...
#else
	cmd	= ((uint32)g_enc_image_ptr->OneFrameBitstream_addr_phy) >> 3;	// Bsm_buf0_frm_addr[31:0]
#endif
