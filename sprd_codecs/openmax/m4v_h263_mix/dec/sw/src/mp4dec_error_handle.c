...
LOCAL void BI_SpatialConceal(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_BFR_T  * mb_cache_ptr, int32 nei_avail)
{
    int32 mb_x = vop_mode_ptr->mb_x;
    int32 mb_y = vop_mode_ptr->mb_y;
        ...
        for (mb_x = 0; mb_x < vop_mode_ptr->MBNumX; mb_x++)
        {
            ...
            if (curr_mb_dec_status < DECODED_NOT_IN_ERR_PKT) //not coded or coded but in error packet
            {
                ...
                if (is_error)
                {
                    ...
                    if (nei_intra_num > 1)
                    {
                        BI_SpatialConceal (vop_mode_ptr, mb_cache_ptr, nei_avail);
                    } else
                    {
