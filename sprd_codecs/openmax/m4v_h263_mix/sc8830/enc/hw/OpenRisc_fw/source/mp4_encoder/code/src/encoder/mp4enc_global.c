...
//	OR1200_WRITE_REG(AHB_BASE_ADDR + 0, cmd, "AHB: ARM_ACCESS_CTRL: Shared_RAM = IMCU, VSP_Glb_Reg = IMCU");
	...
	OR1200_WRITE_REG(GLB_REG_BASE_ADDR + 0x28, cmd, "ORSC: RAM_ACC_SEL: SETTING_RAM_ACC_SEL=0(SW)");

	cmd = 0;
	cmd |= VSP_MPEG4 << 0;	// VSP_STAND[3:0], 0x0100:H264, 0x0010:VP8, 0x0001:VSP_MPEG4
	...
	cmd |= 1 << 31;		// VSP_RUN[31]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0, cmd, "ORSC_SHARE: MODE_CFG: H264 Encode");

	cmd = 0;
	cmd |= 352/*g_enc_image_ptr->frame_width_in_mbs*/ << 0;	// MB_X_MAX[7:0]
	cmd |= 288/*g_enc_image_ptr->frame_height_in_mbs*/ << 8;	// MB_Y_MAX[15:8]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x4, cmd, "AHB: IMG_SIZE: Set MB_X_MAX & MB_Y_MAX");

	cmd = INTER_MALLOC_MEM_START_ADDR;	// VSP_MEM0_ST_ADDR[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x8, cmd, "ORSC_SHARE: VSP_MEM0_ST_ADDR: Start address of code run (heap & stack ) memory space");
	cmd = TOTAL_INTER_MALLOC_SIZE;		// CODE_RUN_SIZE[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xc, cmd, "ORSC_SHARE: CODE_RUN_SIZE: Size for code run, start from VSP_MEM0_ST_ADDR");

	cmd = EXTRA_MALLOC_MEM_START_ADDR;	// VSP_MEM1_ST_ADDR[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x10, cmd, "ORSC_SHARE: VSP_MEM1_ST_ADDR: Start address of frame and mb_info memory space");
	cmd = TOTAL_EXTRA_MALLOC_SIZE;		// VSP_MEM1_SIZE[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x14, cmd, "ORSC_SHARE: VSP_MEM1_SIZE: Size of frame and mb_info memory space(byte)");

	cmd = BS_START_ADDR;	// STREAM_BUF_ADDR[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x18, cmd, "ORSC_SHARE: STREAM_BUF_ADDR: Bit stream address. Input for decoder, output for encoder");
	cmd = BS_BUF_SIZE;		// STREAM_BUF_SIZE[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x1c, cmd, "ORSC_SHARE: STREAM_BUF_SIZE: Indicate bit stream buffer size (byte)");

	cmd = 0;				// stream_len[31:0]
//	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x20, cmd, "AHB: stream_len: Length of used size in stream buffer");

	cmd = VLC_TABLE_BFR_ADDR;	// VLC_TABLE_ST_ADDR[31:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x24, cmd, "ORSC_SHARE: VLC_TABLE_ST_ADDR");
	cmd = VLC_TABLE_BFR_SIZE;	// VLC_TABLE_SIZE[15:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x28, cmd, "ORSC_SHARE: VLC_TABLE_SIZE");

	cmd = FRAME_BUF_SIZE;	// UV_FRAME_BUF_SIZE[15:0]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x34, cmd, "ORSC_SHARE: UV_FRAME_BUF_SIZE");

	//james:not enable rate control
	/*cmd = 0;
	cmd |= TARGET_BITRATE&0x7fffffff;	// Target_rate[30:0]
	cmd |= (1 << 31);	// RATE_CONTROL_EN[31]
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0x38, cmd, "ORSC_SHARE: RATE_CONTROL");*/
...
#else
	cmd	= ((uint32)g_enc_vop_mode_ptr->pOneFrameBitstream) >> 3;	// Bsm_buf0_frm_addr[31:0]
#endif
