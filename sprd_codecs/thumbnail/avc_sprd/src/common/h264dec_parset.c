...
//LOGI("%s, %d", __FUNCTION__, __LINE__);
	...
	{
#ifdef _VSP_DEC_	
		img_ptr->ext_width  = img_ptr->width;
		img_ptr->ext_height = img_ptr->height;
		img_ptr->start_in_frameY = 0;
		img_ptr->start_in_frameUV = 0;
...
//	LOGI("%s, %d", __FUNCTION__, __LINE__);
		...
		if(img_ptr->VSP_spsCb)
		{
			int ret = (*img_ptr->VSP_spsCb)(img_ptr->avc_user_data,img_ptr->width,img_ptr->height,sps_ptr->num_ref_frames);
			if(!ret)
			{
...
//LOGI("%s, %d", __FUNCTION__, __LINE__);
	...
	if (!img_ptr->is_cabac) //UVLC
	{
		...
		if (img_ptr->VSP_used)
		{
			img_ptr->sw_vld_mb= decode_mb_cavlc_hw;
		}else
#endif		
		{
			img_ptr->sw_vld_mb= decode_mb_cavlc_sw;		
		}
	}else
	{
		...
		if (img_ptr->VSP_used)
		{
		//	img_ptr->sw_vld_mb= decode_mb_cabac_hw;	
		}else
#endif		
		{
			img_ptr->sw_vld_mb= decode_mb_cabac_sw;	
		}
	}
...
//    LOGI("%s, %d, pic_width: %d, pic_height: %d", __FUNCTION__, __LINE__, sps_ptr->pic_width_in_mbs_minus1, sps_ptr->pic_height_in_map_units_minus1);
...
LOCAL MMDecRet H264Dec_interpret_pps (DEC_PPS_T *pps_ptr, H264DecContext *img_ptr)
{
	...
	if (pps_ptr->num_slice_groups_minus1 > 0)
	{
		...
		{
			SCI_ASSERT(NULL != (pps_ptr->slice_group_id = (uint8 *)H264Dec_InterMemAlloc(img_ptr, SIZE_SLICE_GROUP_ID*sizeof(uint8), 4)));
		}
