...
void quant(int16 *pDct, int32 Inter_mode_en, int32 standard, int32 blk_idx)
{
	...
	int32 i;	
	int32 *noneZeroflag_ptr = vsp_dct_io_1 + 192 + 4*blk_idx;
	int		map[8] = {0, 4, 2, 6, 1, 5, 3, 7};//weihu
	...
	}else //JPEG
	{
		...
		for (i = 0; i < 64; i++)
		{
			...
			quant_shift	= (quant_tbl_ptr[i]&0xF) + 11;
			quant_val = ((quant_tbl_ptr[i]>>4) & 0xfff) ;
		#endif
			...
			nzFlag(map[i&7] * 8 + map[i>> 3], pDct [i], blk_idx);
		}
	}
}
		...
		{	
			...
			if(standard != VSP_JPEG)
			{
				...
				}else
				{
					...
					IQCoeff = ((2 * coeff + k)* quant_tbl_ptr[g_ASIC_DCT_Matrix[i]] * qp)>>4;//weihu
				//	IQCoeff = ((2 * coeff + k)* quant_tbl_ptr[i] * qp)>>4;
					pDct [i] = (int16)(IQCoeff * sign);
				}
			}else
			{
			#if defined(JPEG_DEC)
		//		pDct [i] = pDct [i] * quant_tbl_ptr [jpeg_fw_ASIC_DCT_Matrix[i]];//weihu
				pDct [i] = pDct [i] * quant_tbl_ptr [i];
			#endif
			}
