...
LOCAL void init_dequant8_coeff_table(H264DecContext *img_ptr){
    ...
    for(i=0; i<2; i++ ){
        ...
        for(j=0; j<i; j++){
            if(!memcmp(img_ptr->g_active_pps_ptr->ScalingList8x8[j],img_ptr->g_active_pps_ptr->ScalingList8x8[i], 64*sizeof(uint8))){
        ...
        for(q=0; q<52; q++){
            ...
            for(x=0; x<64; x++)
                img_ptr->dequant8_coeff[i][q][x] =
                   ...
                   img_ptr->g_active_pps_ptr->ScalingList8x8[i][x]) << shift;
        }
    }
}
...
LOCAL void init_dequant4_coeff_table(H264DecContext *img_ptr){
    ...
    for(i=0; i<6; i++ ){
       img_ptr->dequant4_coeff[i] = img_ptr->dequant4_buffer[i];
        for(j=0; j<i; j++){
            if(!memcmp(img_ptr->g_active_pps_ptr->ScalingList4x4[j], img_ptr->g_active_pps_ptr->ScalingList4x4[i], 16*sizeof(uint8))){
        ...
        for(q=0; q<52; q++){
            ...
            for(x=0; x<16; x++)
               img_ptr->dequant4_coeff[i][q][x] =
                   ...
                   img_ptr->g_active_pps_ptr->ScalingList4x4[i][x]) << shift;
        }
    }
}
...
LOCAL void init_dequant_tables(H264DecContext *img_ptr){
    ...
    if(!img_ptr->g_active_pps_ptr->pic_scaling_matrix_present_flag)
    {
	memcpy(img_ptr->g_active_pps_ptr->ScalingList4x4, img_ptr->g_active_sps_ptr->ScalingList4x4,6*16*sizeof(uint8));
	memcpy(img_ptr->g_active_pps_ptr->ScalingList8x8, img_ptr->g_active_sps_ptr->ScalingList8x8,2*64*sizeof(uint8));
    }
...
//    		LOGI("%s, %d, img_ptr->error_flag: %d", __FUNCTION__, __LINE__, img_ptr->error_flag);
...
//					SCI_TRACE_LOW("bind in output_frame_func\t");
					(*avc_bindCb)(avc_user_data,dpb_ptr->fs[i]->frame->pBufferHeader);
				}
#endif
			}
		}
