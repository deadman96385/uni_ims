...
PUBLIC MMDecRet H264DecDecode_NALU(MMDecInput *dec_input_ptr, MMDecOutput *dec_output_ptr)
{
	...
	ret = H264Dec_Read_SPS_PPS_SliceHeader (dec_input_ptr);//g_nalu_ptr->buf, g_nalu_ptr->len);//weihu
...
PUBLIC MMDecRet H264DecDecode(AVCHandle *avcHandle, MMDecInput *dec_input_ptr, MMDecOutput *dec_output_ptr)
{
	...
	OR1200_READ_REG_POLL(BSM_CTRL_REG_BASE_ADDR+BSM_DBG0_OFF, V_BIT_27,0x0,"BSM_clr enable");//check bsm is idle	
	OR1200_WRITE_REG(GLB_REG_BASE_ADDR+BSM0_FRM_ADDR_OFF, bs_start_addr/8,"BSM_buf0 addr");
	OR1200_WRITE_REG(BSM_CTRL_REG_BASE_ADDR+BSM_OP_OFF, V_BIT_2|V_BIT_1,"BSM_OP clr BSM");//clr BSM
			
	OR1200_WRITE_REG(BSM_CTRL_REG_BASE_ADDR+BSM_CFG1_OFF, ((V_BIT_31|V_BIT_30)|g_stream_offset),"BSM_cfg1 check startcode");//byte align
	OR1200_WRITE_REG(BSM_CTRL_REG_BASE_ADDR+BSM_CFG0_OFF, (V_BIT_31|bs_buffer_length),"BSM_cfg0 stream buffer size");//BSM load data
  	OR1200_READ_REG_POLL(BSM_CTRL_REG_BASE_ADDR+BSM_DBG1_OFF, V_BIT_2,V_BIT_2,"startcode found");//check bsm is idle	
	...
	while(g_stream_offset<bs_buffer_length)
	{
		...
		OR1200_WRITE_REG(GLB_REG_BASE_ADDR+BSM0_FRM_ADDR_OFF, bs_start_addr/8,"BSM_buf0 addr");
		OR1200_WRITE_REG(BSM_CTRL_REG_BASE_ADDR+BSM_OP_OFF, V_BIT_2|V_BIT_1,"BSM_OP clr BSM");//clr BSM
		OR1200_WRITE_REG(BSM_CTRL_REG_BASE_ADDR+BSM_CFG1_OFF, (V_BIT_31|g_stream_offset),"BSM_cfg1 stream buffer offset");//point to the start of NALU.
		OR1200_WRITE_REG(BSM_CTRL_REG_BASE_ADDR+BSM_CFG0_OFF, V_BIT_31|((bs_buffer_length+128)&0xfffffffc),"BSM_cfg0 stream buffer size");// BSM load data. Add 16 DW for BSM fifo loading.

		ret = H264DecDecode_NALU(dec_input_ptr, dec_output_ptr);

        SCI_TRACE_LOW("%s, %d, g_slice_datalen: %d, g_stream_offset: %d, bs_buffer_length: %d, frame_dec_finish: %d,ret:  %d ", __FUNCTION__, __LINE__, g_slice_datalen, g_stream_offset, bs_buffer_length, frame_dec_finish, ret);	
