...
**---------------------------------------------------------------------------*/
 ...
 void dump_cmd_info()
{	
  	FILE* info = fopen("/data/m4v_info.bin","wb");
	FILE* data = fopen("/data/m4v_data.bin","wb");  
...
void dump_output_yuv()
{
	...
	unsigned char * src_y = g_rec_buf.imgY;
	unsigned char * src_u= src_y+width*height;
	unsigned char * src_v = src_u+width*height/4;
	
	if(g_nFrame_dec == 0 /*|| g_nFrame_dec ==  243*/)
	{
		output_yuv = fopen("/data/m4v_out.yuv","wb");  
	}
...
#if _DEBUG_
if(vop_mode_ptr->is_first_frame)
         	dump_cmd_info();
#endif
...
if(g_nFrame_dec == 0)
{
	FILE*bits = fopen("/data/ffmpeg.m4v","wb");

	SCI_TRACE_LOW("vop_mode_ptr->frame_bistrm_ptr %x",vop_mode_ptr->frame_bistrm_ptr);
	if(bits!= NULL)
		fwrite(vop_mode_ptr->frame_bistrm_ptr,1,dec_input_ptr->dataLen,bits);
	fclose(bits);
		...
		//	SCI_TRACE_LOW( "nframe: %d, Y: %d, X: %d\n", g_nFrame_dec, pos_y, pos_x);
			...
			if (vop_mode_ptr->error_flag)
			{
				...
				//	vop_mode_ptr->VopPredType = IVOP;  //here, set it's original value
					if (ret != MMDEC_OK)
					{
						SCI_TRACE_LOW("Mp4Dec_EC_FillMBGap ERROR %s %d",__FUNCTION__,__LINE__);
						return ret;
					}
