...
//		SCI_TRACE_LOW ("\t B VOP\t%d\n", dec_input_ptr->dataLen);
	...
	//	ret = Mp4Dec_frm_level_sync_hw_sw_normal (dec_input_ptr, dec_output_ptr, vop_mode_ptr);
		...
		//ALOGE("hulei add frame_height = %d, frame_width = %d", dec_output_ptr->frame_height ,dec_output_ptr->frame_width);
		//dump_yuv(vop_mode_ptr->pCurRecFrame->pDecFrame->imgY, dec_output_ptr->frame_height * dec_output_ptr->frame_width );
		//dump_yuv(vop_mode_ptr->pCurRecFrame->pDecFrame->imgU, dec_output_ptr->frame_height * dec_output_ptr->frame_width  / 4);
		//dump_yuv(vop_mode_ptr->pCurRecFrame->pDecFrame->imgV, dec_output_ptr->frame_height * dec_output_ptr->frame_width  / 4);
		//dump_yuv(dec_output_ptr->pOutFrameY, dec_output_ptr->frame_height * dec_output_ptr->frame_width *3 /2 );
	}else
	{
	#if 0
