...
MMDecRet Mp4Dec_DecH263PlusHeader(Mp4DecObject *vo)
{
    DEC_VOP_MODE_T *vop_mode_ptr = vo->vop_mode_ptr;
    DEC_BS_T *bitstrm_ptr = vop_mode_ptr->bitstrm_ptr;
    H263_PLUS_HEAD_INFO_T *h263_plus = vo->h263_plus_head_info_ptr;
    int32 tmp;
...
PUBLIC MMDecRet Mp4Dec_DecH263Header(Mp4DecObject *vo)
{
    ...
    vo->g_dec_pre_vop_format = (VOP_PRED_TYPE_E)tmpVar;

    if(tmpVar == EXTENDED_PTYPE)
    {
        ret = Mp4Dec_DecH263PlusHeader(vo);
...
PUBLIC MMDecRet Mp4Dec_FlvH263PicHeader(Mp4DecObject *vo)
{
    ...
    vo->g_dec_pre_vop_format = (VOP_PRED_TYPE_E) format;
