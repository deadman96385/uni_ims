       ...
       switch (nParamIndex) {
	    ...
	    case OMX_IndexParamGetAndroidNativeBuffer:
	    {
			GetAndroidNativeBufferUsageParams *pganbp;
    			...
    			//pganbp->nUsage = GRALLOC_USAGE_HW_VIDEO_ENCODER;
 ...
 void OpenmaxMpeg4AO::ReleaseReferenceBuffers()
{
    if (ipMpegDecoderObject)
    {
		...
		do{
			...
			status = ipMpegDecoderObject->FlushOutput_OMX(&ipOutputBufferForRendering);
			SCI_TRACE_LOW("Seeklog :::iFlushOutputStatus% d\n",(int)status);
	...
	OMX_ERRORTYPE ret = OMX_ErrorNone;
       ...
       switch (nIndex) {
	    case OMX_IndexConfigCommonOutputCrop:
	    {
			...
			SCI_TRACE_LOW("OpenmaxMpeg4AO OMX_IndexConfigCommonOutputCrop %d,%d,%d,%d",rectParams->nLeft,rectParams->nTop,rectParams->nWidth,rectParams->nHeight);	
	    }	
			break;
	    default:
