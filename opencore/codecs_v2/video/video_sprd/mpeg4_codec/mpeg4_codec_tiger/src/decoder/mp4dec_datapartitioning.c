...
PUBLIC void Mp4Dec_RvlcIntraTCOEF(DEC_VOP_MODE_T *vop_mode_ptr, int16 * iDCTCoefQ, int32 iCoefStart)
{
	...
	do 
	{
		run_level = Mp4Dec_GetIntraRvlcTcoef(vop_mode_ptr, bitstrm_ptr);
...
PUBLIC void Mp4Dec_RvlcInterTCOEF(DEC_VOP_MODE_T *vop_mode_ptr, int16 *iDCTCoefQ)
{
	...
	do 
	{
		run_level = Mp4Dec_GetInterRvlcTcoef(vop_mode_ptr, bitstrm_ptr);
...
MMDecRet Mp4Dec_ResyncDataPartitioning (DEC_VOP_MODE_T *vop_mode_ptr, int32 start_mb_no, uint32 uAddBits)
{
	/*error concealment*/
	int32 rsc_found;
...
PUBLIC MMDecRet Mp4Dec_DecPVOPErrResDataPartitioning(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	while(!vop_mode_ptr->stop_decoding)
	{
		...
		/**/
		if(Mp4Dec_CheckResyncMarker(uAddBits) && (vop_mode_ptr->mbnumDec < vop_mode_ptr->MBNum))
		{
