...
#else
PUBLIC  int32 read_reg_poll(uint32 reg_addr, uint32 msk,uint32 exp_value, uint32 time, char *pstring);
...
//#define VSP_WRITE_REG(reg_addr, value, pstring) do{SCI_TRACE_LOW("%s,%d",__FUNCTION__,__LINE__);\
//	(*(volatile uint32 *)(reg_addr-VSP_DCAM_BASE+s_vsp_Vaddr_base)  = (value));\
//	SCI_TRACE_LOW("value %x",value);}while(0)
#define VSP_WRITE_REG(reg_addr, value, pstring) (*(volatile uint32 *)(reg_addr-VSP_DCAM_BASE+s_vsp_Vaddr_base)  = (value))   
#define VSP_READ_REG(reg_addr, pstring)	((*(volatile uint32 *)(reg_addr-VSP_DCAM_BASE+s_vsp_Vaddr_base)))

#define VSP_READ_REG_POLL(reg_addr, msk, exp_value, time, pstring) read_reg_poll(reg_addr, msk, exp_value, time, pstring) 
//command queue mode
//#define VSP_WRITE_REG_CQM(reg_addr, value, pstring) do{SCI_TRACE_LOW("%s,%d",__FUNCTION__,__LINE__);\
