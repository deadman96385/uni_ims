...
MMEncRet H264EncGenHeader(MMEncOut *pOutput, int is_sps)
{
	...
	OR_VSP_RST();		
        ...
        if (is_sps)
        {
            command = H264ENC_GEN_SPS;
        }else
        {
            command = H264ENC_GEN_PPS;
        }

        // Send H264ENC_GEN_SPS/PPS signal to Openrisc.
        VSP_WRITE_REG(SHARE_RAM_BASE_ADDR+0x58, command,"Call  H264ENC_GEN_HEADER function.");	
