...
static const char _stateName[] = "RADIO:CS";
    ...
    RPM_Event    *event_ptr)
{
    ...
    RFSM_State *nextState_ptr = (RFSM_State *)&_RFSM_STATE_CS;
    ...
    RPM_Event    *event_ptr)
{
    ...
    context_ptr->csState = RPM_SERVICE_STATE_ACTIVE;
    /* Set up radio interface for CS */
    context_ptr->radioInfc.radioType = RPM_RADIO_TYPE_CS;
    /* Update emergency radio infc */
    if (RPM_RADIO_TYPE_LTE_EMERGENCY !=
            context_ptr->emergencyRadioInfc.radioType) {
        /* Emergency service is not active, change to CS */
        context_ptr->emergencyRadioInfc.radioType = RPM_RADIO_TYPE_CS;
    }
}
 ...
 */
const RFSM_State _RFSM_STATE_CS = {
