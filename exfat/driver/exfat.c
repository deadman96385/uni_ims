...
INT32 ffsMapCluster(struct inode *inode, INT32 clu_offset, UINT32 *clu, int create)
{
	...
	*clu = last_clu = fid->start_clu;
	...
	if (!create && *clu == CLUSTER_32(~0)) {
		printk("[EXFAT] error: no mapping for the %u-th cluster\n", clu_offset);
		printk("FID: {{%d,%d,%d},%d,%d,%d,%d,  %llu,%d,%llu,%d,%d}\n",
			fid->dir.dir, fid->dir.size, fid->dir.flags,
			fid->entry, fid->type, fid->attr, fid->start_clu,
			fid->size, fid->flags, fid->rwoffset, fid->hint_last_off, fid->hint_last_clu);
		printk("Inode: %llu, map:%llu, block: %d\n", i_size_read(inode), EXFAT_I(inode)->mmu_private, inode->i_blocks);
		dump_stack();
		return FFS_MEDIAERR;
	}
