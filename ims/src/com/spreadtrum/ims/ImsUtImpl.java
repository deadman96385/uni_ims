...
import com.android.internal.telephony.CallForwardInfoSprd;
...
public class ImsUtImpl extends IImsUt.Stub {
                    ...
                    try {
                        if(ar != null){
                            if (ar.exception != null) {
                                ...
                                CallForwardInfoSprd infos[] = (CallForwardInfoSprd[]) ar.result;
                                if(infos.length == 0 || ar.userObj instanceof Throwable){
                                    mImsUtListener.utConfigurationQueryFailed((IImsUt)ar.userObj,
                                            msg.arg1,
                                            new ImsReasonInfo(ImsReasonInfo.CODE_UT_OPERATION_NOT_ALLOWED, 0));
                                } else {
                                    CallForwardInfoSprd cfInfo;
                                    ImsCallForwardInfoEx callForwardInfo;
                                    ImsCallForwardInfoEx[] callForwardInfoList
                                    = new ImsCallForwardInfoEx[infos.length];
                                    for (int i = 0; i < infos.length; i++) {
                                        cfInfo = infos[i];
                                        callForwardInfo = new ImsCallForwardInfoEx();
                                        callForwardInfo.mStatus = cfInfo.status;
                                        callForwardInfo.mCondition = cfInfo.reason;
                                        callForwardInfo.mToA = cfInfo.toa;
                                        callForwardInfo.mNumber = cfInfo.number;
                                        callForwardInfo.mTimeSeconds = cfInfo.timeSeconds;
                                        callForwardInfo.mServiceClass = cfInfo.serviceClass;
                                        callForwardInfo.ruleset = cfInfo.ruleset;
                                        callForwardInfo.numberType = cfInfo.numberType;
                                        callForwardInfoList[i] = callForwardInfo;
                                        Log.i(TAG,"ACTION_QUERY_CF->callForwardInfo:" + callForwardInfo.toString());
                                    }
                                    mImsUtListener.utConfigurationCallForwardQueriedEx(
                                            (IImsUt)ar.userObj, msg.arg1, callForwardInfoList);
                                }
                            }
                        }
                    ...
                    try {
                        if (ar != null) {
                            if (ar.exception != null) {
                                ...
                                mImsUtListener.utConfigurationCallBarringFailed(msg.arg1, null, error.ordinal());
                            } else {
                                if (ar.result != null) {
                                    mImsUtListener.utConfigurationCallBarringResult(msg.arg1,
                                            (int[])ar.result);
                                } else {
                                    mImsUtListener.utConfigurationCallBarringResult(msg.arg1, null);
                                }
                            }
                        } else {
                            mImsUtListener.utConfigurationCallBarringFailed(msg.arg1, null, Error.GENERIC_FAILURE.ordinal());
                        }
                    } catch (RemoteException e) {
                        e.printStackTrace();
                    }
    ...
    public int changeBarringPassword(String facility, String oldPwd, String newPwd) {
        Log.i(TAG, "onexcue changeBarringPassword" );
        int id = getReuestId();
    ...
    public int setFacilityLock(String facility, boolean lockState, String password,
            int serviceClass) {
        Log.i(TAG, "onexcue setFacilityLock ");
        int id = getReuestId();
    ...
    public int queryFacilityLock(String facility, String password, int serviceClass) {
        Log.i(TAG, "onexcue queryFacilityLock");
        int id = getReuestId();
