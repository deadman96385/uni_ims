...
/*	DBK_FPRINTF (g_dbk_trace_fp, "U component:\n");
	pix_ptr = (uint8 *)(g_dbk_line_buf + U_OFFSET_LINE_BUF + mb_x*2 + 2*64);
	for (i = 0; i < 2; i++)
	{
		DBK_FPRINTF (g_dbk_trace_fp,"            ");
		for (j = 0; j < 8; j++)
	...
	pix_ptr = (uint8 *)(vsp_dbk_out_bfr + U_OFFSET_BUF_C + 12) + 2;
	for (i = 0; i < 8; i++)
	{
		for (j = 0; j < 10; j++)
		{
...
/*	DBK_FPRINTF (g_dbk_trace_fp, "V component:\n");
	pix_ptr = (uint8 *)(g_dbk_line_buf + V_OFFSET_LINE_BUF + mb_x*2 + 2*64);
	for (i = 0; i < 2; i++)
	{
		DBK_FPRINTF (g_dbk_trace_fp,"            ");
		for (j = 0; j < 8; j++)
	...
	pix_ptr = (uint8 *)(vsp_dbk_out_bfr + V_OFFSET_BUF_C + 12) + 2;
	for (i = 0; i < 8; i++)
	{
		for (j = 0; j < 10; j++)
		{
...
if(g_trace_enable_flag&TRACE_ENABLE_DBK)
{
	...
	/*U component*/
/*	DBK_FPRINTF (g_dbk_trace_fp, "U component:\n");
	pix_ptr = (uint8 *)(vsp_dbk_out_bfr + U_OFFSET_BUF_C + 4*2 + 2);
	for (i = 0; i < 2; i++)
	{
		DBK_FPRINTF (g_dbk_trace_fp,"            ");
		for (j = 0; j < 8; j++)
	...
	pix_ptr = (uint8 *)(vsp_dbk_out_bfr + U_OFFSET_BUF_C + 16) + 2;
	for (i = 0; i < 4; i++)
	{
		for (j = 0; j < 10; j++)
		{
	...
	/*V component*/
	/*DBK_FPRINTF (g_dbk_trace_fp, "V component:\n");
	pix_ptr = (uint8 *)(vsp_dbk_out_bfr + V_OFFSET_BUF_C + 3 *2 + 1);
	for (i = 0; i < 2; i++)
	{
		DBK_FPRINTF (g_dbk_trace_fp,"            ");
		for (j = 0; j < 8; j++)
	...
	pix_ptr = (uint8 *)(g_dbk_line_buf + V_OFFSET_LINE_BUF + mb_x * 2 - 1) + 2;
	for (i = 0; i < 4; i++)
	{
		for (j = 0; j < 10; j++)
		{
