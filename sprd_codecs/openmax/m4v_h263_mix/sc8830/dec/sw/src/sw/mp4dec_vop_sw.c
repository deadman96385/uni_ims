 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecIVOP_sw(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 pos_y, pos_x;
...
#endif
PUBLIC MMDecRet Mp4Dec_DecPVOP_sw(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 pos_y, pos_x;
	DEC_MB_MODE_T * mb_mode_ptr = vop_mode_ptr->pMbMode;
	DEC_MB_BFR_T  * mb_cache_ptr = vop_mode_ptr->mb_cache_ptr;
	...
	{	
		...
		{	
			...
			{
				Mp4Dec_DecInterMBTexture_sw(vd, vop_mode_ptr, mb_mode_ptr, mb_cache_ptr);				
			}
 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecBVOP_sw(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	MMDecRet ret = MMDEC_OK;
#if 0
	int32 pos_y, pos_x;
	...
	{	
		...
		for(pos_x = 0; pos_x < total_mb_num_x; pos_x++)
		{
			...
			{	
				...
				Mp4Dec_DecInterMBTexture_sw(vd, vop_mode_ptr, mb_mode_bvop_ptr,mb_cache_ptr);
			}
			else
			{
