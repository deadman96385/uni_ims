    ...
    OSAL_Boolean *timeout_ptr)
{
    ...
    switch (msTimeout) {
        ...
        case OSAL_NO_WAIT:
            if (q_ptr->sz == (unsigned)write(q_ptr->fid,
                    buffer_ptr,
                    q_ptr->sz)) {
                status = OSAL_SUCCESS;
            }
 ...
 * ======== OSAL_msgQSimpInit() ========
 *
 * This function is used to create a FIFO queue.
 *
 * Returns:
...
OSAL_MsgQId OSAL_msgQSimpInit(
    int size)
{
    _OSAL_MsgQSimpParams *q_ptr;
    if (size <= 0) {
        return (NULL);
    }

    if (NULL == (q_ptr = (_OSAL_MsgQSimpParams *)OSAL_memCalloc(
           sizeof(_OSAL_MsgQSimpParams),
           1,
           0))) {
        return(NULL);
    }
    
    if (NULL == (q_ptr->array_ptr = (int *)OSAL_memCalloc(
           size * sizeof(int32),
           1,
           0))) {
        OSAL_memFree(q_ptr, 0);
    ...
    q_ptr->sem = OSAL_semMutexCreate();
    q_ptr->size = size;
    if (NULL == q_ptr->sem) {
        OSAL_memFree(q_ptr->array_ptr, 0);
        OSAL_memFree(q_ptr, 0);
        return (NULL);
    }
    return ((OSAL_MsgQId)q_ptr);
 ...
 * ======== OSAL_msgQSimpDelete() ========
 *
 * This function is used to delete a FIFO queue.
 *
 * Returns:
 * OSAL_SUCCESS : Success
 * OSAL_FAIL : Unable
...
OSAL_Status OSAL_msgQSimpDelete(
    OSAL_MsgQId qId)
{
    _OSAL_MsgQSimpParams *q_ptr = (_OSAL_MsgQSimpParams *)qId;
    if (NULL == q_ptr) {
        return (OSAL_FAIL);
    }

    if (OSAL_SUCCESS == OSAL_memFree(q_ptr->array_ptr, 0)) {
        if (OSAL_SUCCESS == OSAL_semDelete(q_ptr->sem)) {
            if (OSAL_SUCCESS == OSAL_memFree(q_ptr, 0)) {
                return (OSAL_SUCCESS);
            }
        }
    }
 ...
 * ======== OSAL_msgQSimpGet() ========
 *
 * This function is used to get a value from a FIFO queue.
 *
 * Returns:
 * OSAL_SUCCESS : Success
 * OSAL_FAIL : Unable
...
OSAL_Status OSAL_msgQSimpGet(
    ...
    uint32       msTimeout)
{
    OSAL_Status ret = OSAL_FAIL;
    _OSAL_MsgQSimpParams *q_ptr = (_OSAL_MsgQSimpParams *)qId;
    if (NULL == q_ptr) {
        return (OSAL_FAIL);
    }

    if (OSAL_FAIL != OSAL_semAcquire(q_ptr->sem, msTimeout)) {
        if (q_ptr->head != q_ptr->tail) {
            *val_ptr = q_ptr->array_ptr[q_ptr->tail];
            q_ptr->tail++;
            if (q_ptr->tail >= q_ptr->size) {
                q_ptr->tail = 0;
            }
            ret = OSAL_SUCCESS;
        }
    }
    OSAL_semGive(q_ptr->sem);
    return (ret);
}
 ...
 * ======== OSAL_msgQSimpPut() ========
 *
 * This function is used to put a value in a FIFO queue.
 *
 * Returns:
 * OSAL_SUCCESS : Success
 * OSAL_FAIL : Unable
    ...
    uint32      msTimeout)
{
    ...
    if (OSAL_FAIL != OSAL_semAcquire(q_ptr->sem, msTimeout)) {
        if (!((((q_ptr->size - 1) == q_ptr->head) && 
                (0 == q_ptr->tail)) ||
                (1 == (q_ptr->tail - q_ptr->head)))) {
            q_ptr->array_ptr[q_ptr->head] = val;
            q_ptr->head++;
            if (q_ptr->head >= q_ptr->size) {
                q_ptr->head = 0;
            }
            ret = OSAL_SUCCESS;
        }
