...
PUBLIC void H264Dec_init_img_buffer (DEC_IMAGE_PARAMS_T *img_ptr)
{
	...
	img_ptr->frame_bistrm_ptr = (uint8 *)H264Dec_ExtraMemAlloc_64WordAlign (H264DEC_FRM_STRM_BUF_SIZE);
...
//		g_vsp_cmd_info_base = (uint32 *) H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256*2);
		...
		g_cmd_data_base = (uint32 *)H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256 * 6);
		g_cmd_info_base = (uint32 *)H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256*2);
	}
	else
	{
//		g_vsp_cmd_data_base = (uint32 *) H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256 * 3);
//		g_vsp_cmd_info_base = (uint32 *) H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256*1);
	
		g_cmd_data_base = (uint32 *)H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256 * 3);
		g_cmd_info_base = (uint32 *)H264Dec_ExtraMemAlloc_64WordAlign(total_mb_num * 256*1);
	}
...
PUBLIC void	H264Dec_init_dpb (DEC_IMAGE_PARAMS_T *img_ptr)
{
	...
	for (i = 0; i < MAX_REF_FRAME_NUMBER+1; i++)
	{
		...
		{
		//SCI_TRACE_LOW("dpb_ptr->fs[i] == PNULL");
			...
			dpb_ptr->fs[i]->frame->ref_idx_ptr[0] = (int8 *)H264Dec_ExtraMemCacheAlloc(frm_size_in_blk);
			dpb_ptr->fs[i]->frame->ref_idx_ptr[1] = (int8 *)H264Dec_ExtraMemCacheAlloc(frm_size_in_blk);
...
PUBLIC void H264Dec_reorder_list (DEC_IMAGE_PARAMS_T *img_ptr)
{
	DEC_SLICE_T	*currSlice = img_ptr->curr_slice_ptr;
