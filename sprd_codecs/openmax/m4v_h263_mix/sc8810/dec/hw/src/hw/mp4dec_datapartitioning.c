 ...
 **	Note:
 *****************************************************************************/
LOCAL void Mp4Dec_DecSecondPartitionIntraErrRes(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr, int32 start_mb_no)
{
 ...
 **	Note:
 *****************************************************************************/
LOCAL void Mp4Dec_DecSecondPartitionInterErrRes(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr, int32 start_mb_no)
{
	int32 mb_num = start_mb_no;
	DEC_MB_MODE_T *mb_mode_ptr = PNULL;
	int32 total_mb_num_x = vop_mode_ptr->MBNumX;
...
PUBLIC MMDecRet Mp4Dec_DecIVOPErrResDataPartitioning(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	while(!vop_mode_ptr->stop_decoding)
	{
		...
		if(vop_mode_ptr->VSP_used)
		{
			Mp4Dec_DecSecondPartitionIntraErrRes(vd, vop_mode_ptr, start_mb_no);
		}
		#if 0
		else
		{
...
PUBLIC MMDecRet Mp4Dec_DecPVOPErrResDataPartitioning(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	while(!vop_mode_ptr->stop_decoding)
	{
		...
		if(vop_mode_ptr->VSP_used)
		{
			Mp4Dec_DecSecondPartitionInterErrRes(vd, vop_mode_ptr, start_mb_no);
		}
		#if 0
		else
		{
