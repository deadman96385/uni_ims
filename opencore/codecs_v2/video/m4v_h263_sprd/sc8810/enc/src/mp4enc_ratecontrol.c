...
PUBLIC void Mp4Enc_InitRateCtrl(RateCtrlPara *rc_par_ptr, RCMode *rc_mode_ptr)
{
	...
	rc_par_ptr->rc_ena				= vop_mode_ptr->RateCtrlEnable;
	rc_par_ptr->bit_rate			= vop_mode_ptr->targetBitRate;
	rc_par_ptr->frame_rate			= vop_mode_ptr->FrameRate;
	rc_par_ptr->qp_i				= vop_mode_ptr->StepI;
	rc_par_ptr->qp_p				= vop_mode_ptr->StepP;
	rc_par_ptr->p_between_i			= vol_mode_ptr->PbetweenI;
...
//	rc_mode_ptr->Ns					= enc_ctr_ptr->samp_rate;
	...
	rc_mode_ptr->Bs					= vop_mode_ptr->vbv_buf_size;
...
//		rc_mode_ptr->B += nbit_fst_frame;	
	...
	PRINTF ("\t QP: %d\n", vop_mode_ptr->StepI);
	PRINTF ("\t BitsTotalCurr=%d\n\t buffer fullness: %f\n", nbit_fst_frame, (1.0*rc_mode_ptr->B + nbit_fst_frame - rc_mode_ptr->Rp) / rc_mode_ptr->Bs);

	FPRINTF (g_rgstat_fp, "\t QP: %d\n", vop_mode_ptr->StepI);
	FPRINTF (g_rgstat_fp, "\t buffer fullness: %f\n\t\ nBitsTotalCurr=%d\n", 
		(1.0*rc_mode_ptr->B + nbit_fst_frame - rc_mode_ptr->Rp)/rc_mode_ptr->Bs, nbit_fst_frame);
	FPRINTF (g_buf_full_pf, "%f\n", (1.0*rc_mode_ptr->B + nbit_fst_frame - rc_mode_ptr->Rp) / rc_mode_ptr->Bs);
#endif
}

static void RCModelEstimator (RCMode* rc_mode_ptr, uint32 n_windowSize)
{
	uint32 i;
	BOOLEAN estimateX2;
	int32 oneSampleQ = 0;
	...
	for (i = 0; i < n_windowSize; i++)	
	{
		...
		if ((rc_mode_ptr->rgQp[i] != oneSampleQ) && !rc_mode_ptr->rgRejected[i]) //lint !e644 ***this line caused a 644 warning that oneSampleQ is not initialized.
		{
			estimateX2 = TRUE;
		}
...
#if defined(SIM_IN_WIN)
	PRINTF("\t BitsTotalCurr=%d\n\t complex: %f\n\t buffer fullness: %f\n", 
		tot_bits_cur, Ec_Q8*1.0/256, (1.0*rc_mode_ptr->B + rc_mode_ptr->Rc - rc_mode_ptr->Rp) / rc_mode_ptr->Bs);

	FPRINTF(g_rgstat_fp, "\t BitsTotalCurr=%d\n\t complex: %f\n\t buffer fullness: %f\n", 
		tot_bits_cur, Ec_Q8*1.0/256, (1.0*rc_mode_ptr->B + rc_mode_ptr->Rc - rc_mode_ptr->Rp) / rc_mode_ptr->Bs);	

	FPRINTF (g_buf_full_pf, "%f\n", (1.0*rc_mode_ptr->B + rc_mode_ptr->Rc - rc_mode_ptr->Rp) / rc_mode_ptr->Bs);
#endif
...
static int32 Qp_FirstPinGop(ENC_VOP_MODE_T *vop_mode_ptr, RCMode *rc_mode_ptr)
{
	int32 Qp;
//	int32 Qp_sum = 0;
	...
	{
		Qp = vop_mode_ptr->StepI;
	}

	Qp = mmax(Qp, (vop_mode_ptr->StepI * (100 - RC_MAX_Q_INCREASE) + 99)/100);
	Qp = mmin(Qp, (vop_mode_ptr->StepI * (100 + RC_MAX_Q_INCREASE) + 99)/100);

	return Qp;
}

//comput target bits number according to its complex
static void GetTargetBits(RCMode *rc_mode_ptr, int32 Ec_Q8)
{
	int32 i;
	int32 Nc;
	int32 total_Ec_Q8 = 0;
	...
	rc_mode_ptr->T = rc_mode_ptr->Rp * Ec_Q8 * Nc / total_Ec_Q8; //lint !e795
...
static uint32 sqrt_simple(uint32 d)
{
	uint32 N = 16;
	uint32 t, q= 0, r =d;
...
//	int32 min_qp;
	...
	if (rc_mode_ptr->Nc == 0)
	{
		vop_mode_ptr->StepP = rc_mode_ptr->Qc = Qp_FirstPinGop(vop_mode_ptr, rc_mode_ptr);
...
//	according to complex to allocate target bits
	...
	if (rc_mode_ptr->is_pfrm_skipped)
	{
	...
	vop_mode_ptr->StepP = rc_mode_ptr->Qc = Qc;
...
PUBLIC void Mp4Enc_UpdateIVOP_StepSize(ENC_VOP_MODE_T *vop_mode_ptr, RCMode * rc_mode_ptr)
{
	...
	vop_mode_ptr->StepI = rc_mode_ptr->Qc = Qp;
}
...
PUBLIC void Mp4Enc_AnalyzeEncResult(RCMode *rc_mode_ptr, int32 total_bits_cur, int32 vop_type, int32 Ec_Q8)
{
	...
	if (g_enc_vop_mode_ptr->intra_mb_num > 60)
	{
		be_scene_cut = TRUE;
	}
	...
	if (IVOP == vop_type)
	{
		...
		}else if (total_bits_cur > ((180 * rc_mode_ptr->Bs)>>8)) //0.7
		{
