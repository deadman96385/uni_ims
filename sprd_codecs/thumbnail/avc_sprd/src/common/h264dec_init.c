...
PUBLIC void H264Dec_init_global_para (H264DecContext *img_ptr)
{
	...
	img_ptr->g_stream_offset = 0;
#if _CMODEL_
	VSP_Init_CModel();
#endif //_CMODEL_

	img_ptr->g_sps_array_ptr = (DEC_SPS_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_SPS_T) * MAX_SPS, 4);
    img_ptr->g_pps_array_ptr = (DEC_PPS_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_PPS_T) * MAX_PPS, 4);
	img_ptr->g_sps_ptr = (DEC_SPS_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_SPS_T), 4);
	img_ptr->g_sps_ptr->vui_seq_parameters =  (DEC_VUI_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_VUI_T), 4);
	img_ptr->g_pps_ptr = (DEC_PPS_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_PPS_T), 4);
	img_ptr->g_dpb_ptr = (DEC_DECODED_PICTURE_BUFFER_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_DECODED_PICTURE_BUFFER_T), 4);
#ifdef _VSP_LINUX_
	...
	img_ptr->g_old_slice_ptr = (DEC_OLD_SLICE_PARAMS_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_OLD_SLICE_PARAMS_T), 4);
	img_ptr->g_nalu_ptr = (DEC_NALU_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_NALU_T), 4);

//	img_ptr->g_image_ptr = (H264DecContext *)H264Dec_InterMemAlloc(img_ptr, sizeof(H264DecContext));
	img_ptr->bitstrm_ptr = (DEC_BS_T *)H264Dec_InterMemAlloc(img_ptr, sizeof(DEC_BS_T), 4);
	img_ptr->g_curr_slice_ptr = (DEC_SLICE_T *)H264Dec_InterMemAlloc(img_ptr, sizeof(DEC_SLICE_T), 4);

	img_ptr->g_no_reference_picture_ptr = (DEC_STORABLE_PICTURE_T *)H264Dec_InterMemAlloc(img_ptr, sizeof(DEC_STORABLE_PICTURE_T), 4);

	img_ptr->g_mb_cache_ptr = (DEC_MB_CACHE_T *)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_MB_CACHE_T), 16);
	
	img_ptr->g_dpb_ptr->fs = (DEC_FRAME_STORE_T **)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_FRAME_STORE_T*)*(MAX_REF_FRAME_NUMBER+1), 4);
	img_ptr->g_dpb_ptr->fs_ref = (DEC_FRAME_STORE_T **)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_FRAME_STORE_T*)*(MAX_REF_FRAME_NUMBER+1), 4);
	img_ptr->g_dpb_ptr->fs_ltref = (DEC_FRAME_STORE_T **)H264Dec_InterMemAlloc (img_ptr, sizeof(DEC_FRAME_STORE_T*)*(MAX_REF_FRAME_NUMBER+1), 4);
	...
	img_ptr->g_firstBsm_init_h264 = TRUE;
	...
	img_ptr->is_need_init_vsp_hufftab = TRUE;
	img_ptr->cmd_buf_idx = 0;
