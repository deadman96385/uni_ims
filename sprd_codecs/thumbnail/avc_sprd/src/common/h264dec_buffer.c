...
#endif	
	{
		img_ptr->g_halfPixTemp = (int16 *)H264Dec_ExtraMemAlloc(img_ptr, 24*16*sizeof(int16), 16, SW_CACHABLE);
	}

	H264Dec_ExtraMemAlloc (img_ptr, 2*mb_num_x*sizeof(DEC_MB_INFO_T), 4, SW_CACHABLE); //for posy = -1 mb.
	img_ptr->mb_info = (DEC_MB_INFO_T *)H264Dec_ExtraMemAlloc (img_ptr, (uint32)total_mb_num*sizeof(DEC_MB_INFO_T), 4, SW_CACHABLE);
	img_ptr->i4x4pred_mode_ptr = (int8 *)H264Dec_ExtraMemAlloc (img_ptr, total_mb_num*sizeof(int8)*16, 4, SW_CACHABLE);
	img_ptr->direct_ptr = (int8 *)H264Dec_ExtraMemAlloc (img_ptr, total_mb_num*sizeof(int8)*16, 4, SW_CACHABLE);
	img_ptr->nnz_ptr= (int8 *)H264Dec_ExtraMemAlloc (img_ptr, total_mb_num*sizeof(int8)*24, 4, SW_CACHABLE);	/// 16 y  + 4 u +4v
	img_ptr->mvd_ptr[0] = (int16 *)H264Dec_ExtraMemAlloc (img_ptr, total_mb_num*sizeof(int16)*16*2, 4, SW_CACHABLE);
	img_ptr->mvd_ptr[1] = (int16 *)H264Dec_ExtraMemAlloc (img_ptr, total_mb_num*sizeof(int16)*16*2, 4, SW_CACHABLE);
	img_ptr->slice_nr_ptr = (int32 *)H264Dec_ExtraMemAlloc (img_ptr, total_mb_num*sizeof(int32), 4, SW_CACHABLE);
...
PUBLIC void H264Dec_init_dpb (H264DecContext *img_ptr, DEC_SPS_T *sps_ptr)
{
	...
	for (i = 0; i < MAX_REF_FRAME_NUMBER+1; i++)
	{
		...
		{
			dpb_ptr->fs[i] = (DEC_FRAME_STORE_T *)H264Dec_ExtraMemAlloc (img_ptr, sizeof(DEC_FRAME_STORE_T), 4, SW_CACHABLE);
			dpb_ptr->fs[i]->frame = (DEC_STORABLE_PICTURE_T *)H264Dec_ExtraMemAlloc(img_ptr, sizeof(DEC_STORABLE_PICTURE_T), 4, SW_CACHABLE);
			dpb_ptr->fs[i]->frame->mv_ptr[0] = (int16 *)H264Dec_ExtraMemAlloc(img_ptr, (frm_size_in_blk << 1) * sizeof(int16), 4, SW_CACHABLE);
			dpb_ptr->fs[i]->frame->mv_ptr[1] = (int16 *)H264Dec_ExtraMemAlloc(img_ptr, (frm_size_in_blk << 1) * sizeof(int16), 4, SW_CACHABLE);
			dpb_ptr->fs[i]->frame->ref_idx_ptr[0] = (int8 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size_in_blk, 16, SW_CACHABLE);
			dpb_ptr->fs[i]->frame->ref_idx_ptr[1] = (int8 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size_in_blk, 16, SW_CACHABLE);
			dpb_ptr->fs[i]->frame->ref_pic_id_ptr[0] = (int32 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size_in_blk * sizeof(int32), 4, SW_CACHABLE);
			dpb_ptr->fs[i]->frame->ref_pic_id_ptr[1] = (int32 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size_in_blk * sizeof(int32), 4, SW_CACHABLE);

#ifndef YUV_THREE_PLANE
			if (!img_ptr->VSP_used)	//sw
			{
				dpb_ptr->fs[i]->frame->imgYUV[0] = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, ext_frm_size, 4, SW_CACHABLE);
				dpb_ptr->fs[i]->frame->imgYUV[1] = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, ext_frm_size>>2, 4, SW_CACHABLE);
				dpb_ptr->fs[i]->frame->imgYUV[2] = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, ext_frm_size>>2, 4, SW_CACHABLE);
			}
#endif			
...
#else	//ony for simulation
			dpb_ptr->fs[i]->frame->imgY = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size, 256, HW_NO_CACHABLE);
			dpb_ptr->fs[i]->frame->imgU = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size/2, 256, HW_NO_CACHABLE);
#endif
...
//		fs_ptr->is_reference = DELAYED_PIC_REF;
	...
	{	
		...
		{
			(*img_ptr->avc_unbindCb)(img_ptr->avc_user_data,fs_ptr->frame->pBufferHeader);
...
LOCAL void H264Dec_get_smallest_poc (H264DecContext *img_ptr, int32 *poc,int32 * pos)
{
	int32 i;
	DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr = img_ptr->g_dpb_ptr;
...
LOCAL void H264Dec_output_one_frame_from_dpb (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr)
{
	...
	H264Dec_get_smallest_poc(img_ptr, &poc, &pos);
...
//	SCI_TRACE_LOW("H264Dec_flush_dpb 0, used_size: %d\n", dpb_ptr->used_size);
	...
	while(dpb_ptr->used_size && !img_ptr->error_flag)
	{
...
LOCAL void H264Dec_mm_unmark_short_term_for_reference (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr,  DEC_STORABLE_PICTURE_T *picture_ptr, int32 difference_of_pic_nums_minus1)
{
	int32 pic_num_x;
	int32 i;
...
LOCAL void H264Dec_mm_unmark_long_term_for_reference (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, DEC_STORABLE_PICTURE_T *picture_ptr, int32 long_term_pic_num)
{
	int32 i;
...
LOCAL void H264Dec_unmark_long_term_for_reference_by_frame_idx (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, int32 long_term_frame_idx)
{
	int32 i;
...
LOCAL void H264Dec_mm_assign_long_term_frame_idx (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, DEC_STORABLE_PICTURE_T *picture_ptr, int32 difference_of_pic_nums_minus1, int32 long_term_frame_idx)
{
	int32 pic_num_x;

	pic_num_x = H264Dec_get_pic_num_x (picture_ptr, difference_of_pic_nums_minus1);

	H264Dec_unmark_long_term_for_reference_by_frame_idx (img_ptr, dpb_ptr, long_term_frame_idx);
...
LOCAL void H264Dec_mm_update_max_long_term_frame_idx (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, int32 max_long_term_frame_idx_plus1)
{
	int32 i;
...
{
	H264Dec_mm_update_max_long_term_frame_idx(img_ptr, dpb_ptr, 0);
}

//mark the current picture used for long term reference
LOCAL void H264Dec_mm_mark_current_picture_long_term (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, DEC_STORABLE_PICTURE_T *picture_ptr, int32 long_term_frame_idx)
{
...
LOCAL void H264Dec_adaptive_memory_management (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, DEC_STORABLE_PICTURE_T *picture_ptr)
{
	...
	while (i < img_ptr->g_dec_ref_pic_marking_buffer_size)
	{
		...
		case 0:
			...
			break;
		case 1:
			H264Dec_mm_unmark_short_term_for_reference (img_ptr, dpb_ptr, picture_ptr, tmp_drpm_ptr->difference_of_pic_nums_minus1);
			H264Dec_update_ref_list(dpb_ptr);
			break;
		case 2:
			H264Dec_mm_unmark_long_term_for_reference (img_ptr, dpb_ptr, picture_ptr, tmp_drpm_ptr->long_term_pic_num);
			H264Dec_update_ltref_list (dpb_ptr);
			break;
		case 3:
			H264Dec_mm_assign_long_term_frame_idx (img_ptr, dpb_ptr, picture_ptr, tmp_drpm_ptr->difference_of_pic_nums_minus1, tmp_drpm_ptr->long_term_frame_idx);
			H264Dec_update_ref_list (dpb_ptr);
			H264Dec_update_ltref_list(dpb_ptr);
			break;
		case 4:
			H264Dec_mm_update_max_long_term_frame_idx (img_ptr,dpb_ptr, tmp_drpm_ptr->max_long_term_frame_idx_plus1);
			H264Dec_update_ltref_list (dpb_ptr);
			break;
		case 5:
			...
			break;
		case 6:
			H264Dec_mm_mark_current_picture_long_term (img_ptr, dpb_ptr, picture_ptr, tmp_drpm_ptr->long_term_frame_idx);
...
#if 0//def _VSP_LINUX_
		if(!img_ptr->VSP_used)
		{
		if(curr_fs_ptr->frame->pBufferHeader!=NULL)
			(*avc_bindCb)(avc_user_data,curr_fs_ptr->frame->pBufferHeader);
		}
#endif
...
PUBLIC void H264Dec_store_picture_in_dpb (H264DecContext *img_ptr, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr, DEC_STORABLE_PICTURE_T *picture_ptr)
{
...
//jin.zhou
	// first try to remove unused frames
	  ...
	  {
	    	H264Dec_remove_unused_frame_from_dpb(img_ptr, dpb_ptr);
	  }
	  
	  while ((dpb_ptr->used_size == dpb_ptr->size) && !img_ptr->error_flag)
	  {
	    // flush a frame
	    H264Dec_output_one_frame_from_dpb(img_ptr, dpb_ptr);
	  }
...
LOCAL DEC_STORABLE_PICTURE_T *H264Dec_get_short_term_pic (H264DecContext *img_ptr, int32 pic_num)
{
	int32 i;
	DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr = img_ptr->g_dpb_ptr;
...
LOCAL void H264Dec_reorder_short_term (H264DecContext *img_ptr, DEC_STORABLE_PICTURE_T **ref_picture_listX_ptr, int32 num_ref_idx_lX_active_minus1, int32 pic_num_lx, int32 *ref_idx_lx)
{
	...
	pic_lx_ptr = H264Dec_get_short_term_pic (img_ptr, pic_num_lx);
...
LOCAL void H264Dec_reorder_long_term (H264DecContext *img_ptr,DEC_STORABLE_PICTURE_T **ref_picture_listX_ptr, int32 num_ref_idx_lX_active_minus1, 
...
LOCAL void H264Dec_reorder_ref_pic_list (H264DecContext *img_ptr, DEC_STORABLE_PICTURE_T **picture_list_ptr, int32 num_ref_idx_lX_active_minus1,
