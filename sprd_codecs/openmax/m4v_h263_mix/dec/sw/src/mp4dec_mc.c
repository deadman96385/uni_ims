...
void Mp4Dec_motionCompY_fourMV(DEC_VOP_MODE_T *vop_mode_ptr,  MOTION_VECTOR_T *pmv,uint8* pDstFrameY, int32 dstWidth, int32 bck_direction)
{
    ...
    uint8 *pframe_refY = bck_direction ? vop_mode_ptr->YUVRefFrame0[0] : vop_mode_ptr->YUVRefFrame2[0];
    int32 width = vop_mode_ptr->FrameExtendWidth;
    int32 xMBPos = vop_mode_ptr->mb_x <<5;
    int32 yMBPos = vop_mode_ptr->mb_y <<5;
    ...
    for(iBlk = 0; iBlk < 4; iBlk++)
    {
        ...
        vop_mode_ptr->g_mp4dec_mc_8x8[(dx<<1) | dy] (pframe_ref, pRecBlk, width, dstWidth);
...
void Mp4Dec_motionCompY_oneMV(DEC_VOP_MODE_T *vop_mode_ptr,  MOTION_VECTOR_T *pmv,uint8* pDstFrameY, int32 dstWidth , int32 bck_direction)
{
    ...
    pframe_ref = pframe_refY + ((yRef >> 1) + YEXTENTION_SIZE) * width + (xRef >> 1) + YEXTENTION_SIZE;
    vop_mode_ptr->g_mp4dec_mc_16x16[(dx<<1) | dy] (pframe_ref, pDstFrameY, width, dstWidth);
}
...
void Mp4Dec_motionCompensationUV(DEC_VOP_MODE_T *vop_mode_ptr, int32 iMVUVx, int32 iMVUVy,uint8* pDstFrameU,uint8* pDstFrameV, int32 dstWidth , int32 bck_direction)
{
    ...
    uint8 *pframe_ref;
    int32 offset_ref;
    int32 width = vop_mode_ptr->FrameExtendWidth >> 1;

    xRef = (vop_mode_ptr->mb_x <<4) + iMVUVx;
    yRef = (vop_mode_ptr->mb_y <<4) + iMVUVy;
    dx = xRef & 1;
    dy = yRef & 1;

    offset_ref = ((yRef >> 1) + UVEXTENTION_SIZE) * width + (xRef >> 1) + UVEXTENTION_SIZE;

    pframe_ref =(bck_direction ? vop_mode_ptr->YUVRefFrame0[1] : vop_mode_ptr->YUVRefFrame2[1])+ offset_ref;
    vop_mode_ptr->g_mp4dec_mc_8x8[(dx<<1) | dy] (pframe_ref, pDstFrameU, width, dstWidth);

    pframe_ref = (bck_direction ? vop_mode_ptr->YUVRefFrame0[2] : vop_mode_ptr->YUVRefFrame2[2])+ offset_ref;
    vop_mode_ptr->g_mp4dec_mc_8x8[(dx<<1) | dy] (pframe_ref, pDstFrameV, width, dstWidth);
}

LOCAL void Mp4Dec_MC_GetAverage(uint8 * pSrc0, uint8 * pSrc1, uint8 *pDst, int32 dst_width, int32 iYUVFlag)
{
    int i;
