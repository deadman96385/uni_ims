...
void pmem_test()
{
	sp<MemoryHeapBase> masterHeap[100];
	sp<MemoryHeapPmem>  iDecExtPmemHeap[100];
	for(int i=0;i<100;i++){
		masterHeap[i] = new MemoryHeapBase(PMEM_DRIVER,1024*1024,MemoryHeapBase::NO_CACHING);
 	        iDecExtPmemHeap[i] = new MemoryHeapPmem(masterHeap[i],MemoryHeapBase::NO_CACHING);
 	 	int fd = iDecExtPmemHeap[i]->getHeapID();
	 	if(fd>=0){
	       		iDecExtPmemHeap[i]->slap();
			//masterHeap[i].clear();
	        	struct pmem_region region;
	        	::ioctl(fd,PMEM_GET_PHYS,&region);			
			SCI_TRACE_LOW("pmem_test %d %x,%x,%x,%x\n",i,iDecExtPmemHeap[i]->getHeapID(),iDecExtPmemHeap[i]->base(),region.offset,region.len);	
			iDecExtPmemHeap[i].clear();
    		}else
    		{
    			SCI_TRACE_LOW("pmem_test  error %d\n",i);
    		}
	}
}
*/
   ...
   //SCI_TRACE_LOW("Mpeg4Decoder_OMX::Mp4DecodeVideo %d aInBufSize %5d marker %d::buffer %x,%x,%x,%x,%x,%x\n", (*aFrameCount),*aInBufSize,aMarkerFlag,(*aInputBuf)[0],(*aInputBuf)[1],(*aInputBuf)[2],(*aInputBuf)[3],(*aInputBuf)[4],(*aInputBuf)[5]);
	...
	uint32 ext_buffer_size;
	 SCI_TRACE_LOW("Mpeg4Decoder_OMX::Mp4DecodeVideo width %d,height %d\n",aPortParam->format.video.nFrameWidth,aPortParam->format.video.nFrameHeight);
		...
		ext_buffer_size = ((frm_size*3*3)>>1) + mb_x*4*8*sizeof(int16) + frm_size*3 + 1000*1024;
	
		if(aDeBlocking)
		{
			ext_buffer_size += ((frm_size*3*3)>>1); //display buffer
		}
	#elif defined(CHIP_TIGER)
		ext_buffer_size = ((frm_size*3*3)>>1) ; //yuv
	        ext_buffer_size += mb_x*4*8*sizeof(int16) ;//pTopCoeff
	        ext_buffer_size += MPEG4_DECODER_STREAM_BUFFER_SIZE;
	        if(mb_x <= 11)
	       {
	      		ext_buffer_size += frm_size*7 ;//cmd
	       }
	       else
	       {
	 ...
	 sp<MemoryHeapBase> masterHeap = new MemoryHeapBase(PMEM_DRIVER,ext_buffer_size,MemoryHeapBase::NO_CACHING);
 	 iDecExtPmemHeap = new MemoryHeapPmem(masterHeap,MemoryHeapBase::NO_CACHING);
 	 int fd = iDecExtPmemHeap->getHeapID();
	 if(fd>=0){
	       	iDecExtPmemHeap->slap();
		masterHeap.clear();
	        struct pmem_region region;
	        ::ioctl(fd,PMEM_GET_PHYS,&region);
		iDecExtPhyAddr =(OMX_U32)region.offset;					
		SCI_TRACE_LOW("Mpeg4Decoder_OMX ext mem pmempool %x,%x,%x,%x\n",iDecExtPmemHeap->getHeapID(),iDecExtPmemHeap->base(),region.offset,region.len);
...
#else
#if   defined(CHIP_TIGER) //LEON to modify????
	...
	ext_buffer_size += 4 * 8 * sizeof(uint8);				//pLeftCoeff_sw
	ext_buffer_size += 6 * 64 * sizeof(int16);				//coef_block
	ext_buffer_size += total_mb_num * (sizeof(uint32) + 64 *sizeof(uint32));	//dc_store,dp
#if 0// cmd_data cmd_info
	//ext_buffer_size += frm_size * 3;			//cmd
	if(total_mb_num < 100)
	{
		ext_buffer_size += frm_size *7;			//cmd
	}
...
#endif
	ext_buffer_size += 1024;				//misc
#else //defined(CHIP_8810)
	ext_buffer_size = total_mb_num *2*sizeof(DEC_MB_MODE_T); 	//mb_info
	ext_buffer_size += total_mb_num * 1 * sizeof(uint8);		//mbdec_stat_ptr
	ext_buffer_size += mb_x * 4 * sizeof(int16);			//pTopLeftDCLine
	ext_buffer_size += 4 * 8 * sizeof(uint8);				//pLeftCoeff_sw
	ext_buffer_size += 6 * 64 * sizeof(int16);				//coef_block
	...
	SCI_TRACE_LOW("aOutBuffer->pBuffer %x,picPhyAddr %x",(uint32)aOutBuffer->pBuffer,picPhyAddr);
    if(!picPhyAddr)
    {
	...
	SCI_TRACE_LOW("vaddr %x,picPhyAddr %x",vaddr,picPhyAddr);
    ...
    oscl_memcpy((void *)iStreamVAddr, *aInputBuf,*aInBufSize);
    //little_to_big_endian((char *)iStreamVAddr,*aInBufSize);
    ...
    //SCI_TRACE_LOW("Mpeg4Decoder_OMX Status %d\n",Status);
    ...
    OMX_S32* aWidth, OMX_S32* aHeight, OMX_U8** aBuffer, OMX_S32* aSize, OMX_S32 mode, OMX_BOOL aDeBlocking)
{
    ...
    SCI_TRACE_LOW("Mpeg4Decoder_OMX::InitializeVideoDecode\n");
     ...
     {
#if 0
		 sp<MemoryHeapBase> masterHeap = new MemoryHeapBase(PMEM_DRIVER,MPEG4_DECODER_STREAM_BUFFER_SIZE,MemoryHeapBase::NO_CACHING);
		...
		SCI_TRACE_LOW("Mpeg4Decoder_OMX stream mem pempool %x,%x,%x,%x\n",iStreamPmemHeap->getHeapID(),iStreamPmemHeap->base(),region.offset,region.len);
	 	iStreamVAddr = (OMX_U32)iStreamPmemHeap->base();
		iStreamBufferWasSet = OMX_TRUE;
    	}
#else
	iStreamPmemHeap = new MemoryHeapIon(SPRD_ION_DEV, MPEG4_DECODER_STREAM_BUFFER_SIZE, MemoryHeapBase::NO_CACHING, ION_HEAP_CARVEOUT_MASK);
	 ...
	 if(fd>=0){
		...
		SCI_TRACE_LOW("Mpeg4 Decoder_OMX ext mem pmempool %x,%x,%x,%x\n",iStreamPmemHeap->getHeapID(),iStreamPmemHeap->base(),phy_addr,buffer_size);
	 	iStreamVAddr = (OMX_U32)iStreamPmemHeap->base();
		SCI_TRACE_LOW("Mpeg4 Decoder_OMX stream mem pmempool  sucessful \n");
		iStreamBufferWasSet = OMX_TRUE;
    	}
#endif
	else
    	{
    		iBufferAllocFail = OMX_TRUE;    	
    		SCI_TRACE_LOW("Mpeg4Decoder_OMX stream mem pmempool  error\n");
		return PV_FALSE;	
    	}
    }
    ...
    {
        oscl_memcpy((void *)iStreamVAddr,*aBuffer,*aSize);
	//little_to_big_endian((char *)iStreamVAddr,*aSize);   
	video_format.p_extra =(void *) iStreamVAddr;
