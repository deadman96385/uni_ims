...
PUBLIC MMDecRet Mp4Dec_InitSessionDecode(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	if (vop_mode_ptr->VSP_used)
	{
		...
		vop_mode_ptr->frame_bistrm_ptr = (uint8 *)Mp4Dec_ExtraMemAlloc( sizeof(uint8) * MP4DEC_FRM_STRM_BUF_SIZE, 4, HW_CACHABLE);
...
//vop_mode_ptr->frame_bistrm_ptr = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign( sizeof(uint8) * MP4DEC_FRM_STRM_BUF_SIZE);
		SCI_TRACE_LOW("cmd_data_buf allocated from s_CMD_mem_cache_bfr_ptr");
		if(total_mb_num < 100) { ////Modified by Leon.Li @20120925
			vop_mode_ptr->cmd_data_buf[0]= (uint32 *) s_CMD_mem_cache_bfr_ptr;			
			vop_mode_ptr->cmd_info_buf[0] = (uint32 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*6);			
			
			vop_mode_ptr->cmd_data_buf[1]= (uint32 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*8);
			vop_mode_ptr->cmd_info_buf[1] = (uint32 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*14);

			vop_mode_ptr->frame_bistrm_ptr = (uint8 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*16 );
                }
		else{

			vop_mode_ptr->cmd_data_buf[0]= (uint32 *) s_CMD_mem_cache_bfr_ptr;			
			vop_mode_ptr->cmd_info_buf[0] = (uint32 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*3);			
			
			vop_mode_ptr->cmd_data_buf[1]= (uint32 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*4);
			vop_mode_ptr->cmd_info_buf[1] = (uint32 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*7);

			vop_mode_ptr->frame_bistrm_ptr = (uint8 *) ((uint32)s_CMD_mem_cache_bfr_ptr+vop_mode_ptr->MBNum * 256*8 );
		}
#else
		SCI_TRACE_LOW("cmd_data_buf allocated from s_CMD_mem_cache_bfr_ptr");
		if(mb_num_x*mb_num_y < 100) { ////Modified by Leon.Li @20120925
			vop_mode_ptr->cmd_data_buf[0]= (uint32 *)Mp4Dec_ExtraMemAlloc(6*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);		
			vop_mode_ptr->cmd_info_buf[0] = (uint32 *)Mp4Dec_ExtraMemAlloc(2*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);			
			
			vop_mode_ptr->cmd_data_buf[1]= (uint32 *)Mp4Dec_ExtraMemAlloc(6*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
			vop_mode_ptr->cmd_info_buf[1] = (uint32 *)Mp4Dec_ExtraMemAlloc(2*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);

		}
		else{

				vop_mode_ptr->cmd_data_buf[0]= (uint32 *)Mp4Dec_ExtraMemAlloc(3*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);		
				vop_mode_ptr->cmd_info_buf[0] = (uint32 *)Mp4Dec_ExtraMemAlloc(1*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);			
			
				vop_mode_ptr->cmd_data_buf[1]= (uint32 *)Mp4Dec_ExtraMemAlloc(3*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
				vop_mode_ptr->cmd_info_buf[1] = (uint32 *)Mp4Dec_ExtraMemAlloc(1*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
		}

#endif

			if(NULL == vop_mode_ptr->cmd_data_buf[0] \
			|| NULL == vop_mode_ptr->cmd_data_buf[1]\
			|| NULL == vop_mode_ptr->cmd_info_buf[0]\
			|| NULL == vop_mode_ptr->cmd_info_buf[1])
			{
				SCI_TRACE_LOW("cmd_data_buf allocating failed");
			    return MMDEC_MEMORY_ERROR;
			}
