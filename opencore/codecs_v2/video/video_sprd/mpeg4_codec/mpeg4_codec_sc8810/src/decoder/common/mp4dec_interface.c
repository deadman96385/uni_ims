...
PUBLIC MMDecRet MP4DecInit(MMCodecBuffer *pBuffer, MMDecVideoFormat *video_format_ptr)
{
	...
	{		
		if(video_format_ptr->i_extra > 0)
		{
			...
			if(MMDEC_OK == is_init_success)
			{
				...
				}else
				{
					...
					video_format_ptr->frame_width = vop_mode_ptr->OrgFrameWidth + YEXTENTION_SIZE*2;
					video_format_ptr->frame_height= vop_mode_ptr->OrgFrameHeight + YEXTENTION_SIZE*2;
#else
					video_format_ptr->frame_width = vop_mode_ptr->OrgFrameWidth;
					video_format_ptr->frame_height= vop_mode_ptr->OrgFrameHeight;
#endif
		...
		{
#if 0
			video_format_ptr->frame_width = (((vop_mode_ptr->OrgFrameWidth+15)>>4)<<4);
			video_format_ptr->frame_height= (((vop_mode_ptr->OrgFrameHeight+15)>>4)<<4);
#else
			video_format_ptr->frame_width = vop_mode_ptr->OrgFrameWidth;
			video_format_ptr->frame_height= vop_mode_ptr->OrgFrameHeight;
#endif
