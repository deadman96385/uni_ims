	   ...
	   //pBaseComponentPort->pBuffer[ii]->pBuffer = (OMX_BYTE) oscl_malloc(nSizeBytes);
	    ...
	    }else
	    {
	       ...
	       if(iStoreMetaDataBuffer)
	       {
			...
			SCI_TRACE_LOW("OmxComponentMpeg4EncAO::AllocateBuffer StoreMetaDataBuffer");
	       }
			...
			{
				SCI_TRACE_LOW("OmxComponentMpeg4EncAO::AllocateBuffer pmempool error %d %d\n",nSizeBytes_64word,pBaseComponentPort->PortParam.nBufferCountActual);
				return OMX_ErrorInsufficientResources;
			}
	    	}
	        ...
	        {       
	        	SCI_TRACE_LOW("OmxComponentMpeg4EncAO::AllocateBuffer iNumberOfPmemHeaps>=nBufferCountActual %d %d\n",iNumberOfPmemBuffers,pBaseComponentPort->PortParam.nBufferCountActual);
			return OMX_ErrorInsufficientResources;	
	        }
...
OMX_ERRORTYPE OmxComponentMpeg4EncAO::ConstructComponent(OMX_PTR pAppData, OMX_PTR pProxy)
{
    ...
    ipPorts[OMX_PORT_INPUTPORT_INDEX]->PortParam.nBufferCountActual =2;// NUMBER_INPUT_BUFFER_MP4ENC; jgdu for satgefright
    ...
    pOutPort->VideoQuantType.nQpI = 10;
    pOutPort->VideoQuantType.nQpP = 10;
    pOutPort->VideoQuantType.nQpB = 10;
    ...
    OMX_INOUT OMX_PTR ComponentParameterStructure)
{
	OMX_ERRORTYPE ret = OMX_ErrorNone;
       OmxComponentMpeg4EncAO* pOpenmaxAOType = (OmxComponentMpeg4EncAO*)((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate;
	//SCI_TRACE_LOW("OmxComponentMpeg4EncAO::GetParameter");	   
    	if (NULL == pOpenmaxAOType)
    	{
            return OMX_ErrorBadParameter;
    	}
       switch (nParamIndex) {
	    ...
	    {	
			...
			SCI_TRACE_LOW("OmxComponentMpeg4EncAO::GetParameter OMX_IndexParamStoreMetaDataBuffer %d",pStoreMetaData->bStoreMetaData);
	    }
			break;
	    default:
    ...
    OMX_IN  OMX_PTR ComponentParameterStructure)
{
	OMX_ERRORTYPE ret = OMX_ErrorNone;
       OmxComponentMpeg4EncAO* pOpenmaxAOType = (OmxComponentMpeg4EncAO*)((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate;
	//SCI_TRACE_LOW("OmxComponentMpeg4EncAO::SetParameter");	
    	if (NULL == pOpenmaxAOType)
    	{
            return OMX_ErrorBadParameter;
    	}	
	switch (nParamIndex) {
	    case OMX_IndexParamStoreMetaDataBuffer:
	    {
			...
			if (pStoreMetaData->bStoreMetaData == OMX_FALSE) {
        			SCI_TRACE_LOW("OmxComponentMpeg4EncAO::disable StoreMetaData");
        			iStoreMetaDataBuffer = OMX_FALSE;
    				pOutPort->PortParam.format.video.eColorFormat = (OMX_COLOR_FORMATTYPE)OMX_COLOR_FormatYUV420SemiPlanar;				
    			} else {
        			SCI_TRACE_LOW("OmxComponentMpeg4EncAO::enable StoreMetaData");
    ...
    OMX_OUT OMX_INDEXTYPE* pIndexType)
{
    ...
    {
    		SCI_TRACE_LOW("OmxComponentMpeg4EncAO::%s",SPRD_INDEX_PARAM_STORE_METADATA_BUFFER);
...
void OmxComponentMpeg4EncAO::ProcessData()
{
    ...
    //SCI_TRACE_LOW("OmxComponentMpeg4EncAO::ProcessData %x,%lld\n",ipFrameDecodeBuffer,iFrameTimestamp);
...
OMX_BOOL OmxComponentMpeg4EncAO::CopyDataToOutputBuffer()
{
    ...
    while (iInternalOutBufFilledLen > 0)
    {
        if (ipOutputBuffer->nAllocLen >= iInternalOutBufFilledLen)
        {
            ...
            SCI_TRACE_LOW("0 dst %x,src %x,len %d",ipOutputBuffer->pBuffer,ipInternalOutBuffer,iInternalOutBufFilledLen);
        ...
        {
            SCI_TRACE_LOW("1 dst %x,src %x,len %d",ipOutputBuffer->pBuffer,ipInternalOutBuffer,ipOutputBuffer->nAllocLen);
