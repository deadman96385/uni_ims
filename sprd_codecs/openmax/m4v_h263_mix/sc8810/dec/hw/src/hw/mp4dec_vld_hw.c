 ...
 *****************************************************************************/
PUBLIC int32 Mp4Dec_VlcDecIntraTCOEF_hw(DEC_VOP_MODE_T *vop_mode_ptr, int16 *iCoefQ, int32 iCoefStart)
{
	int32 len;
	int32 temp;	
	uint32 code;
...
//	const uint8 * rgiZigzag = vop_mode_ptr->pZigzag; 
	...
	while(!last)
	{		
		code = Mp4Dec_ShowBits(pBitstrm, 32);
		temp = code >> 20;
		...
		{	
			...
			if(!level_offset) 
			{
				...
				level += vop_mode_ptr->pIntra_max_level[last * 64 + run];
			...
			}else
			{
				...
				if(!run_offset) 
				{
					...
					last = (tab->code >> 14) & 1;
						...
						run = run + vop_mode_ptr->pIntra_max_run [last * 32 + level] + 1;
 ...
 **	Note:     
 *****************************************************************************/
PUBLIC void Mp4Dec_VlcDecInterTCOEF_H263_hw(DEC_VOP_MODE_T *vop_mode_ptr/*, int16 *iDCTCoef*/, int32 iCoefStart, DEC_BS_T *pBitstrm)
{
	uint32 code;
	uint32 temp;
	const VLC_TABLE_CODE_LEN_T *tab = PNULL;
 ...
 **	Note:     vld do deScan.
 *****************************************************************************/
PUBLIC void Mp4Dec_VlcDecInterTCOEF_Mpeg_hw(DEC_VOP_MODE_T *vop_mode_ptr/*, int16 *iDCTCoef*/, DEC_BS_T * pBitstrm)
{
	uint32 code;
	int32 i = 0/*, index*/;
	const VLC_TABLE_CODE_LEN_T *tab = PNULL;
...
//		FPRINTF(g_fp_trace_fw, "\tbs32=%0x\n", code);
		...
		if(ESCAPE == tab->code)
		{
			...
			if(!level_offset) 
			{
				...
				level += vop_mode_ptr->pInter_max_level[last * 64 + run];
				//sign = Mp4Dec_ReadBits (pBitstrm, 1); 	//, "SIGN"	
				sign = code >> 31;
#endif				
				code = code << 1;
			...
			}else
			{
				...
				if(!run_offset) 
				{
					...
					last = (tab->code >> 12) & 1;
					level = tab->code & 15;
					run = run + vop_mode_ptr->pInter_max_run[last * 16 + level] + 1;
