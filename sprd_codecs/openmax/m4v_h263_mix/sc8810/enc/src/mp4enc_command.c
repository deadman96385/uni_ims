...
//following code should be same as C-MODEL.
	...
	{			
		...
		cfg = (cfg & 0xfffff8ff) | (VSP_FLV_V1<< 8);
	...
	if (!vop_mode_ptr->big_size_flag)//removed by xweluo@20120210 for sw/me pipeline
	{
		...
		{			
			...
			cfg = (cfg & 0xfffff8ff) | (VSP_ITU_H263 << 8);
			VSP_WRITE_REG(VSP_GLB_REG_BASE+GLB_CFG0_OFF, cfg, "GLB_CFG0: init the global register");
		}
	}
}
...
PUBLIC void Mp4Enc_ConfigDctQuantMB(ENC_VOP_MODE_T *vop_mode_ptr, ENC_MB_MODE_T *mb_mode_ptr)
{
	...
	/*configure QUANT PA*/
	Qp = mb_mode_ptr->StepSize;
	shift_bits = vop_mode_ptr->pQuant_pa[Qp<<1]; 
	shift_val = vop_mode_ptr->pQuant_pa[(Qp<<1) + 1]; 
	...
	if(mb_mode_ptr->bIntra)
	{
		...
		Y_inv_shift = vop_mode_ptr->pDC_scaler[pMBCache->dc_scaler_y * 2 + 1];
		Y_shift = vop_mode_ptr->pDC_scaler[pMBCache->dc_scaler_y * 2];
		UV_inv_shift = vop_mode_ptr->pDC_scaler[pMBCache->dc_scaler_c * 2 +1];
		UV_shift = vop_mode_ptr->pDC_scaler[pMBCache->dc_scaler_c * 2];
...
//	VSP_WRITE_REG(VSP_DCT_REG_BASE+DCT_CFG_FINISH_OFF, 1, "configure finish of DCT");   
	...
	mb_mode_ptr->CBP = VSP_READ_REG(VSP_DCT_REG_BASE+DCT_OUT_CBP_OFF, "read CBP from DCT out_CBP");   //read CBP from DCT core 
...
//	int32 cbp = 0x3F;
	...
	VSP_WRITE_REG(VSP_DCT_REG_BASE+DCT_IN_CBP_OFF, mb_mode_ptr->CBP, "IDCT CBP");
...
void Mp4Enc_VspMBInit(ENC_VOP_MODE_T *vop_mode_ptr)
{
	...
	if (vop_mode_ptr->short_video_header)
	{			
		uint32 cfg = VSP_READ_REG(VSP_GLB_REG_BASE+GLB_CFG0_OFF, "read gobal register");
		
		cfg = (cfg & 0xfffff8ff) | (VSP_ITU_H263 << 8);
		VSP_WRITE_REG(VSP_GLB_REG_BASE+GLB_CFG0_OFF, cfg, "GLB_CFG0: init the global register");
	}
#if 0	//removed by xweluo@20120210 for sw/me pipeline	
...
{
	VSP_READ_REG_POLL(VSP_DBK_REG_BASE+DBK_CTR1_OFF, V_BIT_0, 0 , TIME_OUT_CLK, "DBK_CTR1: polling mbc done, the last mb of current frame");
	VSP_READ_REG_POLL(VSP_AHBM_REG_BASE+AHBM_STS_OFFSET, V_BIT_0, 0, TIME_OUT_CLK, "AHBM_STS: polling AHB idle status");	
}
