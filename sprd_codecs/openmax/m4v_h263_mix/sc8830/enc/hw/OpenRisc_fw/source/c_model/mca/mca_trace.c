...
int g_mca_tv_num = 0;
int g_mbc_ipred_num = 0;

PUBLIC void Mp4Dec_Trace_MCA_result_one_macroblock(uint8 *pMcaBfr)
{
	FILE *fp= fopen("..\\..\\test_vectors\\Cmodel_MCA_out.log","wb");
	if(NULL == fp)
		exit;//weihu
	if(g_vector_enable_flag&VECTOR_ENABLE_MCA)
	{
		int32 mb_x = g_glb_reg_ptr->VSP_CTRL0 & 0x7f; //weihu //0x3f
 	...
 	// 	FPRINTF (g_fp_mca_tv, "frame: %d, mb_y: %d, mb_x: %d\n", g_nFrame_dec, mb_y, mb_x);
		...
		for (ii = 0; ii < MB_SIZE; ii++)
		{
			...
			{
#ifdef  VP8_DEC
				FomatPrint4Pix(pLine+4, fp);
				FomatPrint4Pix(pLine, fp);
				fprintf(fp, "\n");
#else				
				FomatPrint4Pix(pLine+4, g_fp_mca_tv);
				FomatPrint4Pix(pLine, g_fp_mca_tv);
				fprintf(g_fp_mca_tv, "\n");
				//
				FomatPrint4Pix(pLine+4, g_fp_mbc_ipred);
				FomatPrint4Pix(pLine, g_fp_mbc_ipred);
				fprintf(g_fp_mbc_ipred, "\n");
#endif
...
#if (defined(MPEG4_DEC)||defined(H264_DEC))
...
if(g_trace_enable_flag&TRACE_ENABLE_MCA)
{
	...
	int i, j;
 	...
 	MCA_FPRINTF (g_fp_trace_mca, "frame: %d, mb_y: %d, mb_x: %d\n", g_nFrame_dec, mb_y, mb_x);
#else
	FPRINTF (g_fp_trace_mca, "frame: %d, mb_y: %d, mb_x: %d\n", g_nFrame_dec_h264, mb_y, mb_x);
#endif
	//Y
	MCA_FPRINTF (g_fp_trace_mca, "Y MB\n");
	pix_ptr = pMcaBfr;
	for (i = 0; i < MB_SIZE; i++)
	{
		for (j = 0; j < MB_SIZE; j++)
		{
			MCA_FPRINTF (g_fp_trace_mca, "%3d, ", pix_ptr[i*MB_SIZE + j]);
		}
		MCA_FPRINTF (g_fp_trace_mca, "\n");
	}
	...
	for (i = 0; i < BLOCK_SIZE; i++)
	{
		...
		{
			MCA_FPRINTF (g_fp_trace_mca, "%3d, ", pix_ptr[i*BLOCK_SIZE + j]);
		}
		MCA_FPRINTF (g_fp_trace_mca, "\n");
	}
