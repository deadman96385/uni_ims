...
int vp8_decode_frame(VP8D_COMP *pbi)
{
	...
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf4, tmp, "FPGA test");
...
#endif
OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x22222222, "FPGA test");
    if (xd->segmentation_enabled)
    {
...
#endif
OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x33333333, "FPGA test");
    if (xd->mode_ref_lf_delta_enabled)
    {
		...
		OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x33333334, "FPGA test");
    }
#ifdef TV_OUT
...
OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x44444444, "FPGA test");
	...
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x77777777, "FPGA test");
...
//    vp8_build_block_doffsets(xd);
	...
	OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x88888888, "FPGA test");	
...
//    else
...
#else
	{
		...
		if(tmp&0x31)	// (VLC_ERR|TIME_OUT)
		{
			...
			if(tmp&0x1)
			OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x55555555, "FPGA test");
		}
		else if((tmp&0x00000004)==0x00000004)	// MBW_FMR_DONE
		{
			...
			OR1200_WRITE_REG(SHARE_RAM_BASE_ADDR + 0xf8, 0x66666666, "FPGA test");
		}
		OR1200_READ_REG_POLL(ORSC_BSM_CTRL_OFF+0x18, V_BIT_27, 0x00000000, "ORSC: Polling BSM_DBG0: !DATA_TRAN, BSM_clr enable"); //check bsm is idle
	}
#endif
