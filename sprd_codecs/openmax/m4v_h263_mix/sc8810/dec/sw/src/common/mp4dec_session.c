...
PUBLIC MMDecRet Mp4Dec_InitSessionDecode(MP4Handle *mp4Handle, MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	if (vop_mode_ptr->VSP_used)
	{
	    ...
	    vop_mode_ptr->pTopCoeff = (int16 *)Mp4Dec_ExtraMemAlloc_64WordAlign(vd,  sizeof(int16) * (uint8)(vop_mode_ptr->MBNumX) * 4 * 8);
	    if(NULL == vop_mode_ptr->pTopCoeff)
	    {
	        return MMDEC_MEMORY_ERROR;
	    }

		vop_mode_ptr->frame_bistrm_ptr = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(vd,  sizeof(uint8) * MP4DEC_FRM_STRM_BUF_SIZE);
...
//			vd->g_Mp4Dec_GetInterBlkTCoef_hw = Mp4Dec_VlcDecInterTCOEF_H263_hw;
	...
	if (vop_mode_ptr->VSP_used)
	{
		...
		for(i = 0; i< 2; i++)
		{
			vop_mode_ptr->cmd_data_buf[i]= (uint32 *) Mp4Dec_ExtraMemAlloc_64WordAlign(vd, 3*(vop_mode_ptr->MBNum)*256);

			if(NULL == vop_mode_ptr->cmd_data_buf[i])
			{
			    return MMDEC_MEMORY_ERROR;
			}
			vop_mode_ptr->cmd_info_buf[i] = (uint32 *) Mp4Dec_ExtraMemAlloc_64WordAlign(vd, (vop_mode_ptr->MBNum)*256);
