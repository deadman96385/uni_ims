						...
						)
{
	...
	if (line_id >= 4)
	{
		...
		{
			addr = (line_id & 3) * Y_LINE_BUF_WIDTH + mb_x*4 - 2 + (blk_wr_x + 1);
		}
		else if (blk_wr_comp == BLK_COMPONET_U)
		{
			addr = U_OFFSET_LINE_BUF + (line_id & 3) * UV_LINE_BUF_WIDTH + mb_x * 2 - 2 + (blk_wr_x + 1);
		}
		else
		{
			addr = V_OFFSET_LINE_BUF + (line_id & 3) * UV_LINE_BUF_WIDTH + mb_x * 2 - 2 + (blk_wr_x + 1);
		}
	...
	{	
		if (blk_wr_comp == BLK_COMPONET_Y)
		{
			if ((mb_x != (mb_num_x - 1)) && blk_wr_x == 4 && blk_wr_y == 0)
			{
				addr = line_id * 6;
		...
		{
			if ((mb_x != (mb_num_x - 1)) && blk_wr_x == 2 && blk_wr_y == 0)
			{
				addr = U_OFFSET_TBUF_C + line_id * 4;
				vsp_rdbk_tbuf[addr] = dbk_buf_wdata;
			}
			else
			{
				addr = U_OFFSET_OBUF_C + (blk_wr_y * 4 + line_id) * 4 + (blk_wr_x + 1);
				vsp_rdbk_obuf[addr] = dbk_buf_wdata;
			}
		...
		else
		{
			if ((mb_x != (mb_num_x - 1)) && blk_wr_x == 2 && blk_wr_y == 0)
			{
				addr = V_OFFSET_TBUF_C + line_id * 4;
				vsp_rdbk_tbuf[addr] = dbk_buf_wdata;
			}
			else
			{
				addr = V_OFFSET_OBUF_C + (blk_wr_y * 4 + line_id) * 4 + (blk_wr_x + 1);
				vsp_rdbk_obuf[addr] = dbk_buf_wdata;
			}
					 ...
					 )
{
	...
	if (line_id >= 4)
	{
		...
		{
			addr = U_OFFSET_OBUF_C + (blk_rd_y * 4 + line_id) * 4 + (blk_rd_x + 1);
		}
		else
		{
			addr = V_OFFSET_OBUF_C + (blk_rd_y * 4 + line_id) * 4 + (blk_rd_x + 1);
		}
	...
	else
	{
		...
		if (blk_rd_comp == BLK_COMPONET_Y)
		{
			...
			{
				addr = (line_id & 3) * Y_LINE_BUF_WIDTH + mb_x*4 - 2 + (blk_rd_x + 1);
		...
		else if (blk_rd_comp == BLK_COMPONET_U)
		{
			...
			{
				addr = U_OFFSET_LINE_BUF + (line_id & 3) * UV_LINE_BUF_WIDTH + mb_x * 2 - 2 + (blk_rd_x + 1);
				read_data = vsp_rdbk_lbuf[addr];
			}
		}
		else
		{
			...
			{
				addr = V_OFFSET_LINE_BUF + (line_id & 3) * UV_LINE_BUF_WIDTH + mb_x * 2 - 2 + (blk_rd_x + 1);
				read_data = vsp_rdbk_lbuf[addr];
			}
...
//	uint8	pix_filter[8];
	...
	{
		ComputeBlkPos_rv8(blk_id,   &blk_rd_x, &blk_rd_y, &blk_rd_comp);
		ComputeBlkPos_rv8 (blk_id-2, &blk_wr_x, &blk_wr_y, &blk_wr_comp);
...
/*		g_blk_id = blk_id;*/
		...
		for (line_id = 7; line_id >= 0; line_id--)
		{
			...
			**********************************************************/
			GetFourPixWrite_rv8 (line_id, pix_write);
			
			//for first block, there is no data need to be written to buffer
			write_lt_blk = 0/*(blk_wr_x == 0) && (blk_wr_y == 0) && (line_id < 4)*/;

		//	if (blk_wr_x >= 0)
			if ((!write_lt_blk) & ((blk_wr_x > 0) | ((blk_wr_x == 0) && (mb_x > 0))))
				FilterPixWriteBuf_rv8 (mb_x, mb_num_x, line_id, pix_write, blk_wr_x, blk_wr_y, blk_wr_comp);
			...
			**********************************************************/
			ReadFourPixels_rv8 (mb_x, line_id, pix_read, blk_rd_x, blk_rd_y, blk_rd_comp);

			PixBlkUptWrite_rv8  (line_id, pix_read[0], pix_read[1], pix_read[2], pix_read[3]);
			...
			**********************************************************/
			Get8PixFiltering_rv8 (line_id, pix_p, pix_q);

			is_hor_filter = (line_id >= 4) ? 1 : 0;
			filt_blk_id   = is_hor_filter ? blk_id : blk_id - 1;
			ComputeBlkPos_rv8 (filt_blk_id, &filt_blk_x, &filt_blk_y, &filt_blk_comp);						
