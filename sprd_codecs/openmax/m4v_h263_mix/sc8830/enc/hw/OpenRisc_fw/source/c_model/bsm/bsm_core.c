...
#elif defined(SIM_IN_WIN)&&defined(H264_DEC)
    if (!tmp)
	{
...
void write_nbits(uint32 val, uint32 nbits, uint32 vlc_op)
{
	...
	if (vlc_op)
	{
		...
		}else //video
		{
			...
			PrintfVLCOut(val, nbits, "coeff_token");
		}
	}
#endif
	return;
...
void clear_bsm_fifo()
{
	...
	if(count == 1 && left_byte == 0)
		fprintf(g_fp_bsm_tv, "%08x\n", 0);
#else
	}
#endif
	...
	for(i = 0; i < left_byte; i++)
	{
		...
		*bsmr->bs_ptr++ = (jpeg_bsmr.data[0] >> (24 - i*8))&0xff;
	}
