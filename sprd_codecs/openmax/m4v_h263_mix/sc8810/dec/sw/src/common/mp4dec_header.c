...
//	assert((vop_mode_ptr->time_inc_resolution >= 1) && (vop_mode_ptr->time_inc_resolution < 65536));
	...
	{	
		...
		if(vop_mode_ptr->bLoadIntraQMatrix)
		{
			...
			do 
			{
				...
				vop_mode_ptr->IntraQuantizerMatrix[g_standard_zigzag_normal[i]] = QMatrixElement;
				i++;
			...
			if(0 == QMatrixElement)
			{
				...
				{
					vop_mode_ptr->IntraQuantizerMatrix[g_standard_zigzag_normal[j]] = 
					vop_mode_ptr->IntraQuantizerMatrix[g_standard_zigzag_normal[i-1]];
				}	
			}
		}else
		{
			vop_mode_ptr->IntraQuantizerMatrix = (uint8 *)g_default_intra_qmatrix_normal;
		}
		...
		if (vop_mode_ptr->bLoadInterQMatrix)
		{
			...
			do 
			{
				...
				vop_mode_ptr->InterQuantizerMatrix [g_standard_zigzag_normal[i]] = QMatrixElement;
				i++;			
			...
			if(0 == QMatrixElement)
			{
				...
				for (j = i; (j < BLOCK_SQUARE_SIZE)&&((i-1) < BLOCK_SQUARE_SIZE); j++)
				{
					vop_mode_ptr->InterQuantizerMatrix [g_standard_zigzag_normal[j]] = 
						vop_mode_ptr->InterQuantizerMatrix[g_standard_zigzag_normal[i-1]];
				}	
			}					
		}else
		{
			vop_mode_ptr->InterQuantizerMatrix = (uint8 *)g_default_inter_qmatrix_normal;
		}
	}
	...
	//modify quant-table if sw decoding
	if (Q_MPEG == vop_mode_ptr->QuantizerType && !vop_mode_ptr->VSP_used)
	{
		uint32 i, row, col;
		uint8 tbl[BLOCK_SQUARE_SIZE];
		
		if(vop_mode_ptr->bLoadIntraQMatrix)
		{
			memcpy(tbl, vop_mode_ptr->IntraQuantizerMatrix, BLOCK_SQUARE_SIZE);
			...
			for (i = 0; i < BLOCK_SQUARE_SIZE; i++)
			{
				...
				vop_mode_ptr->IntraQuantizerMatrix[(col<<3)+row] = tbl[i];
			}
		}else
		{
			vop_mode_ptr->IntraQuantizerMatrix = (uint8 *)g_default_intra_qmatrix_trans;
		}

		if(vop_mode_ptr->bLoadInterQMatrix)
		{
			memcpy(tbl, vop_mode_ptr->InterQuantizerMatrix, BLOCK_SQUARE_SIZE);
			...
			for (i = 0; i < BLOCK_SQUARE_SIZE; i++)
			{
				...
				vop_mode_ptr->InterQuantizerMatrix[(col<<3)+row] = tbl[i];
			}
		}else
		{
			vop_mode_ptr->InterQuantizerMatrix = (uint8 *)g_default_inter_qmatrix_trans;
		}
	}
...
LOCAL MMDecRet Mp4Dec_DecVopHeader(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	{
		vop_mode_ptr->RoundingControl = (int8)Mp4Dec_ReadBits(bitstrm_ptr, 1);//"vop_rounding_type"
	}
...
PUBLIC MMDecRet Mp4Dec_DecMp4Header(DEC_VOP_MODE_T *vop_mode_ptr, uint32 uOneFrameLen)
{
	...
	while(((nDecTotalBits>>3)/* + 4*/) <= uOneFrameLen)
	{
		...
		//uStartCode = Mp4Dec_ShowBits_sw(bitstrm_ptr, 32);
		uStartCode = Mp4Dec_Show32Bits(bitstrm_ptr);
...
MMDecRet Mp4Dec_DecH263PlusHeader(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	DEC_BS_T *bitstrm_ptr = vop_mode_ptr->bitstrm_ptr;
	H263_PLUS_HEAD_INFO_T *h263_plus = vd->h263_plus_head_info_ptr;
	int32 tmp;
...
int32 get_h263_frametype(uint8 *bitstrm_ptr) //@ leon 
{
	int32 source_format;
	int32 UFEP;
	int32 frm_type;
	...
	bitstrm_ptr += 4;
	source_format = ((*bitstrm_ptr)>>2)&0x7;
	
	if(0x7== source_format)
	{
		//h263+
		UFEP = (((*bitstrm_ptr)&0x3)<<1) | (bitstrm_ptr[1]>>7);
		bitstrm_ptr += 1;
		
		if(1 == UFEP)
		{
			bitstrm_ptr += 2;
			frm_type = ((*bitstrm_ptr)>>2)&0x7;
		}
		else
		{
			frm_type = ((*bitstrm_ptr)>>4)&0x7;
		}
		
	}
	else
	{
		frm_type = ((*bitstrm_ptr)>>1)&0x1;
	}
 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecH263Header(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	uint32 tmpVar;
	DEC_BS_T *bitstrm_ptr = vop_mode_ptr->bitstrm_ptr;
	BOOLEAN *pIs_stop_decode_vol = &vd->g_dec_is_stop_decode_vol;
	MMDecRet ret;
 
 	vop_mode_ptr->VopPredType = get_h263_frametype((uint8 *)(bitstrm_ptr->rdbfr));//@ leon??
	...
	tmpVar = (tmpVar & 0x07);
	if((tmpVar != (uint32)vd->g_dec_pre_vop_format) && (!vd->g_dec_is_first_frame))
	{
		vd->g_dec_is_changed_format = TRUE;
	}

	vd->g_dec_pre_vop_format = (VOP_PRED_TYPE_E)tmpVar;

	if(tmpVar == EXTENDED_PTYPE)
	{
		ret = Mp4Dec_DecH263PlusHeader(vd, vop_mode_ptr);
	...
	if(vd->g_dec_is_first_frame || vd->g_dec_is_changed_format)
	{
		vop_mode_ptr->QuantPrecision = 5;
...
PUBLIC MMDecRet Mp4Dec_FlvH263PicHeader(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	if((format != (int32)vd->g_dec_pre_vop_format) && (!vd->g_dec_is_first_frame))
	{
		vd->g_dec_is_changed_format = TRUE;
	}

	vd->g_dec_pre_vop_format = (VOP_PRED_TYPE_E) format;
