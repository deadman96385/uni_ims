...
{
    DEC_IMAGE_PARAMS_T *img_ptr = vo->g_image_ptr;
    uint32 MaxPicOrderCntLsb = (1<<(vo->g_active_sps_ptr->log2_max_pic_order_cnt_lsb_minus4+4));
    int32 i;

    switch (vo->g_active_sps_ptr->pic_order_cnt_type)
    {
    case 0:	//poc mode 0
        ///1 st
        if (img_ptr->idr_flag)
    ...
    case 1:	//poc mode 1
        ...
        ///2nd
        if (vo->g_active_sps_ptr->num_ref_frames_in_pic_order_cnt_cycle)
        {
        ...
        if (vo->g_active_sps_ptr->num_ref_frames_in_pic_order_cnt_cycle)
        {
            for (i = 0; i < vo->g_active_sps_ptr->num_ref_frames_in_pic_order_cnt_cycle; i++)
            {
                img_ptr->ExpectedDeltaPerPicOrderCntCycle += vo->g_active_sps_ptr->offset_for_ref_frame[i];
            }
        }

        if (img_ptr->AbsFrameNum)
        {
            img_ptr->PicOrderCntCycleCnt = (img_ptr->AbsFrameNum-1)/vo->g_active_sps_ptr->num_ref_frames_in_pic_order_cnt_cycle;
            img_ptr->FrameNumInPicOrderCntCycle = (img_ptr->AbsFrameNum-1)%vo->g_active_sps_ptr->num_ref_frames_in_pic_order_cnt_cycle;
            ...
            {
                img_ptr->ExpectedPicOrderCnt += vo->g_active_sps_ptr->offset_for_ref_frame[i];
            }
        } else
        {
        ...
        if (!img_ptr->nal_reference_idc)
        {
            img_ptr->ExpectedPicOrderCnt += vo->g_active_sps_ptr->offset_for_non_ref_pic;
        }
        ...
        if (img_ptr->field_pic_flag == 0)
        {
            ...
            img_ptr->bottompoc = img_ptr->toppoc + vo->g_active_sps_ptr->offset_for_top_to_bottom_field + img_ptr->delta_pic_order_cnt[1];
