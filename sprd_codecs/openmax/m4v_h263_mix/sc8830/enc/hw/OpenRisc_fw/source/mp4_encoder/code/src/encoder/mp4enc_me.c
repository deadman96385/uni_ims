...
void Mp4Enc_JudgeMBMode(ENC_MB_MODE_T * pMb_mode)
{
	...
	//james: integer pixel mv
	{
		mea_mv = (int32)VSP_READ_REG(VSP_MEA_REG_BASE+MEA_IME_OFF, "read integer mv of inter MB from mea");
		pMb_mode->imv.y = (int16)((mea_mv << 16) >> 24);
		pMb_mode->imv.x = (int16)((mea_mv << 24) >> 24);
	}
...
{
	int fetchX;
	int y_start_pos = (pVop_mode->mb_y + 1) * MB_SIZE;
...
PUBLIC void Mp4Enc_MVprediction(ENC_VOP_MODE_T *pVop_mode, ENC_MB_MODE_T *pMb_mode) 
{
...
// 	uint32 vp_mb_num = pVop_mode->MBNumOneVP;
	uint32 i;
	...
	{
#if 0
		candidate_mv[0].x = (pMb_mode -1)->mv[0].x;
		candidate_mv[0].y = (pMb_mode -1)->mv[0].y;
#else
		//james:use integer mv of left MB
		candidate_mv[0].x = (pMb_mode -1)->imv.x;
		candidate_mv[0].y = (pMb_mode -1)->imv.y;
#endif
		is_in_bound[0] = TRUE;
		num_in_Bound++;
	}
	...
	if(mb_pos_y != 0)
	{
		if (!is_top_bndry && pVop_mode->pMbModeAbv[mb_pos_x].dctMd != INTRA)
		{			
		...
		if (!is_right_bndry && pVop_mode->pMbModeAbv[mb_pos_x+1].dctMd != INTRA)
		{			
	...
	if(num_in_Bound == 1)
	{
		for (i = 0; i < 3; i++)
		{
			...
			{
				pMb_mode->mvPred = candidate_mv[i];
#if 0//james20121225:if out range set mvp to (0,0) at soft_motion_estimation
				if (pVop_mode->short_video_header)
				{
					int32 ref_pos_x = ((mb_pos_x<<5) + (pMb_mode->mvPred.x));
					int32 ref_pos_y = ((mb_pos_y<<5) + (pMb_mode->mvPred.y));

					/* checking in half-pixel precise */
					//modify mv
					if ((ref_pos_x < 0) || (ref_pos_x > ((pVop_mode->FrameWidth  - MB_SIZE)*2)))
					{
						pMb_mode->mvPred.x = 0;
					}

					if ((ref_pos_y < 0) || (ref_pos_y > ((pVop_mode->FrameHeight - MB_SIZE)*2)))
					{
						pMb_mode->mvPred.y = 0;
					}
				}
