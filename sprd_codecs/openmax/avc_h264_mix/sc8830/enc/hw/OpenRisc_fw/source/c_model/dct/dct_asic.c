			   ...
			   )
{
	...
	for (iy = 0; iy < 8; iy++)
	{
		...
		/* 8 point dct */
		dct_1D_8(tmp_buf, MEDIAN_VAR_BIT);
		if (8 == data_bit_width)
		{
			in_buf8  += buf_width;
		}else
	...
	for (ix = 0; ix < 8; ix++)
	{
		...
		for (iy = 0; iy < 8; iy ++)
		{
			med_buf[iy] = *(tmp_buf + iy * 8);
		}
		/* 8 point dct */
		dct_1D_8(med_buf, (MEDIAN_VAR_BIT+1));  // the outdata is S21
		for (iy = 0; iy < 8; iy ++)
		{
 ...
 *****************************************************************************/
LOCAL void mpeg4_idct(int16 *idct_data,  // input data
				...
				)  	
{
	...
	for (iy = 0; iy < 8; iy++)
	{
		...
		{
			t0 = (*(in_buf + ix));
			t0 = (t0 * IDCT_matrix[ix + iy * 8]) >> (data_bit_width - 7);
			tmp_buf[ix] = t0 | 0x1;
		}
		/* 8 point dct */
		idct_1D_8(tmp_buf, MEDIAN_VAR_BIT);		
		in_buf  += buf_width;
		tmp_buf += 8;
	}
	/* column direction dct */
	...
	for (ix = 0; ix < 8; ix++)
	{
		...
		/* 8 point dct */
		idct_1D_8(med_buf, (MEDIAN_VAR_BIT+1));  // the outdata is S21
		for (iy = 0; iy < 8; iy ++)
		{
			*(tmp_buf + iy * 8) = med_buf[iy];
		}
