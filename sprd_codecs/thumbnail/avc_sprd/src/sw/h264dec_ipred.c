...
void intra_pred_luma4x4_DC_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	int32 a;
	int32 avail;
	int32 pred = 0;
...
void intra_pred_luma4x4_VERT_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB,  uint8 *pRec, int32 pitch)
{
	uint32 a;
	uint8 * pPix = NULL;
	uint32 * pIntPix = NULL;	
...
void intra_pred_luma4x4_HOR_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	int32 i;
	...
	for (i = 0; i < 4; i++)
	{
		uint32 a = pPix [0] * 0x01010101;	pPix += pitch;
		((uint32 *)pPixBlk)[0] = a;
...
void intra_pred_luma4x4_DIAG_DOWN_RIGHT_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	uint8	* pPix = NULL, * pPixBlk = NULL;
	...
	pPix = pRec - pitch - 1;
	tn1 = pPix [0]; t0 = pPix [1]; t1 = pPix [2]; t2 = pPix [3]; t3 = pPix [4];
	ln1 = tn1;	pPix += pitch;
	l0 = pPix [0];	pPix += pitch;
	l1 = pPix [0];	pPix += pitch;
	l2 = pPix [0];	pPix += pitch;
	l3 = pPix [0];
...
void intra_pred_luma4x4_DIAG_DOWN_LEFT_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	int32 blkCAvai = 1;
	uint8	* pPix, * pPixBlk;
...
void intra_pred_luma4x4_VERT_RIGHT_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	uint8	* pPix = NULL, * pPixBlk = NULL;
	uint8	tn1, t0, t1, t2, t3;
	uint8	l0, l1, l2;
...
void intra_pred_luma4x4_VERT_LEFT_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	int32 blkCAvai = 1;
	uint8	* pPix, * pPixBlk;
	...
	t0 = *pPix++;   t1 = *pPix++;   t2 = *pPix++;  t3 = *pPix++;  
...
void intra_pred_luma4x4_HOR_UP_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	uint8	* pPix = NULL, * pPixBlk = NULL;
	uint8	l0, l1, l2, l3;
	...
	/* (x + 2y) > 5*/
	pPixBlk [34/*2 + 2*MB_SIZE*/] = l3;	pPixBlk [35/*3 + 2*MB_SIZE*/] = l3;  pPixBlk [MB_SIZE_X3] = l3;
	pPixBlk [49/*1 + 3*MB_SIZE*/] = l3;	pPixBlk [50/*2 + 3*MB_SIZE*/] = l3;  pPixBlk [51/*3 + 3*MB_SIZE*/] = l3;
	
}

void intra_pred_luma4x4_HOR_DOWN_PRED (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, int blkIdxInMB, uint8 *pRec, int32 pitch)
{
	uint8	* pPix = NULL, * pPixBlk = NULL;
	uint8	tn1, t0, t1, t2;
	uint8	ln1, l0, l1, l2, l3;
	...
	pPix = pRec - pitch - 1;
	tn1 = pPix [0]; t0 = pPix [1]; t1 = pPix [2]; t2 = pPix [3]; 
	ln1 = tn1;	pPix += pitch;
	l0 = pPix [0];	pPix += pitch;
	l1 = pPix [0];	pPix += pitch;
	l2 = pPix [0];	pPix += pitch;
...
void intraPred_HOR_PRED_16 (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, uint8 * pRec, int32 pitch)
{
	...
	{	
		...
		*pIntPix++ = b;
		...
		vst1q_u8(pPred, v128);pPred += MB_SIZE;
	#endif
...
void intraPred_PLANE_16 (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPred, uint8 * pRec, int32 pitch)
{
	...
	{		
		...
		for (x = 4; x > 0; x--)
		{
			...
			*pred++ = (uint8)pClip [(factor_b + factor_a_c_16) >> 5];				factor_b += b;
			*pred++ = (uint8)pClip [(factor_b + factor_a_c_16) >> 5];				factor_b += b;
			*pred++ = (uint8)pClip [(factor_b + factor_a_c_16) >> 5];				factor_b += b;
			*pred++ = (uint8)pClip [(factor_b + factor_a_c_16) >> 5];				factor_b += b;
		}
...
void intraPred_chroma_DC (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPredMB, uint8 * pRec, int32 pitch)
{
	...
	uint8 * pTopPredPix, * pLeftPredPix;
	uint8 p0, p1, p2, p3;
	uint32 a;
	uint32 * pIntPred;	
	uint32 top4_left, top4_right;
	uint32 left4_top, left4_down;
	int32 constrainedIntra = ((H264DecContext *)img)->constrained_intra_pred_flag;
#ifdef _NEON_OPT_
	uint8x8_t v8x8_top, v8x8_left;
	uint16x4_t v16x4;
	...
	if (leftBlkAvail)
	{
		...
		p0 = pLeftPredPix[0];	pLeftPredPix += pitch;
		p1 = pLeftPredPix[0];	pLeftPredPix += pitch;
		p2 = pLeftPredPix[0];	pLeftPredPix += pitch;
		p3 = pLeftPredPix[0];	pLeftPredPix += pitch;
...
void intraPred_chroma_Hor (void * img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, uint8 * pPredMB, uint8 * pRec, int32 pitch)
{
...
#endif
	pred = pPredPix[0];	pPredPix += pitch;
	...
	for (i = MB_CHROMA_SIZE; i > 0; i--)
	{
		...
		*pIntPredMB++ = a;
		...
		vst1_u8(pPredMB, v8x8);pPredMB += MB_CHROMA_SIZE;
	#endif	
		pred = pPredPix[0];	pPredPix += pitch;
	}
}
