...
#define H264ENC_MALLOC_PRINT   //SPRD_CODEC_LOGD

PUBLIC MMEncRet H264Enc_InitMem (H264EncObject *vo, MMCodecBuffer *pMem, int32 type)
{
    ...
    } else
    {
        ...
        H264ENC_MALLOC_PRINT("%s: type:%d, dw_aligned, %d, v_base: 0x%lx, p_base: 0x%lx, mem_size:%d\n",
...
PUBLIC void *H264Enc_MemAlloc (H264EncObject *vo, uint32 need_size, int32 aligned_byte_num, int32 type)
{
    ...
    H264ENC_MALLOC_PRINT("%s: mem_size:%d, AlignedAddr: %lx, type: %d\n", __FUNCTION__, need_size, AlignedAddr, type);
 ...
 *****************************************************************************/
PUBLIC uint_32or64 H264Enc_ExtraMem_V2P(H264EncObject *vo, uint8 *vAddr, int32 type)
{
    if (type >= MAX_MEM_TYPE)
    {
