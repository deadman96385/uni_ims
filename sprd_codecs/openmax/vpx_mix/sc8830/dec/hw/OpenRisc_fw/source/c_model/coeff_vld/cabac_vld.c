...
uint32 readB8_typeInfo_CABAC_High (void)
{
	...
	if (!bframe)
	{
		if (biari_decode_symbol(g_image_ptr, &ctx->b8_type_contexts[0][0]))//[0][1]
		{
			b8_type = 0;
		}else
		{
			if (biari_decode_symbol(g_image_ptr, &ctx->b8_type_contexts[0][1]))//[0][3]
			{
				if (biari_decode_symbol(g_image_ptr, &ctx->b8_type_contexts[0][2])) //[0][4]
					b8_type = 2;
				else	b8_type = 3;
			}else
			{
...
//	}
	...
	if (mv_local_err < 3) act_ctx = 4*compIdx;//jzy 5* cabac
	else
	{
		if (mv_local_err > 32)	act_ctx = 4*compIdx+3;//jzy 5*
		else	act_ctx = 4*compIdx+2; //5*
	}
