...
PUBLIC void H264Dec_init_dpb (H264DecContext *img_ptr, DEC_SPS_T *sps_ptr)
{
    ...
    for (i = 0; i < MAX_REF_FRAME_NUMBER+1; i++)
    {
        ...
        if (dpb_ptr->fs[i] == PNULL)
        {
            ...
            dpb_ptr->fs[i]->frame->imgU = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size/4, 256, HW_NO_CACHABLE);
            dpb_ptr->fs[i]->frame->imgV = (uint8 *)H264Dec_ExtraMemAlloc(img_ptr, frm_size/4, 256, HW_NO_CACHABLE);
#else	
