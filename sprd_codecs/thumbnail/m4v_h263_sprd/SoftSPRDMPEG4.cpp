...
int32_t SoftSPRDMPEG4::extMemoryAllocWrapper(
        void *decCtrl, void *aUserData, unsigned int width,unsigned int height) {
    return static_cast<SoftSPRDMPEG4 *>(aUserData)->extMemoryAlloc(decCtrl, width, height);
}

int SoftSPRDMPEG4::extMemoryAlloc(void *decCtrl, unsigned int width,unsigned int height) {
    ...
    ALOGI("%s, %d, ext_mem: %0x, ext_mem_size: %d", __FUNCTION__, __LINE__, mCodecExtraBuffer, mCodecExtraBufferSize);
    
    MP4DecMemCacheInit( (VideoDecControls *)decCtrl,&codec_buf);
