...
PUBLIC void Mp4Dec_DecMV_sw(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, DEC_MB_BFR_T *mb_cache_ptr)
{
	...
	}else
	{
		...
		if(INTER4V != mb_mode_ptr->dctMd)/*has one MV*/
		{
			...
			Mp4Dec_motionCompY_oneMV(vop_mode_ptr, mb_cache_ptr, pMv, pDstFrameY);
		}else   /*has 4 MV*/
		{	
			...
			Mp4Dec_motionCompY_fourMV(vop_mode_ptr, mb_cache_ptr, pMv,pDstFrameY);
		}		
		
		Mp4Dec_motionCompensationUV(vop_mode_ptr, mb_cache_ptr, mvc_x, mvc_y, pDstFrameU, pDstFrameV);
	}
}
