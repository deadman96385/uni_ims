...
PUBLIC void Mp4Dec_InitBitstream(void *pOneFrameBitstream, int32 length)
{
	...
	VSP_WRITE_REG(VSP_BSM_REG_BASE+BSM_CFG0_OFF, (V_BIT_31) | (((length+flushBytes+300)>>2)&0x3ffff), "BSM_CFG0: init bsm register: buffer 0 ready and the max buffer size");
...
uint32 Mp4Dec_ShowBitsByteAlign(int32 nbits)
{
	...
	tmpLen = bitsLeft - nAlignBitsLeft;
	...
	if(tmpLen > 0x20)
	{
		...
		uint32 tmpVar2 = Mp4Dec_ShowBits(0x20);
