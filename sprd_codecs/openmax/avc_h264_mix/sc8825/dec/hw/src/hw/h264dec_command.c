...
void dump_output_yuv()
{
	...
	if(g_nFrame_dec_h264 == 0 || g_nFrame_dec_h264 ==  243)
	{
		output_yuv = fopen("/data/H264_dec_8825.yuv","wb");  
	}
...
PUBLIC int32 H264Dec_VSPInit (void)
{
	int cmd;
#ifndef _VSP_LINUX_
	if(1)
...
#endif	
//	VSP_Reset();
    SCI_TRACE_LOW("H264Dec_VSPInit frmNo %d",g_nFrame_dec_h264);
...
//	DEC_IMAGE_PARAMS_T *img_ptr = g_image_ptr;
	...
	VSP_WRITE_REG_CQM(VSP_BSM_REG_BASE+BSM_CFG1_OFF, (0<<31), "BSM_CFG1: configure the bitstream address, disable DE-STUFFING");//Modified by Leon.Li @20120925
...
PUBLIC MMDecRet H264Dec_init_slice_VSP(DEC_IMAGE_PARAMS_T *img_ptr, DEC_SLICE_T *curr_slice_ptr)
{
...
#endif
	{
		...
		/*init vsp command*/ //Modified by Leon.Li @20120925
		cmd = (1<<19)|((img_ptr->is_cabac)<<18) |(0<<17) |(0<<16) |(0<<15) | (0<<14) |(1<<12) | (H264<<8) | (1<<7) | (1<<6) | (1<<3) | (1<<2) | (1<<1) | (1<<0);
...
#else //actual envioronment
	VSP_WRITE_REG_CQM(VSP_MEMO10_ADDR+ 8, ((uint32)(H264Dec_ExtraMem_V2P((img_ptr->frame_bistrm_ptr+g_stream_offset), HW_CACHABLE))>>2), "AHBM_FRM_ADDR_6: source bitstream buffer0 address");
#endif
...
#else
	VSP_WRITE_REG(pTableAddr + 4, ((uint32)H264Dec_ExtraMem_V2P(img_ptr->ipred_top_line_buffer, HW_NO_CACHABLE))>>2, "Top MB bottom data block first line pixel data for IPRED prediction");
	VSP_WRITE_REG(pTableAddr + 4*3, ((uint32)H264Dec_ExtraMem_V2P(img_ptr->vld_cabac_table_ptr, HW_NO_CACHABLE))>>2, "vld cabac buffer address");
#endif
...
//	SCI_TRACE_LOW("H264Dec_Picture_Level_Sync: pipeline wairt for previous  frame , frame num %d\n", g_nFrame_dec_h264 -1);
	...
	if(cur_time < 0)
		cur_time += 1000000;
	SCI_TRACE_LOW("cur frame SW waiting Hw  time frameNO %d %lld",g_nFrame_dec_h264,cur_time);
#endif		
	...
	gettimeofday(&tpstart,NULL);
#endif
memcpy((uint32)g_cmd_data_ptr+4,(uint32)g_cmd_data_base,4*(g_cmd_data_ptr - g_cmd_data_base));
memcpy((uint32)g_cmd_info_ptr+4,(uint32)g_cmd_info_base,4*(g_cmd_info_ptr - g_cmd_info_base));	
#ifdef _DEBUG_TIME_
	gettimeofday(&tpend1,NULL);
	...
	if(cur_time < 0)
		cur_time += 1000000;
	SCI_TRACE_LOW("cur frame MEMCPY CMD buffer  time frameNO %d %lld length %d",g_nFrame_dec_h264,cur_time,4*(g_cmd_data_ptr - g_cmd_data_base)+4*(g_cmd_info_ptr - g_cmd_info_base));
#endif
#endif
...
#else
	VSP_WRITE_REG(VSP_MEMO10_ADDR+20*4, ((uint32)H264Dec_ExtraMem_V2P(g_cmd_info_base, HW_CACHABLE))>>2, "config CMD control info buffer address");
	VSP_WRITE_REG(VSP_MEMO10_ADDR+21*4, ((uint32)H264Dec_ExtraMem_V2P(g_cmd_data_base, HW_CACHABLE))>>2, "config CMD control data buffer address");
#endif
...
//flush cache
	...
	if(cur_time < 0)
		cur_time += 1000000;
	SCI_TRACE_LOW("cur frame flush CMD buffer  time frameNO %d %lld",g_nFrame_dec_h264,cur_time);
#endif
...
//	SCI_TRACE_LOW("H264Dec_Picture_Level_Sync, enable cmd-exe, %d\n", cmd);
