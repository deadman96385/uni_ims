...
#include <utils/Log.h>
#undef LOG_TAG
#define LOG_TAG "PVMFOMXBaseDecNode"
        ...
        OMX_OUT OMX_PTR aEventData)
{
    ...
    switch (aEvent)
    {
        ...
        case OMX_EventError:
        {
            ...
            {
                ALOGE("%s::EventHandlerProcessing: OMX_EventError - Bitstream corrupt error", iName.Str());
                // Errors from corrupt bitstream are reported as info events
               // ReportInfoEvent(PVMFInfoProcessingFailure, NULL);
                Reschedule();//modify by zhangjl for decnode to ignor edec err

            }
            else if (aData1 == (OMX_U32) OMX_ErrorInvalidState)
            {
             	ALOGE("%s::EventHandlerProcessing: OMX_EventError - OMX_ErrorInvalidState", iName.Str());
            ...
            else
            {
                ...
                ALOGE("%s::EventHandlerProcessing: OMX_EventError aData1 0x%x", iName.Str(),aData1);
                // for now, any error from the component will be reported as error
               // ReportErrorEvent(PVMFErrProcessing, NULL, NULL);
               // SetState(EPVMFNodeError);
                Reschedule();//modify by zhangjl for decnode to ignore dec err
            }
            break;
