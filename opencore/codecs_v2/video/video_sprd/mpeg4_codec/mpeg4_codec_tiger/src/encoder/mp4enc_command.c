...
// 	VSP_READ_REG_POLL(VSP_MBC_REG_BASE+MBC_ST0_OFF, V_BIT_5, V_BIT_5, TIME_OUT_CLK, "MBC: polling mbc done, the last mb of current frame");
	...
	g_HW_CMD_START = 0;
	{
		while(ret)
		{
			ret =VSP_READ_REG_POLL(VSP_DCAM_REG_BASE+DCAM_INT_RAW_OFF, V_BIT_16, V_BIT_16, TIME_OUT_CLK, "polling MP4 encoding done interrupt flag");
		}
	}
	VSP_WRITE_REG(VSP_DCAM_REG_BASE+DCAM_INT_CLR_OFF, V_BIT_16, "DCAM_INT_CLR: clear MP4 encoding done interrupt flag");
#if _CMODEL_
	g_dcam_reg_ptr->DCAM_MP4ENC_CTRL_t = (g_rc_par.sad<<1);
#endif
