	...
	}else
	{
		...
		{														//stupid scaling
			...
			*residual = (abs_diff_mv_component - 1) % pMv_dir->ScaleFactor;																	
			vlc_magnitude = (abs_diff_mv_component - (*residual) + (pMv_dir->ScaleFactor - 1))/(pMv_dir->ScaleFactor);	// absorb ++ into here (g_EncVolmd.mvInfo.uiScaleFactor - 1)
			*vlc = vlc_magnitude * ssign(diff_mv_component);
		}
	}
}

MP4_LOCAL uint32 Mp4Enc_SendDiffMV(ENC_VOP_MODE_T *pVop_mode, MOTION_VECTOR_T *diff_mv_halfpel)
{
	int32 vlc;
	int32 residual;
	uint32 NumBits = 0;
	int32 entry;	
	const MV_INFO_T *pMv_dir = &(pVop_mode->mvInfoForward);
	...
	NumBits += Mp4Enc_OutputMV(pVop_mode, entry, residual, pMv_dir->FCode, vlc);
#else
	NumBits += Mp4Enc_OutputMV(pVop_mode, entry);
#endif
...
#define JudgeLeftTopBndry(vop_mode_ptr, pos_x, pos_y, mb_mode_ptr)  \
	((pos_x == 0) || (pos_y == 0) || ((vop_mode_ptr->pMbModeAbv+pos_x - 1)->iPacketNumber != mb_mode_ptr->iPacketNumber)) ? FALSE : TRUE;
...
uint32 Mp4Enc_EncMVVP(ENC_VOP_MODE_T *vop_mode_ptr, ENC_MB_MODE_T *mb_mode_ptr)
{
	...
	pMBCache->bLeftTopAvail = JudgeLeftTopBndry(vop_mode_ptr, mb_pos_x, mb_pos_y, mb_mode_ptr);
...
#ifdef OUTPUT_TEST_VECTOR
			tv_Diff_MV[blk_num] = vector_diff;
#endif
		#if _TRACE_
