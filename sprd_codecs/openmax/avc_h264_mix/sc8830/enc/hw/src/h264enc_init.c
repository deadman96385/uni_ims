...
PUBLIC void H264Enc_InitVSP(H264EncObject *vo)
{
    ...
    VSP_WRITE_REG(FRAME_ADDR_TABLE_BASE_ADDR + 0x4, img_ptr->pYUVRecFrame->imgUVAddr, "ORSC: Frm_addr1: Start address of reconstruct frame UV");
    VSP_WRITE_REG(FRAME_ADDR_TABLE_BASE_ADDR + 0x80, img_ptr->pYUVRefFrame->imgYAddr, "ORSC: Frm_addr32: Start address of Reference list0 frame Y");
    VSP_WRITE_REG(FRAME_ADDR_TABLE_BASE_ADDR + 0x100, img_ptr->pYUVRefFrame->imgUVAddr, "ORSC: Frm_addr64: Start address of Reference list0 frame UV");

    cmd	= (uint32)H264Enc_ExtraMem_V2P(vo, vo->g_vlc_hw_ptr, INTER_MEM)>>3;		// Frm_addr3[31:0], VLC Table set by Fixed Table Address
    VSP_WRITE_REG(FRAME_ADDR_TABLE_BASE_ADDR + 0xC, cmd, "ORSC: Frm_addr3: Start address of VLC table");
    VSP_WRITE_REG(GLB_REG_BASE_ADDR + 0x34, 300, "ORSC: VSP_SIZE_SET: VLC_table_size");
}
...
PUBLIC void H264Enc_InitBSM(H264EncObject *vo)
{
    ...
    cmd |= (((img_ptr->OneframeStreamLen&0xfffffffc) & 0x3FFFFFFF) << 0); // BUFFER_SIZE[29:0], unit byte//cmd = g_bsm_reg_ptr->BSM_CFG0 << 0;
