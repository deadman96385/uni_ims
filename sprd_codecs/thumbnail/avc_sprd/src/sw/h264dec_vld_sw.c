...
/**
  ...
  */
int readCoeff4x4_CAVLC_sw (H264DecContext *img_ptr, DEC_MB_CACHE_T *mb_cache_ptr, int16 *block, int32 n, const uint8 *scantable, const uint32 *qmul, int32 max_coeff)
{
...
void decode_LUMA_DC (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int32 qp)
{
	...
	total_coeff = readCoeff4x4_CAVLC_sw(img_ptr, mb_cache_ptr,DC, LUMA_DC_BLOCK_INDEX, inverse_zigZag, NULL, 16);
	if (total_coeff)
	{
...
void decode_LUMA_AC_sw (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp )
{
	...
	if(currMB->transform_size_8x8_flag)
	{
		...
		for(blk8x8 = 0; blk8x8 < 4; blk8x8++)
		{
			if(cbp & (1 << blk8x8))
			{
				...
				{
					numCoeff = readCoeff4x4_CAVLC_sw(img_ptr, mb_cache_ptr, coeff, ((blk8x8<<2) + blk4x4), inverse_zigZag+16*blk4x4, quant_mat, 16);
				}
	...
	}else
	{
		...
		for (blk4x4 = 0; blk4x4 < 16; blk4x4++)
		{
			if ( cbp & (1 << (blk4x4/4)) )
			{
				...
				numCoeff = readCoeff4x4_CAVLC_sw(img_ptr, mb_cache_ptr, coeff, blk4x4, inverse_zigZag, quant_mat, maxCoeff);
...
void decode_CHROMA_DC (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp_uv[2])
{
	...
	for (uv = 0; uv < 2; uv++)
	{
		...
		total_coeff = readCoeff4x4_CAVLC_sw(img_ptr, mb_cache_ptr,DC, CHROMA_DC_BLOCK_INDEX, inverse_zigZag, NULL, 4);
		if (total_coeff)
		{
...
void decode_CHROMA_AC_sw (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp_uv[2])
{
	int uv;
	int numCoeff;
	int blkIndex;
	...
	for (uv = 0; uv < 2; uv++)
	{
		...
		{	
			...
			numCoeff = readCoeff4x4_CAVLC_sw(img_ptr, mb_cache_ptr, coeff, blkIndex, inverse_zigZag, quant_mat, 15);
			if (numCoeff)
			{
...
void decode_mb_cavlc_sw (void *img_ptr, DEC_MB_INFO_T *mb_info_ptr, DEC_MB_CACHE_T *mb_cache_ptr)
{
	int32 qp_uv, qp_c[2];
	int32 qp;
#ifdef _NEON_OPT_
	...
	decode_LUMA_AC_sw ((H264DecContext *)img_ptr, mb_info_ptr, mb_cache_ptr, qp);
	...
	{
		decode_CHROMA_AC_sw ((H264DecContext *)img_ptr, mb_info_ptr, mb_cache_ptr, qp_c); 
	}
}
...
static int32 readCoeff4x4_CABAC_sw(void *img, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T *mb_cache_ptr, int16 *block, int cat, int32 n, const uint8 *scantable, const uint32 *qmul, int32 max_coeff)
{
...
void decode_LUMA_DC_cabac_sw (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp)
{
	int16 DC[16];
	int32 total_coeff;
	const uint8 * inverse_zigZag = g_inverse_zigzag_tbl;
	...
	total_coeff = readCoeff4x4_CABAC_sw (img_ptr, currMB, mb_cache_ptr, DC, LUMA_DC, 0, inverse_zigZag, NULL, 16);
...
void decode_LUMA_AC_cabac_sw (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp)
{
	int32 maxCoeff;
	int32 blk4x4, blk8x8;
	int16 * coeff = mb_cache_ptr->coff_Y;
	...
	if (currMB->transform_size_8x8_flag)
	{
		...
		for (blk8x8= 0; blk8x8 < 4; blk8x8++)
		{
			if (cbp & (1 << blk8x8))
			{
				...
				clear_Nx16words(coeff, 2);
			#endif
				readCoeff4x4_CABAC_sw (img_ptr, currMB, mb_cache_ptr, coeff, 5, (blk8x8<<2), inverse_zigZag, quant_mat, maxCoeff);
			}
		...
		for (blk8x8= 0; blk8x8 < 4; blk8x8++)
		{
			if (cbp & (1 << blk8x8))
			{
				for (blk4x4 = 0; blk4x4 < 4; blk4x4++)
				{
					...
					readCoeff4x4_CABAC_sw (img_ptr, currMB, mb_cache_ptr, coeff, blk_type, ((blk8x8<<2) + blk4x4), inverse_zigZag, quant_mat, maxCoeff);	
					coeff += 16;
				}
			}else
			{
...
void decode_CHROMA_DC_cabac_sw (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp_uv[2])
{
//	int k;
	int uv;
	int16 DC[4];
	...
	{		
		...
		total_coeff = readCoeff4x4_CABAC_sw (img_ptr, currMB, mb_cache_ptr, DC, CHROMA_DC, uv, inverse_zigZag, NULL, 4);
...
void decode_CHROMA_AC_cabac_sw (H264DecContext *img_ptr, DEC_MB_INFO_T * currMB, DEC_MB_CACHE_T * mb_cache_ptr, int qp_uv[2])
{
//	int k;
	int uv;
	int numCoeff;
...
//	int index;
	...
	for (uv = 0; uv < 2; uv++)
	{
		...
		#else
			numCoeff = readCoeff4x4_CABAC_sw (img_ptr, currMB, mb_cache_ptr, coeff, CHROMA_AC, blkIndex, inverse_zigZag, quant_mat, 15);
		#endif
			if (numCoeff)
			{
				mb_cache_ptr->cbp_uv |= 0x1 << (uv*4 + blk4x4);
...
void decode_mb_cabac_sw (void *img, DEC_MB_INFO_T *mb_info_ptr, DEC_MB_CACHE_T *mb_cache_ptr)
{
	H264DecContext *img_ptr = (H264DecContext *)img;
	int32 qp_uv, qp_c[2];
	int32 qp;
	...
	if (mb_info_ptr->mb_type == I16MB)
	{
		...
		clear_Nx16words(mb_cache_ptr->coff_Y, 8);
	#endif
		decode_LUMA_DC_cabac_sw(img_ptr,mb_info_ptr, mb_cache_ptr, qp);
	}

	decode_LUMA_AC_cabac_sw(img_ptr, mb_info_ptr, mb_cache_ptr, qp);
