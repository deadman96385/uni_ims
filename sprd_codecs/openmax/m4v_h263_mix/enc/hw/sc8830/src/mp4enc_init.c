...
PUBLIC MMEncRet Mp4Enc_InitSession(Mp4EncObject *vo)
{
    ...
    vop_mode_ptr->pYUVRecFrame->imgYAddr = Mp4Enc_ExtraMem_V2P(vo, vop_mode_ptr->pYUVRecFrame->imgY, EXTRA_MEM) >> 3;
    vop_mode_ptr->pYUVRecFrame->imgUAddr = Mp4Enc_ExtraMem_V2P(vo, vop_mode_ptr->pYUVRecFrame->imgU, EXTRA_MEM) >> 3;
    vop_mode_ptr->pYUVRecFrame->imgVAddr = Mp4Enc_ExtraMem_V2P(vo, vop_mode_ptr->pYUVRecFrame->imgV, EXTRA_MEM) >> 3;

    vop_mode_ptr->pYUVRefFrame->imgYAddr = Mp4Enc_ExtraMem_V2P(vo, vop_mode_ptr->pYUVRefFrame->imgY, EXTRA_MEM) >> 3;
    vop_mode_ptr->pYUVRefFrame->imgUAddr = Mp4Enc_ExtraMem_V2P(vo, vop_mode_ptr->pYUVRefFrame->imgU, EXTRA_MEM) >> 3;
    vop_mode_ptr->pYUVRefFrame->imgVAddr = Mp4Enc_ExtraMem_V2P(vo, vop_mode_ptr->pYUVRefFrame->imgV, EXTRA_MEM) >> 3;
...
PUBLIC void Mp4Enc_InitVSP(Mp4EncObject *vo)
{
    ...
    cmd	= (uint32)Mp4Enc_ExtraMem_V2P(vo, (uint8 *)vo->g_vlc_hw_ptr, EXTRA_MEM)>>3;		// Frm_addr3[31:0], VLC Table set by Fixed Table Address
