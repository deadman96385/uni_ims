					 ...
					 )//weihu
{
	...
	//MB_para in
	intra_chroma_pred_mode=(mbc_para_buf[0]>>8)&0x3;
	Ipred_size=(mbc_para_buf[0]>>6)&0x3;	
	Ipred_avail_d=(mbc_para_buf[0]>>5)&0x1;
	Ipred_avail_c=(mbc_para_buf[0]>>2)&0x1;
	Ipred_avail_b=(mbc_para_buf[0]>>3)&0x1;
	Ipred_avail_a=(mbc_para_buf[0]>>4)&0x1;
    //skip_idct=(mbc_para_buf[0]>>16)&0x1;
	is_IPCM=(mbc_para_buf[0]>>1)&0x1;
	is_intra=(mbc_para_buf[0])&0x1;
	
	
    if(Ipred_size==I_16X16)
	{
	   intra16x16_luma_pred_mode=(mbc_para_buf[1])&0x3;//2b
	}
	else //if(Ipred_size==I_4X4)
	{
		for(i=0;i<8;i++)
		{
			i4_luma_pred_mode[i]=(mbc_para_buf[1]>>(i*4))&0xf;//4b
			i4_luma_pred_mode[i+8]=(mbc_para_buf[2]>>(i*4))&0xf;//4b
		}
		
	}

	cbp=(mbc_para_buf[3])&0xffffff;;//24b
	if((cbp==0)&&(!is_intra)||is_IPCM)
		skip_idct=1;
	else
        skip_idct=0;
	
	cur_mb_x=(mbc_para_buf[3]>>24)&0x3f;
	cur_mb_y=(mbc_para_buf[0]>>10)&0x3f;
	...
	if(!is_intra)
		...
		mb_mode=!(Ipred_size&0x1);//!=8x8
	//mbc_cbp=(cbp&0xffc3c3)|((cbp&0x3030)>>2)|((cbp&0x0c0c)<<2);
	mbc_cbp=cbp;

	MBC_CMD0=((Ipred_size>>1)<<31)|((!is_intra)<<30)|(mb_mode<<28)|((Ipred_avail_d&&is_intra)<<27)|((Ipred_avail_a&&is_intra)<<26)|((Ipred_avail_b&&is_intra)<<25)|((Ipred_avail_c&&is_intra)<<24)|mbc_cbp;
    if (Ipred_size==I_16X16)
	{
	...
	{
        MBC_CMD1=(i4_luma_pred_mode[0]<<28)|(i4_luma_pred_mode[1]<<24)|(i4_luma_pred_mode[2]<<20)|(i4_luma_pred_mode[3]<<16)|(i4_luma_pred_mode[4]<<12)|(i4_luma_pred_mode[5]<<8)|(i4_luma_pred_mode[6]<<4)|(i4_luma_pred_mode[7]);
		MBC_CMD2=(i4_luma_pred_mode[8]<<28)|(i4_luma_pred_mode[9]<<24)|(i4_luma_pred_mode[10]<<20)|(i4_luma_pred_mode[11]<<16)|(i4_luma_pred_mode[12]<<12)|(i4_luma_pred_mode[13]<<8)|(i4_luma_pred_mode[14]<<4)|(i4_luma_pred_mode[15]);	
	}
