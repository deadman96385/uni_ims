				   ...
				   )
{
	...
	is_cabac	= (g_hvld_reg_ptr->mb_info >> 31) & 1;

	g_range		= (g_hvld_reg_ptr->arith_bs_state >> 16) & 0x1ff;
	g_offset	= (g_hvld_reg_ptr->arith_bs_state >>  0) & 0x3ff;

	if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
	{
		VLD_FPRINTF (g_hvld_trace_fp, "frame: %d, mb_y: %d, mb_x: %d\n", g_nFrame_dec_h264, (g_glb_reg_ptr->VSP_CTRL0 >> 8) & 0x7f, (g_glb_reg_ptr->VSP_CTRL0 >> 0) & 0x7f);//weihu //0x3f
		VLD_FPRINTF (g_hvld_trace_fp, "cbp: 0x%x\n", cbp);
	}
	...
	if (mb_type != H264_IPCM)
	{
		if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
		{
//			VLD_FPRINTF (g_hvld_trace_fp, "mb type: %d, cbp: %d, left_avai: %d, top_avail: %d\n", 
//				mb_type, cbp, lmb_avail, tmb_avail);
			
//			VLD_FPRINTF (g_hvld_trace_fp, "top_nnz_y: %08x, left_nnz_y: %08x, tl_nnz_cb: %08x, tl_nnz_cr: %08x\n",
//				g_hvld_reg_ptr->top_nnz_y, g_hvld_reg_ptr->left_nnz_y, g_hvld_reg_ptr->tl_nnz_cb, g_hvld_reg_ptr->tl_nnz_cr);
		}
	}

//	if ((g_nFrame_dec == 0) && (g_mb_x == 1) && (g_mb_y == 0))
	//	printf ("");
	...
	if (mb_type == H264_IMB16X16)
	{
		...
		if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
		{
			VLD_FPRINTF (g_hvld_trace_fp, "DC of IMB16x16\n");
		}
	...
	if (cbp_luma)
	{
		if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
		{
//			VLD_FPRINTF (g_hvld_trace_fp, "AC of LUMA\n");
		}
	}
	...
	if (cbp_luma > 0)
	{
		...
		while (1)
		{
			...
			if (cbp_8x8)
			{
				...
				if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
				{
					VLD_FPRINTF (g_hvld_trace_fp, "luma_ac, blk_id: %d\n", blk_id);
				}

				blk_type = is_intra16 ? LUMA_AC_I16 : LUMA_AC;

				if (!is_cabac)
					H264VldBlk (blk_type, blk_id, start_pos, max_coeff_num, lmb_avail, tmb_avail, &error);
				else
					CabacBlk (mb_type, blk_type, blk_id);
	...
	if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
	{
//		VLD_FPRINTF (g_hvld_trace_fp, "DC of CHROMA\n");
	}
	...
	for (blk_id = 0; blk_id < 2; blk_id++)
	{
		...
		if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
		{
			VLD_FPRINTF (g_hvld_trace_fp, "chroma_dc, blk_id: %d\n", blk_id);
		}
		max_coeff_num = 4;
		...
		if (!is_cabac)
			...
			CabacBlk (mb_type, CHROMA_DC, blk_id);
	...
	if (cbp_chroma & 2)
	{
		if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
		{
//			VLD_FPRINTF (g_hvld_trace_fp, "AC of CHROMA\n");
		}
		...
		for (blk_id = 0; blk_id < 8; blk_id++)
		{
			...
			if(g_trace_enable_flag&TRACE_ENABLE_VLD)	
			{
				if (blk_id < 4)
				{
					VLD_FPRINTF (g_hvld_trace_fp, "chroma_ac blk_id: %d\n", blk_id);
				}else
				{
					VLD_FPRINTF (g_hvld_trace_fp, "chroma_ac blk_id: %d\n", blk_id-3);
				}
			...
			if (!is_cabac)
				...
				CabacBlk (mb_type, CHROMA_AC, blk_id);
...
//	if ((g_hvld_reg_ptr->hvld_ctr) & (1 << 2))
//		exit (-1);
	
//	PrintfDCTBuf ();//weihu

	g_hvld_reg_ptr->arith_bs_state = (g_range << 16) | g_offset;
