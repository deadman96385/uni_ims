...
read_ue_v_skip:
		ldr		r12, =g_image_ptr
		ldr		r12, [r12]
		ldrb		bitcnt, [r12, #error_flag_POSITION]
		mov		r0, #0
		orr		bitcnt, bitcnt, #ER_BSM_ID
		strb		bitcnt, [r12, #error_flag_POSITION]
read_ue_v_exit:		
		pop		{r4, pc}
		@ENDFUNC
...
@//int32 get_unit (uint8 *pInStream, int32 frm_bs_len, int32 *slice_unit_len, int32 *start_code_len)
pInStream			.req 		r0		
frm_bs_len			.req		r1
slice_unit_len_ptr		.req 		r2
start_code_len_ptr		.req 		r3
dec_len				.req 		r3
nalu_ptr				.req 		r12
data 				.req 		r4
len					.req 		r5
