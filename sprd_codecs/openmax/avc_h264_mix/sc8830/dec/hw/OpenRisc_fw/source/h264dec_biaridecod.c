...
#if SIM_IN_WIN
	nDecTotalBits = stream->bitcnt;//VSP_READ_REG(VSP_BSM_REG_BASE+BSM_TOTAL_BITS_OFF, "read BSMR_TOTAL_BITS reg");
	OR1200_READ_REG(BSM_CTRL_REG_BASE_ADDR+TOTAL_BITS_OFF,"BSM TOTAL_BITS");
...
uint32 biari_decode_symbol (DEC_IMAGE_PARAMS_T *img_ptr, BiContextType *bi_ctx_ptr)//return binVal, and the updated variables codIRange and codIOffset
{
	DEC_BS_T * stream = img_ptr->bitstrm_ptr;
	DecodingEnvironment* dep = &img_ptr->de_cabac;
...
//	uint32 rLPS = rLPS_table_64x4[bi_ctx_ptr->state][(range>>6)&0x3];
	...
	uint32 rLPS = rLPS_table_64x4[state][(range>>6)&0x3];//state:pStateIdx

	range -= rLPS;

	if (value < range) /*MPS*///value:codIOffset@jzy
	{
	...
	}else	/*LPS*/
	{
		...
		*bi_ctx_ptr = (*bi_ctx_ptr&0x1)|(state<<1);//(*bi_ctx_ptr&0x1):valMPS
	...
	while (range < QUARTER)
	{
		...
		value = (value << 1) | (READ_FLC(stream, 1));
	}
