 ...
 **	Note:
 *****************************************************************************/
PUBLIC void Mp4Dec_DecGobHeader(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	if(gob_number > 0)
	{
		...
		if((GOB_RESYNC_MARKER == tmp_var) || (GOB_RESYNC_MARKER == tmp_var2))
		{
			...
			vop_mode_ptr->StepSize = (int8)Mp4Dec_ReadBits((uint32)(vop_mode_ptr->QuantPrecision));//, "quant_scale"
				
			mb_num						= vop_mode_ptr->mb_y * vop_mode_ptr->MBNumX;
// 			vop_mode_ptr->err_mb_num	+= mb_num - vop_mode_ptr->mbnumDec;
...
int32 Mp4Dec_DecH263PicInfo(DEC_VOP_MODE_T *vop_mode_ptr, int32 source_format)
{
...
PUBLIC MMDecRet Mp4Dec_DecH263Header(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	{		
		Mp4Dec_DecH263PicInfo(vop_mode_ptr, tmpVar);			
		...
		{
			vop_mode_ptr->mvInfoForward.FCode = 2;
		}
 ...
 **	Note:
 *****************************************************************************/
PUBLIC void Mp4Dec_GetVideoPacketHeader(DEC_VOP_MODE_T *vop_mode_ptr, uint32 uAddBits)
{
	int32	tmp_var;
	int32	hec;
	int		mb_num;
