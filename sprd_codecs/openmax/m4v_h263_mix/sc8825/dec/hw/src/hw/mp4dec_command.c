...
PUBLIC void Mp4Dec_init_frame_VSP(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr, MMDecInput *dec_input_ptr)
{
	uint32 cmd;
...
#else
		((void *)Mp4Dec_ExtraMem_V2P(vd, vop_mode_ptr->frame_bistrm_ptr, HW_CACHABLE))
#endif		
...
PUBLIC MMDecRet Mp4Dec_VspFrameInit(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr, uint32 bitstm_addr)
{
	uint32 cmd;
...
#else
	VSP_WRITE_REG(VSP_MEMO10_ADDR+20*4, ((uint32)Mp4Dec_ExtraMem_V2P(vd, g_cmd_info_base, HW_CACHABLE))>>2, "config CMD control info buffer address");
	VSP_WRITE_REG(VSP_MEMO10_ADDR+21*4, ((uint32)Mp4Dec_ExtraMem_V2P(vd, g_cmd_data_base, HW_CACHABLE))>>2, "config CMD control data buffer address");
#endif
...
#else
	VSP_WRITE_REG(pTableAddr+ 12, ((uint32)Mp4Dec_ExtraMem_V2P(vd, vop_mode_ptr->pTopCoeff, HW_NO_CACHABLE))>>2, "config top coeff address");	
#endif 
