			 ...
			 )
{
	...
	SCI_ASSERT(NULL != inter_malloc_bfr_ptr);
	SCI_ASSERT(NULL != extra_malloc_bfr_ptr);
//	SCI_ASSERT(NULL != video_format_ptr);

#ifdef SIM_IN_WIN
//	HVlcTestVectorInit ();
#endif

	h264enc_mem_init(inter_malloc_bfr_ptr, extra_malloc_bfr_ptr);
	...
	img_ptr->width = video_format_ptr->frame_width;
	img_ptr->height = video_format_ptr->frame_height;
// 	img_ptr->i_frame = 0;
	img_ptr->frame_num = 0;
...
#else
	img_ptr->pOneFrameBitstream = bitstream_bfr_ptr->common_buffer_ptr;
	img_ptr->OneframeStreamLen = bitstream_bfr_ptr->size;
...
MMEncRet H264EncStrmEncode(MMEncIn *pInput, MMEncOut *pOutput)
{
	...
	img_ptr->stm_offset = 0;
	img_ptr->pYUVSrcFrame->imgY =  pInput->p_src_y;
	img_ptr->pYUVSrcFrame->imgUV =  pInput->p_src_u;
	...
	else if (i_slice_type == SLICE_TYPE_P)
	{
		...
		if(img_ptr->sh.i_first_mb==0)
		{
			g_h264_enc_config->qp_PSLICE = rc_init_pict(&(rc_gop_paras), &(rc_pic_paras));
			rc_pic_paras.curr_pic_qp = g_h264_enc_config->qp_PSLICE;
			i_global_qp = g_h264_enc_config->qp_PSLICE;
		...
		{
#ifdef NO_BU_CHANGE
			g_h264_enc_config->qp_PSLICE = rc_pic_paras.curr_pic_qp = rc_init_slice(img_ptr, &(rc_pic_paras));
			i_global_qp = g_h264_enc_config->qp_PSLICE;
#else
			g_h264_enc_config->qp_PSLICE = rc_pic_paras.curr_pic_qp;
			i_global_qp = g_h264_enc_config->qp_PSLICE;
#endif
		}
		...
		PRINTF("No.%d Frame:\t P_SLICE\t QP=%d", g_nFrame_enc, g_input->qp_PSLICE);
		fprintf(stat_out, "No.%d Frame:\t P_SLICE\t QP=%d", g_nFrame_enc, g_input->qp_PSLICE);
#endif
		i_nal_type = NAL_SLICE;
...
//			*((uint32*)img_ptr->pOneFrameBitstream) = 0x01000000;
...
#else
		{
			*((volatile uint32*)(&img_ptr->pOneFrameBitstream[img_ptr->stm_offset-or_addr_offset])) = 0x01000000;
			/*if(*((volatile uint32*)(&img_ptr->pOneFrameBitstream[img_ptr->stm_offset-or_addr_offset])) == 0xffffffff)
			{
				uint32 tmp = OR1200_READ_REG(ORSC_BSM_CTRL_OFF+0x2c,"ORSC: DSTUF_NUM");	// just for test
				*((volatile uint32*)(&img_ptr->pOneFrameBitstream[img_ptr->stm_offset-or_addr_offset])) = 0x00000001;
			}
			else
			{
				uint32 tmp = OR1200_READ_REG(ORSC_BSM_CTRL_OFF+0x14,"ORSC: TOTAL_BITS");	// just for test
			}*/
			/*uint32 offset = ((uint32)img_ptr->pOneFrameBitstream);
			offset += img_ptr->stm_offset;
			if( *((volatile uint32*)offset) == 0xffffffff )
				*((volatile uint32*)offset) = 0x00000001;*/
		}
...
//		*((uint32*)img_ptr->pOneFrameBitstream) = 0x01000000;
...
#else
	{
		*((volatile uint32*)(&img_ptr->pOneFrameBitstream[img_ptr->stm_offset-or_addr_offset])) = 0x01000000;
		...
		/*if ( (img_ptr->pOneFrameBitstream[img_ptr->stm_offset+0] == 0xff) &&
			 (img_ptr->pOneFrameBitstream[img_ptr->stm_offset+1] == 0xff) &&
			 (img_ptr->pOneFrameBitstream[img_ptr->stm_offset+2] == 0xff) &&
			 (img_ptr->pOneFrameBitstream[img_ptr->stm_offset+3] == 0xff) )
		{
			img_ptr->pOneFrameBitstream[img_ptr->stm_offset+0] = 0x00;
			img_ptr->pOneFrameBitstream[img_ptr->stm_offset+1] = 0x00;
			img_ptr->pOneFrameBitstream[img_ptr->stm_offset+2] = 0x00;
			img_ptr->pOneFrameBitstream[img_ptr->stm_offset+3] = 0x01;
		}*/
	}
