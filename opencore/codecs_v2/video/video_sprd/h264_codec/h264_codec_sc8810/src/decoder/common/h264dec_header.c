...
#else
		ret = H264Dec_process_slice (img_ptr, nal_ptr);
		if (!g_searching_IDR_pic || (img_ptr->type == I_SLICE))
		{
			g_ready_to_decode_slice = TRUE;
...
LOCAL MMDecRet H264Dec_dec_ref_pic_marking (DEC_IMAGE_PARAMS_T *img_ptr)
{
	int32 val;
...
LOCAL void H264Dec_pred_weight_table (DEC_IMAGE_PARAMS_T *img_ptr, DEC_SLICE_T *curr_slice_ptr)
{
	int32 i, j,list;
	DEC_BS_T *stream = img_ptr->bitstrm_ptr;
	...
	for (list = 0; list < 2; list++)
	{
		for (i = 0; i < img_ptr->ref_count[list]; i++)
		{
			...
			if (luma_weight_flag)
			{
				g_wp_weight[list][i][0] = READ_SE_V(stream);
				g_wp_offset[list][i][0] = READ_SE_V(stream);
			}else
			{
				g_wp_weight[list][i][0] = 1 << img_ptr->luma_log2_weight_denom;
				g_wp_offset[list][i][0] = 0;
			}
			...
			for (j = 1; j < 3; j++)
			{
				...
				{
					g_wp_weight[list][i][j] = READ_SE_V(stream);
					g_wp_offset[list][i][j] = READ_SE_V(stream);
				}else
				{
					g_wp_weight[list][i][j] = 1 << img_ptr->chroma_log2_weight_denom;
					g_wp_offset[list][i][j] = 0;
				}
			}
		}
	}
...
PUBLIC MMDecRet H264Dec_RestSliceHeader (DEC_IMAGE_PARAMS_T *img_ptr, DEC_SLICE_T *curr_slice_ptr)
{
	...
	if (img_ptr->direct_spatial_mv_pred_flag == 1)
	{
		...
		if (img_ptr->VSP_used)
		{
			pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_spatial_hw;
			img_ptr->b8_mv_pred_func[0] = H264Dec_MC8x8_direct_spatial_hw;
		}else
		{
			pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_spatial_sw;
			img_ptr->b8_mv_pred_func[0] = H264Dec_MC8x8_direct_spatial_sw;
		}
	}else
	{
		...
		if (img_ptr->VSP_used)
		{
			pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_temporal_hw;
			img_ptr->b8_mv_pred_func[0] = H264Dec_MC8x8_direct_temporal_hw;
		}else
		{
			pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_temporal_sw;
			img_ptr->b8_mv_pred_func[0] = H264Dec_MC8x8_direct_temporal_sw;
		}
	}
	...
	if ((P_SLICE == img_ptr->type) || (B_SLICE == img_ptr->type))
	{
		...
		if (img_ptr->ref_count[0] > MAX_REF_FRAME_NUMBER)
		{
		 	img_ptr->error_flag |= ER_BSM_ID;
	...
	{
		H264Dec_pred_weight_table (img_ptr, curr_slice_ptr);
	}
