 ...
 * For example if
 * _VE_netLoopbackCount = -1 , data will never be looped back
 * if
 * _VE_netLoopbackCount = 100, first 100 packets will be looped back,
 * encoded data will not be sent while looping back.
 */
static int _VE_netLoopbackCount = -1;
 ...
 *
 */
OSAL_INLINE OSAL_NetSockId _VE_netSocket(
    ...
    OSAL_NetAddress *recvAddr)
{
    ...
    if (_VE_netLoopbackCount > 0) {
        int ret = retVal;
        OSAL_logMsg("Recvd %d bytes, sending back...\n", ret);
        if (OSAL_SUCCESS == VIDEO_NET_SOCKET_SEND_TO(
                &socketFd, buf_ptr, &ret, recvAddr)) {
            OSAL_logMsg("Sent %d bytes back...\n", ret);
        }
        _VE_netLoopbackCount--;
    }
 ...
 * are same or not.
 */
OSAL_INLINE OSAL_Boolean _VE_netIsSameAddr(
