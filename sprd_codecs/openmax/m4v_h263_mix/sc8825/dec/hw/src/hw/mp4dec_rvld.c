...
LOCAL int32 Mp4Dec_FastSearch(const uint16 *code_table, uint16 code);
/**----------------------------------------------------------------------------*
**                           Function Prototype                               **
**----------------------------------------------------------------------------*/
/*****************************************************************************
...
//	TCOEF_T tcoef =	{0, 0, 0, 0};
	const RVLC_TABLE_CODE_LEN_T *rvlc_code_len = PNULL;
	const RVLC_TABLE_CODE_LEN_T *rvlc_code_len_table = g_dec_rvlc_dct3d_tab_intra;
	const uint16 *code_tab = g_dec_rvlc_code_tab_intra;
	const uint16 *index_tab = g_dec_rvlc_index_tab_intra;
	uint32 tmp_var;
	...
	run_level_ptr->run = (rvlc_code_len->code >> 8) & 0xff;
	run_level_ptr->level = rvlc_code_len->code & 0xff;
	run_level_ptr->last = (rvlc_code_len->code >> 16) & 1;		
	...
	{		
		...
		run_level_ptr->last = (int16)(tmp_var>>6); 
		run_level_ptr->run = (int16)(tmp_var & 0x3F);
		...
		run_level_ptr->level = (int16)((tmp_var & 0x1FFFF)>>6);
		run_level_ptr->sign = (int16)(tmp_var & 0x1F);
	}else
	{
		run_level_ptr->sign = (int16)Mp4Dec_ReadBits(bitstrm_ptr, 1); //, "SIGN");
	}	
...
//	TCOEF_T tcoef = {0, 0, 0, 0};	
	int32 index;
	const RVLC_TABLE_CODE_LEN_T *rvlc_code_len;
	const RVLC_TABLE_CODE_LEN_T *rvlc_code_len_table = g_dec_rvlc_dct3d_tab_inter;
	const uint16 *code_tab = g_dec_rvlc_code_tab_inter;
	const uint16 *index_tab = g_dec_rvlc_index_tab_inter;
	uint32 tmp_var;
	...
	run_level_ptr->run = (rvlc_code_len->code >> 8) & 255;
	run_level_ptr->level = rvlc_code_len->code & 255;
	run_level_ptr->last = (rvlc_code_len->code >> 16) & 1;  
