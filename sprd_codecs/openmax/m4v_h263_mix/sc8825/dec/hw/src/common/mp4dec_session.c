...
#else
		if (mb_num_x*mb_num_y < 100) {
			vop_mode_ptr->cmd_data_buf[0]= (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 6*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
			vop_mode_ptr->cmd_info_buf[0] = (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 2*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);

			vop_mode_ptr->cmd_data_buf[1]= (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 6*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
			vop_mode_ptr->cmd_info_buf[1] = (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 2*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
		}
		else{
			vop_mode_ptr->cmd_data_buf[0]= (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 3*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
			vop_mode_ptr->cmd_info_buf[0] = (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 1*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);

			vop_mode_ptr->cmd_data_buf[1]= (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 3*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
			vop_mode_ptr->cmd_info_buf[1] = (uint32 *) Mp4Dec_ExtraMemAlloc(vd, 1*(vop_mode_ptr->MBNum)*256, 4, HW_CACHABLE);
		}
