...
void h264_add_ref_and_residual (void)
{
	...
	int32 bZeroBlk;	
	int32 uv;
	int32 s_blk_offset_in_mca_bfr[24] = 
	{
		//luma
		0,								4,                   8,                  12,
	...
	//8,   9,  10,  11,
	//12, 13,  14,  15
	if((g_mbc_reg_ptr->MBC_CMD0 >> 31)&0x1)//8x8block
	{
		...
		for (blkIdxInMB = 0; blkIdxInMB < 4; blkIdxInMB++)
		{
			...
			pMbc = pMbcBfr + 26*4 + i * 8 * MBC_Y_SIZE + j * 8;
			p4x4blk = pBlkCff + g_blk_rec_ord_tbl[i*8+j*2] * 16;
			pRef = pMcaBfr + s_blk_offset_in_mca_bfr[i*8+j*2];

			bZeroBlk = !((g_mbc_reg_ptr->MBC_CMD0 >> (i*8+j*2)) & 0x01);

			//add residual coef
			for (k = 0; k < 8; k++)
			{
				pMbc[0] = IClip(0,255, pRef[0] + (bZeroBlk ? 0 : p4x4blk[k*8+0]));
				pMbc[1] = IClip(0,255, pRef[1] + (bZeroBlk ? 0 : p4x4blk[k*8+1]));
				pMbc[2] = IClip(0,255, pRef[2] + (bZeroBlk ? 0 : p4x4blk[k*8+2]));
				pMbc[3] = IClip(0,255, pRef[3] + (bZeroBlk ? 0 : p4x4blk[k*8+3]));
				pMbc[4] = IClip(0,255, pRef[4] + (bZeroBlk ? 0 : p4x4blk[k*8+4]));
				pMbc[5] = IClip(0,255, pRef[5] + (bZeroBlk ? 0 : p4x4blk[k*8+5]));
				pMbc[6] = IClip(0,255, pRef[6] + (bZeroBlk ? 0 : p4x4blk[k*8+6]));
				pMbc[7] = IClip(0,255, pRef[7] + (bZeroBlk ? 0 : p4x4blk[k*8+7]));
	...
	else//4x4block
	{
		...
		for (blkIdxInMB = 0; blkIdxInMB < 16; blkIdxInMB++)
		{
			...
			pRef = pMcaBfr + s_blk_offset_in_mca_bfr[blkIdxInMB];
			...
			for (k = 0; k < 4; k++)
			{
				pMbc[0] = IClip(0,255, pRef[0] + (bZeroBlk ? 0 : p4x4blk[k*4+0]));
				pMbc[1] = IClip(0,255, pRef[1] + (bZeroBlk ? 0 : p4x4blk[k*4+1]));
				pMbc[2] = IClip(0,255, pRef[2] + (bZeroBlk ? 0 : p4x4blk[k*4+2]));
				pMbc[3] = IClip(0,255, pRef[3] + (bZeroBlk ? 0 : p4x4blk[k*4+3]));
