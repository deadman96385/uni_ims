...
PUBLIC void Mp4Dec_InitYUVBfr(DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 id;
	int32 size_y = vop_mode_ptr->FrameWidth * vop_mode_ptr->FrameHeight;	
	int32 size_uv = size_y >> 2;
	DEC_FRM_BFR *pDecFrame = g_FrmYUVBfr;
	...
	for(id = 0; id < DEC_YUV_BUFFER_NUM; id++)
	{
		...
		pDecFrame->imgY = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(size_y * sizeof(uint8));  //y
		if (vop_mode_ptr->uv_interleaved) //two plane
		{
			pDecFrame->imgU = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(2*size_uv * sizeof(uint8));  //uv
			pDecFrame->imgV = PNULL;
		}else //three plane
		{
			pDecFrame->imgU = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(size_uv * sizeof(uint8));  //u
			pDecFrame->imgV = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(size_uv * sizeof(uint8));  //v
		}
