			   ...
			   )
{
	...
	if(isyahdamard==1)//if hadamard transform
	{
		...
		else if((decoder_format==STREAM_ID_VP8)||(decoder_format==STREAM_ID_WEBP))//if VP8/webp 
		{
			...
			if(step==0)
			{   
				r=0; s=0; z=32768;//65536; 
			}
			else
			{
				r=3; s=3; z=32768;//65536;
			}
		}
			   ...
			   )
{
	...
	short a, c, f, r0, r1, s;//,,,,,,,round,shift;
	int z;//,clip
	short x,y,z0,z1,m;
	...
	{
		a=1; c=1;  x=2; y=0; z0=3; z1=1; m=1; //f=1/2;
		if(step==0)
		{   
    ...
    for (i = 0; i < blk_size; i++)
    {
        for (j = 0; j < blk_size; j++)
		{
			...
			if(decoder_format==STREAM_ID_H264)
			{
				 ...
				 {
					 int per_off;
					 per_off = (blk_size==8) ? 2 : 0;
					 if(per>=4+per_off)
					 {
						 qp_const=0;
						 shift_value=0;
						 lshift_value=per-per_off-4;//0-4 per:4-8
					 }
					 else
					 {
						 qp_const=1<<(3+per_off-per);//0-3
						 shift_value=4+per_off-per;//1-4 per:0-3
						 lshift_value=0;
					 }
				   ...
				   )
{
   	...
   	default:    //STREAM_ID_H264:h.264
		{
			...
			cbp26=(cbp26&0x3ffc3c3)|((cbp26&0x3030)>>2)|((cbp26&0x0c0c)<<2);
			inter=!intra_flag;//
 ...
 if(!skip_flag)    
 {
		 ...
		 if (need_y_hadama)
		{			
			fprintf(g_fp_hadarm_tv,"before_iq\n");
			for (i = 0; i < 4; i++)
					{
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+0]&0xffff);
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+1]&0xffff);
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+2]&0xffff);
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+3]&0xffff);
					   fprintf(g_fp_hadarm_tv,"\n");
					}
       ...
       
			{
				...
				fprintf(g_fp_hadarm_tv,"after_iq\n");
				for (i = 0; i < 4; i++)
					{
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+0]&0xffff);
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+1]&0xffff);
				...
				fprintf(g_fp_hadarm_tv,"after_itrans\n");
				for (i = 0; i < 4; i++)
					{
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+0]&0xffff);
					   fprintf(g_fp_hadarm_tv,"%04x",inbuf[384+i*4+1]&0xffff);
		...
		for (i = 0; i < 24; i+=b4x4inblk)//for (i = 0; i < 16; i++)
		{
            ...
            if(((cbp26>>i)&0x1)||(need_y_hadama&&(i<16))||(i>=16)&&((cbp26>>24)&0x1)&&(decoder_format==STREAM_ID_H264));
			{
				fprintf(g_fp_iquant_tv,"before_blk=%d,qp_ac=%d,qp_dc=%d\n",i, qp_ac, qp_dc);
				if((MB_info&0x1)==0)
				{
					for (j = 0; j < 4; j++)
						{
						   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*4+0]&0xffff);
						   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*4+1]&0xffff);
						   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*4+2]&0xffff);
						   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*4+3]&0xffff);					   
						   fprintf(g_fp_iquant_tv,"\n");
				...
				else
				{
				   for (j = 0; j < 8; j++)
					{
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+0]&0xffff);
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+1]&0xffff);
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+2]&0xffff);
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+3]&0xffff);	
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+4]&0xffff);
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+5]&0xffff);
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+6]&0xffff);
					   fprintf(g_fp_iquant_tv,"%04x",inbuf[16*i+j*8+7]&0xffff);
					   fprintf(g_fp_iquant_tv,"\n");
				...
				iquantblk(inbuf+16*i, inbuf+16*i, qp_ac, qp_dc, decoder_format,slice_info, MB_info,ycbcr,isyhadamard, inter);//iquant for DCT
				//iquant.txt//jzy
				fprintf(g_fp_iquant_tv,"after_blk=%d\n",i);
