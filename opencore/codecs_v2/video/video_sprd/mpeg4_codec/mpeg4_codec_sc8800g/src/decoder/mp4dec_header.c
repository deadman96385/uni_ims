...
//	assert((vop_mode_ptr->time_inc_resolution >= 1) && (vop_mode_ptr->time_inc_resolution < 65536));
	...
	vop_mode_ptr->size_decode_flag = TRUE;
...
PUBLIC MMDecRet Mp4Dec_DecMp4Header(DEC_VOP_MODE_T *vop_mode_ptr, uint32 uOneFrameLen)
{
	...
	{
		//SCI_TRACE_LOW("Mp4Header %d,%d",nDecTotalBits,uOneFrameLen);
		READ_REG_POLL (VSP_BSM_REG_BASE+BSM_DEBUG_OFF, V_BIT_3, V_BIT_3, TIME_OUT_CLK,
		...
		//SCI_TRACE_LOW("Mp4Dec_DecMp4Header uStartCode %x\n",uStartCode);
...
static MMDecRet Mp4Dec_DecH263PicInfo(DEC_VOP_MODE_T *vop_mode_ptr, int32 source_format)
{
...
static MMDecRet Mp4Dec_DecH263PlusHeader(DEC_VOP_MODE_T *vop_mode_ptr, int8 PicSize)
{
...
PUBLIC MMDecRet Mp4Dec_DecH263Header(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	{
		ret = Mp4Dec_DecH263PlusHeader(vop_mode_ptr, 0);
...
/*****************************************************************************
 **	Name : 			Mp4Dec_DecH263PicSize
 ** Description:	Get the size of Vop from the bitstream,H263(short video header)
 ** Author:			Xiaowei Luo
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecH263PicSize(DEC_VOP_MODE_T *vop_mode_ptr)
{
	uint32 tmpVar;
	BOOLEAN *pIs_stop_decode_vol = &g_dec_is_stop_decode_vol;
	MMDecRet ret;
	...
	if(tmpVar == EXTENDED_PTYPE)
	{
		ret = Mp4Dec_DecH263PlusHeader(vop_mode_ptr, 1);		
	}else
	{
		//H263_PLUS_HEAD_INFO_T *h263_plus = Mp4Dec_GetH263PlusHeadInfo();
		...
		g_dec_vop_mode_ptr->size_decode_flag = TRUE;
	}
...
PUBLIC MMDecRet Mp4Dec_FlvH263PicHeader(DEC_VOP_MODE_T *vop_mode_ptr)
{
    ...
    switch (format) {
    case 0:
        width = Mp4Dec_ReadBits(8);
        height = Mp4Dec_ReadBits(8);
        break;
    case 1:
        width = Mp4Dec_ReadBits(16);
        height = Mp4Dec_ReadBits(16);
        break;
    case 2:
        width = 352;
        height = 288;
    ...
    Mp4Dec_FlushBits(1); /* deblocking flag */
    s->StepSize = (int8)Mp4Dec_ReadBits(5);
...
PUBLIC MMDecRet Mp4Dec_FlvH263PicSize(DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 format, width, height;
	DEC_VOP_MODE_T *s = vop_mode_ptr;
