...
{
if(g_vector_enable_flag&VECTOR_ENABLE_VLC)
{
...
//	if (g_vlc_status)
	...
	//	VLC_FPRINTF (g_fp_vlc_tv, "%08x,%08x\n", val_la, nbits);
		//VLC_FPRINTF (g_fp_vlc_tv, "//data=%x length=%x\n", val, nbits);
		//VLC_FPRINTF (g_fp_vlc_tv, "%08x\n", val_la);
		VLC_FPRINTF (g_fp_vlc_tv, "data=%08x\n", val_la);
		VLC_FPRINTF (g_fp_vlc_tv, "length=%08x\n", nbits);
...
//		fprintf (g_huff_tab_fp, "%08x\n", val);
		fprintf(g_fp_huff_tab_tv, "%08x\n", val);
	}
	/*for (i = 0; i < 64; i++)
	{
		val = vsp_huff_dcac_tab[i*2];
		
		//		fprintf (g_huff_tab_fp, "%08x\n", val);
		fprintf(g_fp_huff_tab_tv, "%08x", vsp_huff_dcac_tab[i*2]);
		fprintf(g_fp_huff_tab_tv, "%08x\n", vsp_huff_dcac_tab[i*2+1]);
	}*/
}
}
