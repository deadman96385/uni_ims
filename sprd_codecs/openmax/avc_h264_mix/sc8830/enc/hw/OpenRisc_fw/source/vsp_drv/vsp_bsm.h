...
typedef struct  bsmr_control_register_tag
{
	...
	volatile uint32 BSM_CFG2;		//[29:24]: OPT_BITS, Number of bits to be flushed, only valid in decoding. The supported range is from 1 to 32 bits
									...
									//[1]: BSM_CLR, Move data remained in FIFO to external memory(encode) or discard data in FIFO(decode)
	...
	volatile uint32 BSM_DEBUG;		//[31]: BSM_STATUS, BSM is active(0)/inactive(1)
									//[30:28]: BSM_STATE, BSM control status
									//[27]: DATA_TRAN, 0: idle, bsm can be cleard,1, can not be cleared
	...
	volatile uint32 BSM_GLO_OPT;	//[3]: SE_V(), for encoder
									//[2]: UE_V(), for encoder
									//[1]: SE_V(), for decoder
									//[0]: UE_V(), for decoder
