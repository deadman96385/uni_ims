...
void h264enc_sps_write (ENC_SPS_T *sps)
{
	...
	H264Enc_OutputBits (sps->i_profile_idc, 8);
	H264Enc_OutputBits (sps->b_constraint_set0, 1);
	H264Enc_OutputBits (sps->b_constraint_set1, 1);
	H264Enc_OutputBits (sps->b_constraint_set2, 1);

	H264Enc_OutputBits (0, 5);	//reserved
	
	H264Enc_OutputBits (sps->i_level_idc, 8);
	...
	H264Enc_OutputBits (sps->b_gaps_in_frame_num_value_allowed, 1);
	WRITE_UE_V (sps->i_mb_width -1);
	WRITE_UE_V (sps->i_mb_height - 1);
	H264Enc_OutputBits (sps->b_frame_mbs_only, 1);
	H264Enc_OutputBits (sps->b_direct8x8_inference, 1);
	H264Enc_OutputBits (sps->b_crop, 1);
	if(sps->b_crop)
	{
		WRITE_UE_V (0);	// frame_crop_left_offset
		WRITE_UE_V (0);	// frame_crop_right_offset
		WRITE_UE_V (0);	// frame_crop_top_offset
		WRITE_UE_V (4);	// frame_crop_bottom_offset
	}
	H264Enc_OutputBits (sps->b_vui, 1);
...
void h264enc_pps_write (ENC_PPS_T *pps)
{
	...
	H264Enc_OutputBits (0/*pps->b_cabac*/, 1);
	H264Enc_OutputBits (pps->b_pic_order, 1);
	WRITE_UE_V(pps->i_num_slice_groups-1);

	WRITE_UE_V(pps->i_num_ref_idx_l0_active-1);
	WRITE_UE_V(pps->i_num_ref_idx_l1_active-1);
	H264Enc_OutputBits (0/*pps->b_weighted_pred*/, 1);
	H264Enc_OutputBits (0/*pps->b_weighted_bipred*/, 2);

	WRITE_SE_V (pps->i_pic_init_qp - 26);
	WRITE_SE_V (pps->i_pic_init_qs - 26);
	WRITE_SE_V (pps->i_chroma_qp_index_offset);

	H264Enc_OutputBits (pps->b_deblocking_filter_control, 1);
	H264Enc_OutputBits (pps->b_constrained_intra_pred, 1);
	H264Enc_OutputBits (pps->b_redundant_pic_cnt, 1);
...
//	free(opts);
	...
	H264Enc_OutputBits (0x5, 8);	//payload_type = user_data_unregistered
	//payload_size
	for(i = 0; i <= length-255; i+=255)
	{
		H264Enc_OutputBits(255, 8);
	}
	H264Enc_OutputBits(length-i, 8);

	for(i = 0; i < 16; i++)
	{
		H264Enc_OutputBits(uuid[i], 8);
	}

	for(i = 0; i < length-16; i++)
	{
		H264Enc_OutputBits(version[i], 8);
	}
