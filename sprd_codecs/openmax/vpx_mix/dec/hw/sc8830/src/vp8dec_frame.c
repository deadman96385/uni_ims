...
LOCAL int32 get_delta_q(VPXDecObject *vo, vp8_reader *bc, int32 prev, int32 *q_update)
{
    int32 ret_val = 0;
...
{
    const uint8 *data		= (const uint8 *)vo->Source;
    const uint8 *data_end	= (const uint8 *)(data + vo->source_sz);
    uint32 cmd;
    vp8_reader *const bc = & vo->bc;
    VP8_COMMON *const pc = & vo->common;
    MACROBLOCKD *const xd  = & vo->mb;
    int32 first_partition_length_in_bytes;
    int32 i, j, k, l;
    ...
    // 3 byte header
    pc->frame_type = (FRAME_TYPE_E)(data[0] & 1);
    ...
    if (pc->frame_type == KEY_FRAME) {
        pc->clr_type    = (YUV_TYPE_E)vp8_read_bit(bc);
        pc->clamp_type  = (CLAMP_TYPE_E)vp8_read_bit(bc);
    }

    // Is segmentation enabled
    xd->segmentation_enabled = (uint8)vp8_read_bit(bc);

    if (xd->segmentation_enabled)
    {
        // Signal whether or not the segmentation map is being explicitly updated this frame.
        xd->update_mb_segmentation_map = (uint8)vp8_read_bit(bc);
        xd->update_mb_segmentation_data = (uint8)vp8_read_bit(bc);

        if (xd->update_mb_segmentation_data)
        {
            xd->mb_segement_abs_delta = (uint8)vp8_read_bit(bc);	// segment_feature_mode, 0:delta, 1: abs
            ...
            for (i = 0; i < MB_LVL_MAX; i++)
            {
                for (j = 0; j < MAX_MB_SEGMENTS; j++)
                {
                    ...
                    if (vp8_read_bit(bc))
                    {
                        xd->segment_feature_data[i][j] = (int8)vp8_read_literal(vo, bc, mb_feature_data_bits[i]);

                        if (vp8_read_bit(bc))
                        {
                            xd->segment_feature_data[i][j] = (int8)(-xd->segment_feature_data[i][j]);
                        }
                    } else
                    {
    ...
    xd->mode_ref_lf_delta_enabled = (uint8)vp8_read_bit(bc);	// loop_filter_adj_enable

    if (xd->mode_ref_lf_delta_enabled)
    {
        // Do the deltas need to be updated
        xd->mode_ref_lf_delta_update = (uint8)vp8_read_bit(bc);
        ...
        if (xd->mode_ref_lf_delta_update)
        {
            ...
            for (i = 0; i < MAX_REF_LF_DELTAS; i++)
            {
                if (vp8_read_bit(bc))
                {
                    ...
                    xd->ref_lf_deltas[i] = (int8)vp8_read_literal(vo, bc, 6);

                    if (vp8_read_bit(bc))        // Apply sign
                        xd->ref_lf_deltas[i] = (int8)(xd->ref_lf_deltas[i] * -1);
                }
            }
            ...
            for (i = 0; i < MAX_MODE_LF_DELTAS; i++)
            {
                ...
                {
                    //sign = vp8_read_bit( bc );
                    xd->mode_lf_deltas[i] = (int8)vp8_read_literal(vo, bc, 6);

                    if (vp8_read_bit(bc))        // Apply sign
                    {
                        xd->mode_lf_deltas[i] = (int8)(xd->mode_lf_deltas[i] * -1);
                    }
                }
            }
        }
    ...
    pc->mb_no_coeff_skip = (int32)vp8_read_bit(bc);
