...
DIRECT_MV_REF_BUF col_mb_buf[34][120][68];//col_located MB para [addr_index][mb_x][mb_y]
LEFT_MB leftmb_info;
int qp_y_prev;//6b
...
//	int mca_buf1[50];
	...
	int curr_POC;//32b
	int addr_index;//6b
	int chroma_log2_weight_denom;//8b
	int Luma_log2_weight_denom;//8b
	...
	int sub_mb_type[4]={0};//3b
	int intra16x16_luma_pred_mode=0;//2b
	int intra_chroma_pred_mode=0;//2b 
	int prev_intra4x4_pred_mode_flag=0;//16b; 
	int prev_intra8x8_pred_mode_flag=0;//4b; 
	int rem_intra4x4_pred_mode[16]={0};//3b
	int rem_intra8x8_pred_mode[4]={0};//3b
	int ref_idxl0[4];//5b
	int ref_idxl1[4];//5b
	int mvd_x[2][16];//15b
	int mvd_y[2][16];//13b
	...
	//int list0[16]={0};
	int list0_POC[16]={0};
	int list0_longterm;//32bit //16b
	//int list1[16]={0};
	int list1_POC[16]={0};
	int list1_longterm;//16b

	// col_buf
	char col_ref_idx[4]={0,0,0,0};
	short col_mv[16][2]={0};
	int  col_ref_poc[4]={0,0,0,0};
	...
	//mca
	char predflag[4][2]={0};//[blk][reflist]//1b
	int w[4][2]={0};//weight[blk][reflist]//9b
    char mb_type_mca;//2b
	...
	char mb_avail_c;//1b
	char mb_avail_d;//1b
	short mv[2][30][2]={0};//14b/12b
	char ref_idx[2][30]={0};//5b  //2*16
	...
	int use_mv_pred;
	int map_idx;
	char out_col_ref_idx[4]={0};
	short out_col_mv[16][2]={0};
	int w_y,w_u,w_v,off_y,off_u,off_v;
	...
	/**********slice para in*********/	  
	  ...
	  chroma_log2_weight_denom=slice_info[34];//8b
	  Luma_log2_weight_denom=slice_info[33];//8b
	  addr_index=slice_info[35];
	  Slice_first_mb_x=slice_info[38];
	  Slice_first_mb_y=slice_info[39];
	  col_wr_en=slice_info[43];
  ...
  // MB para in
	  if(decoder_format==STREAM_ID_H264)
	  {
		  ...
		  is_intra=(mb_type>4);
		  cur_mb_y=(inbuf[0]>>3)&0x7f;//7b [9:3]
		  cur_mb_x=(inbuf[0]>>10)&0x7f;//7b; [16:10]
		  ...
		  cbp= transform_size_8x8_flag ? (inbuf[1]&0x3ff0000)|(inbuf[1]&0x00f000 ? 0x00f000 :0)|(inbuf[1]&0x000f00 ? 0x000f00 :0)|(inbuf[1]&0x0000f0 ? 0x0000f0 :0)|(inbuf[1]&0x00000f ? 0x00000f :0): (inbuf[1])&0x3ffffff;//26b; [25:0]
		  if (is_intra) {
   ...
   // refpic list in
	  if(decoder_format==STREAM_ID_H264)
	  {
		  ...
		  list0_longterm=(ref_list_buf[32])&0x0000ffff;//16b
		  list1_longterm=(ref_list_buf[32]>>16)&0xffff;//16b
		  ...
		  for(i=0;i<16;i++)
		  {
			 ...
			 //	 list0[i]=(ref_list_buf[i/2]>>16)&0xffff;
			  list0_POC[i]=ref_list_buf[i];
			  list1_POC[i]=ref_list_buf[i+16];
		  }
	  ...
	  if(decoder_format==STREAM_ID_H264)
	  {
		  ...
		  cbp26=(cbp&0x3ffc3c3)|((cbp&0x3030)>>2)|((cbp&0x0c0c)<<2);//raster scan
		  if (is_IPCM || is_skip)
			  cbp26=0;
	  ...
	  // col buff in
	  if((decoder_format==STREAM_ID_H264)&&col_wr_en)
	  {
		  ...
		  if((slice_type==B_slice)&&((mb_type==skip_direct_h264)||(sub_mb_type[0]==direct_8X8)||(sub_mb_type[1]==direct_8X8)||(sub_mb_type[2]==direct_8X8)||(sub_mb_type[3]==direct_8X8)))
		  {
			  int address_idx;
			  int temp;
              address_idx=g_list1_map_addr[0];//g_list0_map_addr[maplist1_to_list0[0]];
			  ...
			  if (!is_intra) 
			  {
				  ...
				  fprintf(g_fp_colin_tv, "frame_cnt=%d, mb_x=%d, mb_y=%d\n",g_nFrame_dec_h264,cur_mb_x,cur_mb_y);//jzy
				  for(i=0;i<4;i++)
						  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",col_ref_poc[i]);//jzy
				  temp=((col_mv[0][0]&0x3fff)<<17)+((col_mv[0][1]&0xfff)<<5)+((col_ref_idx[0]<0 ? 0x0 : 0x10|col_ref_idx[0])&0x1f);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[5][0]&0x3fff)<<17)+((col_mv[5][1]&0xfff)<<5)+((col_ref_idx[1]<0 ? 0x0 : 0x10|col_ref_idx[1])&0x1f);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[10][0]&0x3fff)<<17)+((col_mv[10][1]&0xfff)<<5)+((col_ref_idx[2]<0 ? 0x0 : 0x10|col_ref_idx[2])&0x1f);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[15][0]&0x3fff)<<17)+((col_mv[15][1]&0xfff)<<5)+((col_ref_idx[3]<0 ? 0x0 : 0x10|col_ref_idx[3])&0x1f);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[4][0]&0x3fff)<<12)+(col_mv[4][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[1][0]&0x3fff)<<12)+(col_mv[1][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[6][0]&0x3fff)<<12)+(col_mv[6][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[7][0]&0x3fff)<<12)+(col_mv[7][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[8][0]&0x3fff)<<12)+(col_mv[8][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[9][0]&0x3fff)<<12)+(col_mv[9][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[2][0]&0x3fff)<<12)+(col_mv[2][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[11][0]&0x3fff)<<12)+(col_mv[11][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[12][0]&0x3fff)<<12)+(col_mv[12][1]&0xfff);			  
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[13][0]&0x3fff)<<12)+(col_mv[13][1]&0xfff);
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[14][0]&0x3fff)<<12)+(col_mv[14][1]&0xfff);
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy
				  temp=((col_mv[3][0]&0x3fff)<<12)+(col_mv[3][1]&0xfff);
				  PARSER_FPRINTF(g_fp_colin_tv, "%08x\n",temp);//jzy			  
			  }
		  }
	  ...
	  if(decoder_format==STREAM_ID_H264)
	  {
			  ...
			  for (b8_y = 0; b8_y < 2; b8_y+=1)
			  {
				  for (b8_x = 0; b8_x < 2; b8_x+=1)
				  {
								  ...
								  if(direct_blk[blk8x8Idx])//(sub_mb_type[blk8x8Idx]==direct_8X8)||(mb_type==skip_direct_h264))
								  {
									  if(slice_type==B_slice)
									  {
										  ...
										  if(direct_spatial_mv_pred_flag)
										  {
											  ...
											  colzeroflag=((abs(col_mv[direct_blk_idx][0])>>1)==0)&&((abs(col_mv[direct_blk_idx][1])>>1)==0)&&(col_ref_idx[blk8x8Idx]==0)&&!(list1_longterm&0x1);//!((list0_longterm>>maplist1_to_list0[0])&0x1);
										  ...
										  {   
											  ...
											  if (listx==0) 
											  {
                                                  ...
                                                  iTRp = Clip3( -128, 127,  (list1_POC[0] - col_ref_poc[blk8x8Idx]));//list0_POC[maplist1_to_list0[0]]
					  ...
					  if (ref_idx[0][blk4x4StrIdx]>=0)
					  {
						  ...
						  addr_idx[0][blk4x4StrIdx]=addr_idx[0][blk4x4StrIdx+1]=addr_idx[0][blk4x4StrIdx+6]=addr_idx[0][blk4x4StrIdx+7]=g_list0_map_addr[ref_idx[0][blk4x4StrIdx]];
					  }
					  else
					  {
						  predflag[blk8x8Idx][0]=0;
						  addr_idx[0][blk4x4StrIdx]=addr_idx[0][blk4x4StrIdx+1]=addr_idx[0][blk4x4StrIdx+6]=addr_idx[0][blk4x4StrIdx+7]=0x3f;
					  }
					 
					  if((slice_type==B_slice)&&(ref_idx[1][blk4x4StrIdx]>=0))
					  {
					       predflag[blk8x8Idx][1]=1;
						   addr_idx[1][blk4x4StrIdx]=addr_idx[1][blk4x4StrIdx+1]=addr_idx[1][blk4x4StrIdx+6]=addr_idx[1][blk4x4StrIdx+7]=g_list1_map_addr[ref_idx[1][blk4x4StrIdx]];//g_list0_map_addr[maplist1_to_list0[ref_idx[1][blk4x4StrIdx]]];
					  }
					  else
					  {
						  predflag[blk8x8Idx][1]=0;
						  addr_idx[1][blk4x4StrIdx]=addr_idx[1][blk4x4StrIdx+1]=addr_idx[1][blk4x4StrIdx+6]=addr_idx[1][blk4x4StrIdx+7]=0x3f;
					  }
					  ...
					  {   
						  if(predflag[blk8x8Idx][0]&&predflag[blk8x8Idx][1]) 
						  {
							  ...
							  tmp2= ref_idx[1][blk4x4StrIdx];//maplist1_to_list0[ref_idx[1][blk4x4StrIdx]];
							  iTRp = Clip3( -128, 127, (list1_POC[tmp2] - list0_POC[tmp1])); //(list0_POC[tmp2] - list0_POC[tmp1]));
							  
							  if((iTRp==0)||((list0_longterm>>tmp1)&0x1)||((list1_longterm>>tmp2)&0x1))
							  {
		  ...
		  {
		      sub_mb_type_mca[0]=sub_mb_type_mca[1]=sub_mb_type_mca[2]=sub_mb_type_mca[3]=0;
		  }
		  else if (direct_8x8_inference_flag)//||direct_spatial_mv_pred_flag
		  {
		  ...
		  mca_buf[0]=(sub_mb_type_mca[3]<<23)|(sub_mb_type_mca[2]<<21)|(sub_mb_type_mca[1]<<19)|(sub_mb_type_mca[0]<<17)|(mb_type_mca<<15)|(is_intra<<14)|(cur_mb_x<<7)|cur_mb_y;//17b
		  mca_buf[1]=(predflag[3][1]<<23)|(predflag[3][0]<<22)|(predflag[2][1]<<21)|(predflag[2][0]<<20)|(predflag[1][1]<<19)|(predflag[1][0]<<18)|(predflag[0][1]<<17)|(predflag[0][0]<<16)|(chroma_log2_weight_denom<<8)|Luma_log2_weight_denom;
//		  mca_buf1[0]=mca_buf[0];
//		  mca_buf1[1]=mca_buf[1];
		  if(!is_intra)
		  {
			  ...
			  {
				  blk4x4StrIdx = h264_blk_order_map[blk4x4Idx];
				  mca_buf[10+blk4x4Idx]=(mv[0][blk4x4StrIdx][0]<<12)|(mv[0][blk4x4StrIdx][1]&0xfff);//26b
				  mca_buf[34+blk4x4Idx]=(mv[1][blk4x4StrIdx][0]<<12)|(mv[1][blk4x4StrIdx][1]&0xfff);//26b
                //  mca_buf1[10+blk4x4Idx]=mca_buf[10+blk4x4Idx];
				//  mca_buf1[34+blk4x4Idx]=mca_buf[34+blk4x4Idx];
			  }
			  for(listx=0;listx<2;listx++)
			  {
				  for(blk8x8Idx=0;blk8x8Idx<4;blk8x8Idx++)
				  {
					 if(predflag[blk8x8Idx][listx])
					 {
						 blk4x4StrIdx = h264_blk_order_map[blk8x8Idx*4];
						 if ( (weighted_bipred_idc == 2)  && (slice_type == B_slice)) // || (weighted_pred_flag && slice_type !=I_SLICE))
						 {
							  w_y = w_u = w_v = w[blk8x8Idx][listx];
							  off_y = off_u = off_v = 0;
						 }
						 else
						 {
							  w_y = g_wp_weight[listx][ref_idx[listx][blk4x4StrIdx]][0];
							  w_u = g_wp_weight[listx][ref_idx[listx][blk4x4StrIdx]][1];
							  w_v = g_wp_weight[listx][ref_idx[listx][blk4x4StrIdx]][2];
							  off_y = g_wp_offset[listx][ref_idx[listx][blk4x4StrIdx]][0];
							  off_u = g_wp_offset[listx][ref_idx[listx][blk4x4StrIdx]][1];
							  off_v = g_wp_offset[listx][ref_idx[listx][blk4x4StrIdx]][2];
						 }
						 mca_buf[2+blk8x8Idx*2+listx*24]=((w_v&0x1ff)<<18)|((w_u&0x1ff)<<9)|(w_y&0x1ff);
						 mca_buf[2+blk8x8Idx*2+1+listx*24]=((ref_idx[listx][blk4x4StrIdx]&0xf)<<24)|((off_v&0xff)<<16)|((off_u&0xff)<<8)|(off_y&0xff);
					    // mca_buf1[2+blk8x8Idx*2+listx*24]=mca_buf[2+blk8x8Idx*2+listx*24];
						// if (listx==0)							
						//	 mca_buf1[2+blk8x8Idx*2+1+listx*24]=((ref_idx[listx][blk4x4StrIdx]&0x1f)<<24)|((off_v&0xff)<<16)|((off_u&0xff)<<8)|(off_y&0xff);
					  	// else
						//	 mca_buf1[2+blk8x8Idx*2+1+listx*24]=((maplist1_to_list0[ref_idx[listx][blk4x4StrIdx]]&0x1f)<<24)|((off_v&0xff)<<16)|((off_u&0xff)<<8)|(off_y&0xff);
					
					 }
					 else
					 {
						 mca_buf[2+blk8x8Idx*2+listx*24]=0;
						 mca_buf[2+blk8x8Idx*2+1+listx*24]=0;
						 //mca_buf1[2+blk8x8Idx*2+listx*24]=0;
						 //mca_buf1[2+blk8x8Idx*2+1+listx*24]=0;
	  ...
	  if((decoder_format==STREAM_ID_H264)&&col_wr_en)
	  {
			  ...
			  if(blk4x4Idx%4==0)
			  {
				  ...
				  else
				  {
					  if ((slice_type==B_slice)&&(ref_idx[0][blk4x4StrIdx]<0)&&(ref_idx[1][blk4x4StrIdx]>=0)) 
					  {
						  ...
						  list_sel=1;
						  col_ref_poc[blk8x8Idx]=list1_POC[ref_idx[1][blk4x4StrIdx]];//list0_POC[maplist1_to_list0[ref_idx[1][blk4x4StrIdx]]];
					  }
					  else
					  {
		  ...
		  PARSER_FPRINTF(g_fp_colout_tv, "frame_cnt=%d, mb_x=%d, mb_y=%d\n",g_nFrame_dec_h264,cur_mb_x,cur_mb_y);//jzy

		  for(i=0;i<4;i++)
					  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",col_ref_poc[i]);//jzy

		  temp=((out_col_mv[0][0]&0x3fff)<<17)+((out_col_mv[0][1]&0xfff)<<5)+((out_col_ref_idx[0]<0 ? 0x0 : 0x10|out_col_ref_idx[0])&0x1f);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[5][0]&0x3fff)<<17)+((out_col_mv[5][1]&0xfff)<<5)+((out_col_ref_idx[1]<0 ? 0x0 : 0x10|out_col_ref_idx[1])&0x1f);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[10][0]&0x3fff)<<17)+((out_col_mv[10][1]&0xfff)<<5)+((out_col_ref_idx[2]<0 ? 0x0 : 0x10|out_col_ref_idx[2])&0x1f);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[15][0]&0x3fff)<<17)+((out_col_mv[15][1]&0xfff)<<5)+((out_col_ref_idx[3]<0 ? 0x0 : 0x10|out_col_ref_idx[3])&0x1f);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[4][0]&0x3fff)<<12)+(out_col_mv[4][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[1][0]&0x3fff)<<12)+(out_col_mv[1][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[6][0]&0x3fff)<<12)+(out_col_mv[6][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[7][0]&0x3fff)<<12)+(out_col_mv[7][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[8][0]&0x3fff)<<12)+(out_col_mv[8][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[9][0]&0x3fff)<<12)+(out_col_mv[9][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[2][0]&0x3fff)<<12)+(out_col_mv[2][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[11][0]&0x3fff)<<12)+(out_col_mv[11][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[12][0]&0x3fff)<<12)+(out_col_mv[12][1]&0xfff);			  
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[13][0]&0x3fff)<<12)+(out_col_mv[13][1]&0xfff);
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[14][0]&0x3fff)<<12)+(out_col_mv[14][1]&0xfff);
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
			  temp=((out_col_mv[3][0]&0x3fff)<<12)+(out_col_mv[3][1]&0xfff);
			  PARSER_FPRINTF(g_fp_colout_tv, "%08x\n",temp);//jzy
	  ...
	  if(decoder_format==STREAM_ID_H264)
	  {
		  ...
		  //mbc out
          	mbc_buf[0]=(cur_mb_y<<10)|(intra_chroma_pred_mode<<8)|((is_intra ? Ipred_size : (dct_size<<1))<<6)|(Ipred_avail_d<<5)|(Ipred_avail_a<<4)|(Ipred_avail_b<<3)|(Ipred_avail_c<<2)|(is_IPCM<<1)|is_intra;//25b
		if (is_intra) {
			  if (is_IPCM)  //ipcm //czzheng added @20120717
			  {
			  	 mbc_buf[1] = 0;
			        mbc_buf[2] = 0;
			  }			
			  else if( Ipred_size==I_16X16)
			  {
				  mbc_buf[1]=intra16x16_luma_pred_mode;//2b
				  mbc_buf[2]=0;//2b
			  }
			  else if (Ipred_size==I_8X8 || Ipred_size==I_4X4)  //czzheng added @20120717
			  {
			      mbc_buf[1]=(i4_pred_mode_ref[16]<<28)|(i4_pred_mode_ref[15]<<24)|(i4_pred_mode_ref[14]<<20)|(i4_pred_mode_ref[13]<<16)|(i4_pred_mode_ref[10]<<12)|(i4_pred_mode_ref[9]<<8)|(i4_pred_mode_ref[8]<<4)|i4_pred_mode_ref[7];//32b
				  mbc_buf[2]=(i4_pred_mode_ref[28]<<28)|(i4_pred_mode_ref[27]<<24)|(i4_pred_mode_ref[26]<<20)|(i4_pred_mode_ref[25]<<16)|(i4_pred_mode_ref[22]<<12)|(i4_pred_mode_ref[21]<<8)|(i4_pred_mode_ref[20]<<4)|i4_pred_mode_ref[19];//32b
			  }
		}
		else { //inter //czzheng added @20120717
		  	     mbc_buf[1] = 0;
		    	 mbc_buf[2] = 0;
		}
          	mbc_buf[3]=(cur_mb_x<<24)|(is_IPCM ? 0xffffff : (is_intra&&(Ipred_size==I_16X16) ? (cbp26|0x00ffff)&0xffffff: cbp26&0xffffff));//24b//



	/*	  mbc_buf[0]=(intra_chroma_pred_mode<<23)|((is_intra ? Ipred_size : (dct_size<<1))<<21)|(Ipred_avail_d<<20)|(Ipred_avail_c<<19)|(Ipred_avail_b<<18)|(Ipred_avail_a<<17)|(skip_idct<<16)|(is_IPCM<<15)|(is_intra<<14)|(cur_mb_x<<7)|cur_mb_y;//25b
	  ...
	  if(decoder_format==STREAM_ID_H264)
	  {
		  ...
		  for(i=0;i<4;i++)//horizontal line
		  {
			  for(j=0;j<4;j++)//part
			  {
				  ...
				  condition0=(ref0_p==ref0_q)&&((ref0_p==0x3f)||(abs(mv0_p[0]-mv0_q[0])<4)&&(abs(mv0_p[1]-mv0_q[1])<4));
				  condition1=(ref0_p==ref1_q)&&((ref0_p==0x3f)||(abs(mv0_p[0]-mv1_q[0])<4)&&(abs(mv0_p[1]-mv1_q[1])<4));
                  condition2=(ref1_p==ref0_q)&&((ref1_p==0x3f)||(abs(mv1_p[0]-mv0_q[0])<4)&&(abs(mv1_p[1]-mv0_q[1])<4));
				  condition3=(ref1_p==ref1_q)&&((ref1_p==0x3f)||(abs(mv1_p[0]-mv1_q[0])<4)&&(abs(mv1_p[1]-mv1_q[1])<4));
				  ...
				  else 
				  {
                       if((ref0_p==ref1_p)||(ref0_q==ref1_q))
						   bs_h[4*i+j]=(condition0&&condition3)||(condition1&&condition2) ? 0 : 1;
					   else
		  ...
		  for(i=0;i<4;i++)//vertical line
		  {
			  for(j=0;j<4;j++)//part
			  {
				  ...
				  {
					  if((ref0_p==ref1_p)||(ref0_q==ref1_q))
						  bs_v[4*i+j]=(condition0&&condition3)||(condition1&&condition2) ? 0 : 1;
					  else
		  ...
		  //dbk out
		  dbk_buf[0]=(cur_mb_x<<8)|cur_mb_y;//32b
          ...
          dbk_buf[5]=(qp_top<<12)|(qp_left<<6)|(mb_qp_y<<0);
