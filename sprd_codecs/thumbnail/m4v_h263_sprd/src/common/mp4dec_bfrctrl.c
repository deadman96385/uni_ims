...
PUBLIC MMDecRet Mp4Dec_InitYUVBfr(VIDEO_DATA_T *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 id;
	int32 size_y, size_c;
    ...
    for(id = 0; id < DEC_YUV_BUFFER_NUM; id++)
	{
		...
		if (need_malloc_decY)
		{			
	        	pDecFrame->imgY = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(vd, (uint32)size_y * sizeof(uint8));  //y
			if (vop_mode_ptr->uv_interleaved) //two plane
			{
				pDecFrame->imgU = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(vd, 2*(uint32)size_c * sizeof(uint8));  //uv
				pDecFrame->imgV = PNULL;
			}else //three plane
			{
		        	pDecFrame->imgU = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(vd, (uint32)size_c * sizeof(uint8));  //u
		        	pDecFrame->imgV = (uint8 *)Mp4Dec_ExtraMemAlloc_64WordAlign(vd, (uint32)size_c * sizeof(uint8));  //v
		...
		#ifdef _VSP_LINUX_
			pDecFrame->imgYAddr = (uint32)Mp4Dec_ExtraMem_V2Phy(vd, pDecFrame->imgY) >> 8;  //y
			pDecFrame->imgUAddr = (uint32)Mp4Dec_ExtraMem_V2Phy(vd, pDecFrame->imgU) >> 8;  //u
			pDecFrame->imgVAddr = (uint32)Mp4Dec_ExtraMem_V2Phy(vd, pDecFrame->imgV) >> 8;  //v
		#else
		...
		{
			pDecFrame->imgYUV[0] = (uint8 *)Mp4Dec_ExtraMemCacheAlloc_64WordAlign(vd, (uint32)ext_size_y * sizeof(uint8));  //y
			pDecFrame->imgYUV[1] = (uint8 *)Mp4Dec_ExtraMemCacheAlloc_64WordAlign(vd, (uint32)ext_size_c * sizeof(uint8));  //u
       	 		pDecFrame->imgYUV[2] = (uint8 *)Mp4Dec_ExtraMemCacheAlloc_64WordAlign(vd, (uint32)ext_size_c * sizeof(uint8));  //v
...
static BOOLEAN Mp4Dec_GetOneFreeDecBfr(VIDEO_DATA_T *vd, int32 *bfrId)
{
	int id;
...
static BOOLEAN Mp4Dec_GetOneFreeDispBfr(VIDEO_DATA_T *vd, int32 *bfrId)
{
	int id;
...
BOOLEAN Mp4Dec_GetCurRecFrameBfr(VIDEO_DATA_T *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	if(!vop_mode_ptr->post_filter_en)
	{
		...
		if (uv_format == 0)	//uv
		{
			...
			SCI_TRACE_LOW("%s,%0x, %0x, %0x", __FUNCTION__, pDecFrame->imgY, pDecFrame->imgU,pDecFrame->imgV);
		}else	//vu
		{
			pDecFrame->imgY =  vd->g_rec_buf.imgY;
...
DEC_FRM_BFR* Mp4Dec_GetDispFrameBfr(VIDEO_DATA_T *vd, Mp4DecStorablePic *rec_pic)
{
	...
	if((vop_mode_ptr->VopPredType != BVOP)&&(vop_mode_ptr->pCurDispFrame->pDecFrame->pBufferHeader!=NULL))
		(*vd->VSP_bindCb)(vd->g_user_data,vop_mode_ptr->pCurDispFrame->pDecFrame->pBufferHeader,0);
#endif
	...
	{
		(*vd->VSP_unbindCb)(vd->g_user_data,disp_frm->pBufferHeader,0);
