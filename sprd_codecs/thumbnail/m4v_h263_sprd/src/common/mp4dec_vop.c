...
PUBLIC void Mp4Dec_exit_picture(VIDEO_DATA_T *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{	
	...
	if(vop_mode_ptr->VopPredType != BVOP)
	{
		...
		{
			(*vd->VSP_bindCb)(vd->g_user_data,vop_mode_ptr->pCurRecFrame->pDecFrame->pBufferHeader,0);
		}
#endif
		...
		if(vop_mode_ptr->VopPredType == IVOP)
		{
			if(vop_mode_ptr->pBckRefFrame->pDecFrame != NULL)
			{
				...
				{
					(*vd->VSP_unbindCb)(vd->g_user_data,vop_mode_ptr->pBckRefFrame->pDecFrame->pBufferHeader,0);
		...
		if(vop_mode_ptr->pFrdRefFrame->pDecFrame != NULL)
		{
			...
			{
				(*vd->VSP_unbindCb)(vd->g_user_data,vop_mode_ptr->pFrdRefFrame->pDecFrame->pBufferHeader,0);
...
PUBLIC void Mp4Dec_output_one_frame (VIDEO_DATA_T *vd, MMDecOutput *dec_output_ptr, DEC_VOP_MODE_T *vop_mode_ptr)
{
