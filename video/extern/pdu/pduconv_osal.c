...
static int convert_7bit_to_ascii(char *a7bit, int length, char *ascii, int maxTargetLen) {
    int     r;
    int     w;

    /* Make sure we have enough space to convert to ascii */
    if (maxTargetLen < length * 2 + 1) {
        return (0);
    }
...
static int convert_ascii_to_7bit(char *ascii, char **a7bit, int *unknown) {
    int r;
    int w;
    ...
    // xxx *a7bit=OSAL_memAlloc(strlen(ascii)*2+1, 0);

    while (ascii[r]!=0) {
        if ((lookup_ascii8to7[(unsigned char)ascii[r]])<256) {
            if (NPC7 == lookup_ascii8to7[(unsigned char)ascii[r]] &&
                    NPC7 != ascii[r]) {
                /* Then we have a char we don't understand */
                *unknown = 1;
            }
    ...
    *a7bit=realloc(*a7bit,w+1);
    /*append a terminate symbol */
    (*a7bit)[w] = '\0';
    ...
    int maxTargetLen)
{
  ...
  // xxx ascii7bit=OSAL_memAlloc(pdulength*2+1,0); 
  ...
  length=convert_7bit_to_ascii((char *)ascii7bit,w,ascii, maxTargetLen);
  free(ascii7bit);
    ...
    int            maxTargetLen)
{
    ...
    for (r = 0;r < pdulength;r++) {
        if (r % 7 == 0) {
            ...
            ascii7bit[w++] = ((pdu[r]>>7) | ((pdu[r - 1] << 1) & 0x7F)) & 0x7F;
            ascii7bit[w++] = (pdu[r]) & 0x7F;
        } else {
            ascii7bit[w++] = ((pdu[r] >> ((r % 7) + 1)) | 
                    (pdu[r - 1] << (7 - (r % 7)) & 0x7F)) & 0x7F;
        }
    }
 ...
 *
 *  *pdu   Pointer to an array where the pdu-bytes will be stored in
 *
 * *hasUnknownChars A pointer to an int that will set to "1" if any
 *                  unknown chars were encoded as NPC7.
 *
    ...
    int *hasUnknownChars)
{
    ...
    len7bit=convert_ascii_to_7bit(ascii,&ascii7bit, hasUnknownChars);
    /* Make sure we have enught space for pdu */
    if (maxTargetLen < len7bit) {
        free(ascii7bit);
        return (0);
    }
    ...
    int *hasUnknownChars)
{
    ...
    while (r<len7bit) {
        if (r == len7bit - 1) {
            pdu[w]=((ascii7bit[r]<<((w%7)+1)));
        }
        else {
            pdu[w]=((ascii7bit[r]<<((w%7)+1))) | ((ascii7bit[r+1]>>(7-((w%7)+1))));
        }
        if (((w%7)+1)==7) r++;
        r++;
        w++;
...
static int get_septet_length(char ascii) {
    if ((lookup_ascii8to7[(unsigned char)ascii])<256) {
        return 1;
    } else {
       return 2;
    }
...
int count_septets(char *ascii) {
    int x = 0;
    int count = 0;
    while (ascii[x] != 0) {
        count += get_septet_length(ascii[x]);
        x++;
    }
    return count;
}    
...
static int ucs2_to_utf8(unsigned short ucs2, unsigned char* utf8)
{
    if (ucs2 < 0x80) {
        utf8[0] = ucs2;
        utf8[1] = '\0';
    ...
    int             outLen)
{
    ...
    int totalBytes = 0;
    inLen = inLen >> 1;
    for (x = 0 ; x < inLen && totalBytes <= (outLen - 4) ; x++) {
        /* There needs to be at least 4 bytes available in out_ptr */
        bytes = ucs2_to_utf8(OSAL_netNtohs(*in_ptr), out_ptr);
        if (-1 == bytes) {
            return -1;
        }
        in_ptr++;
    ...
    int *hasUnknownChars)
{
    ...
    r=0;
    w=0;
    while (r < strlen(ascii)) {
        if ((unsigned char)ascii[r] > 0x7F) {
            *hasUnknownChars = 1;
            return 0;
        }
        if (r == strlen(ascii) - 1) {
            ...
            pdu[w]=((ascii[r]<<((w%7)+1))) | ((ascii[r+1]>>(7-((w%7)+1))));
        }
        if (((w%7)+1)==7) r++;
        r++;
        w++;
    ...
    int            maxTargetLen)
{
    ...
    for (r = 0;r < pdulength;r++) {
        if (r % 7 == 0) {
            ascii[w++] = (pdu[r] >> 1) & 0x7F;
        }
        else if (r % 7 == 6) {
            ascii[w++] = ((pdu[r]>>7) | ((pdu[r - 1] << 1) & 0x7F)) & 0x7F;
            ascii[w++] = (pdu[r]) & 0x7F;
        } else {
            ascii[w++] = ((pdu[r] >> ((r % 7) + 1)) | 
                    (pdu[r - 1] << (7 - (r % 7)) & 0x7F)) & 0x7F;
