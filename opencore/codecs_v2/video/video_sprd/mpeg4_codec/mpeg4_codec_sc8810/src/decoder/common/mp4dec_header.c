...
//	assert((vop_mode_ptr->time_inc_resolution >= 1) && (vop_mode_ptr->time_inc_resolution < 65536));
	...
	if(RECTANGLE == fAUsage)
	{
		...
		}else
		{
			...
			if ((aligned_frm_width <= 720 && aligned_frm_height <=576) ||
				(aligned_frm_width <= 576 && aligned_frm_height <= 720))
			{
				...
				if ((aligned_frm_width <= 320 && aligned_frm_height <=240) ||
					...
					{
						vop_mode_ptr->VSP_pipeline = 0;
					}else
					{
						vop_mode_ptr->VSP_pipeline = 1;
					}
			}else 
			{
...
PUBLIC MMDecRet Mp4Dec_DecH263Header(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	//judge VSP mode
	{
		...
		SCI_TRACE_LOW("H263 decoder: vsp_used: %d", vop_mode_ptr->VSP_used);
	}
