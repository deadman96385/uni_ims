...
void mea_module(void)
{
	...
	int32 mb_mode = INTRA;
	int32 standard;
	int32 frame_height, frame_width;
	...
	frame_height = g_mea_frame_height*MB_SIZE;
	frame_width = g_mea_frame_width*MB_SIZE;
...
//	print_src_mb(mb_pos_x, mb_pos_y);
	...
	{
		MEA_INFO fake;
		PrintMV_ime_src_mb(g_enc_vop_mode_ptr, fake);
		Print_IMB_fbuf(g_enc_vop_mode_ptr, -1, (MOTION_VECTOR_T *)NULL, fake);
	}
#endif
	if(VSP_JPEG ==standard)
	{
	...
	}else
	{
		if(INTRA == mb_mode)
		{
			...
			{
				fprintf (g_fp_mea_mca_mb_tv, "//frame_cnt=%d, mb_x=%d, mb_y=%d\n", g_nFrame_enc, g_enc_vop_mode_ptr->mb_x, g_enc_vop_mode_ptr->mb_y);
				for(j = 0; j < 48; j++ )
				{
					fprintf(g_fp_mea_mca_mb_tv, "%016x\n", 0);
				}
			}
#endif
		}else //inter mb
