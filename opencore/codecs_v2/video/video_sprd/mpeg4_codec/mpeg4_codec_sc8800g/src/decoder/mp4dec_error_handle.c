...
{	
	...
	SCI_ASSERT (component_num < 3);/*assert verified*/
...
__inline uint32 endian_conv(uint32 in)
{
    uint32 out;
...
static void MCABfr2RecFrm(DEC_VOP_MODE_T *vop_mode_ptr)
{
    int32 cmd, i;
	...
	READ_REG_POLL(VSP_DCAM_REG_BASE+DCAM_CFG_OFF, V_BIT_7, V_BIT_7, TIME_OUT_CLK, "DCAM_CFG: polling dcam clock status");
			...
			if (curr_mb_dec_status < DECODED_NOT_IN_ERR_PKT) //not coded or coded but in error packet
			{
				...
				if (is_error) //then do error concealment
				{
					...
					if (prev_pvop_valid) //try temporal interpolation
					{
                        ...
                        if(READ_REG_POLL(VSP_MCA_REG_BASE+MCA_DEBUG_OFF, V_BIT_2|V_BIT_1|V_BIT_0, 0, TIME_OUT_CLK, "MCA_DEBUG: polling mca done status"))
                        {
               				PRINTF("mca time out!\n");
