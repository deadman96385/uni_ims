...
LOCAL int32 GetHorGrad (uint8 *curr_blk_pix, int32 frame_width, int32 blk_size)
{
	int32 i;
	int32 mad = 0;
	...
	{
		mad += ABS(curr_blk_pix[0] - curr_blk_pix[-1]);
...
LOCAL int32 GetVertGrad (uint8 *curr_blk_pix, int32 frame_width, int32 blk_size)
{
	int32 i;
...
//	{
//		frame_width >>= 1;
//		curr_blk_addr = vop_mode_ptr->mb_y * BLOCK_SIZE * frame_width + vop_mode_ptr->mb_x * BLOCK_SIZE;
//		blk_ptr = vop_mode_ptr->pCurRecFrame->pDecFrame->imgUV + curr_blk_addr;
...
//		frame_width >>= 1;
//		curr_blk_addr = vop_mode_ptr->mb_y * BLOCK_SIZE * frame_width + vop_mode_ptr->mb_x * BLOCK_SIZE;
//		blk_ptr = vop_mode_ptr->pCurRecFrame->pDecFrame->imgV + curr_blk_addr;
//		blk_width = BLOCK_SIZE;
//		shift = 3;
//	}
//
//	if (nei_avail[0]) //left
//	{
//		avg_grad += GetHorGrad(blk_ptr, frame_width, blk_width);
//	}
//
//	if (nei_avail[1]) //right
//	{
//		avg_grad += GetHorGrad(blk_ptr + blk_width, frame_width, blk_width);
//	}
//
//	if (nei_avail[2]) //top
//	{
//		avg_grad += GetVertGrad(blk_ptr, frame_width, blk_width);
//	}
//
//	if (nei_avail[3]) //bottom
//	{
//		avg_grad += GetVertGrad(blk_ptr + blk_width*frame_width, frame_width, blk_width);
//	}
//
//	if (avail_num)
//	{
...
LOCAL s_sx[3][3] = {{-1, 0, 1}, {-2, 0, 2}, {-1, 0, 1}};
LOCAL s_sy[3][3] = {{1, 2, 1}, {0, 0, 0}, {-1, -2, -1}};
...
uint32 endian_conv(uint32 in)
{
    ...
    out |= (((in<<16)>>8) &0xff0000);
    
    out |= (((in<<24)>>0) &0xff000000);
...
void MCABfr2RecFrm(DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	READ_REG_POLL(VSP_MCA_REG_BASE+MCA_DEBUG_OFF, V_BIT_2|V_BIT_1|V_BIT_0, 0, TIME_OUT_CLK, "MCA_DEBUG: polling mca done status");
...
//	uint8 *curr_pix_ptr_u = vop_mode_ptr->pCurRecFrame->pDecFrame->imgU + (mb_y * BLOCK_SIZE) * frame_width_c + (mb_x * BLOCK_SIZE);
//	uint8 *curr_pix_ptr_v = vop_mode_ptr->pCurRecFrame->pDecFrame->imgV + (mb_y * BLOCK_SIZE) * frame_width_c + (mb_x * BLOCK_SIZE);
//	uint8 *top_pix_ptr_y = curr_pix_ptr_y - frame_width;
//	uint8 *top_pix_ptr_u = curr_pix_ptr_u - frame_width_c;
//	uint8 *top_pix_ptr_v = curr_pix_ptr_v - frame_width_c;
//	uint8 *bot_pix_ptr_y = curr_pix_ptr_y + MB_SIZE * frame_width;
//	uint8 *bot_pix_ptr_u = curr_pix_ptr_u + BLOCK_SIZE * frame_width_c;
//	uint8 *bot_pix_ptr_v = curr_pix_ptr_v + BLOCK_SIZE * frame_width_c;
//	int32 i, j;
//
//	if (!top_avail)
//	{
...
//		}
//
//		curr_pix_ptr_u += frame_width_c;
//		curr_pix_ptr_v += frame_width_c;
//	}
}
...
PUBLIC void Mp4Dec_UpdateErrInfo (DEC_VOP_MODE_T *vop_mode_ptr)
{
	...
	bitstream_pos = (VSP_READ_REG(VSP_BSM_REG_BASE+BSM_TOTAL_BITS_OFF, "read BSMR_TOTAL_BITS reg") + 7) >> 3;
