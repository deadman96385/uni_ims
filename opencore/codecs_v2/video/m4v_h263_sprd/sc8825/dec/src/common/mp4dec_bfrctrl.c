    ...
    for(id = 0; id < DEC_YUV_BUFFER_NUM; id++)
	{
		...
		{			
			...
			}else //three plane
			{
		        	...
		        	{
		        		SCI_TRACE_LOW("MMDEC_MEMORY_ERROR  %s %d",__FUNCTION__,__LINE__);
		            		return MMDEC_MEMORY_ERROR;
		        	}
			}
		...
		#ifdef _VSP_LINUX_
			pDecFrame->imgYAddr = (uint32)Mp4Dec_ExtraMem_V2P(pDecFrame->imgY, HW_NO_CACHABLE);  //y
			pDecFrame->imgUAddr = (uint32)Mp4Dec_ExtraMem_V2P(pDecFrame->imgU, HW_NO_CACHABLE);  //u
			pDecFrame->imgVAddr = (uint32)Mp4Dec_ExtraMem_V2P(pDecFrame->imgV, HW_NO_CACHABLE);  //v
		#else
...
static BOOLEAN Mp4Dec_GetOneFreeDecBfr(int32 *bfrId)
{
	...
	{
		SCI_TRACE_LOW("Mp4Dec_GetOneFreeDecBfr failed  %s %d",__FUNCTION__,__LINE__);
		return FALSE;
	}else
	{
		return TRUE;
...
static BOOLEAN Mp4Dec_GetOneFreeDispBfr(int32 *bfrId)
{
	...
	{
		SCI_TRACE_LOW("Mp4Dec_GetOneFreeDispBfr failed  %s %d",__FUNCTION__,__LINE__);
		return FALSE;
	}else
	{
		return TRUE;
...
//#endif
	...
	pDecFrame->imgYAddr = g_rec_buf.imgYAddr;
	pDecFrame->imgUAddr = pDecFrame->imgYAddr ;//+ (size_y>>8);
	pDecFrame->imgVAddr = pDecFrame->imgUAddr + (size_y>>2);// (size_y>>10);
