        ...
        OMX_VIDEO_PARAM_PROFILELEVELTYPE* aProfileLevel)
{
  ...
  //  aRateControlType.eControlRate = OMX_Video_ControlRateDisable;
    ...
    switch (iEncMode)
    {
		...
		SCI_TRACE_LOW("Mpeg4Encoder_OMX::Mp4EncInit H263_MODE_WITH_ERR_RES\n");
            }
            else
            {
                ENC_Mode = H263_MODE;
		SCI_TRACE_LOW("Mpeg4Encoder_OMX::Mp4EncInit H263_MODE\n");
            }
    ...
    {
  #if 0
         sp<MemoryHeapBase> masterHeap = new MemoryHeapBase(PMEM_DRIVER,(((((iSrcWidth + 15) >> 4) * ((iSrcHeight + 15) >> 4)) * 3) << 7),MemoryHeapBase::NO_CACHING);
 	 iYUVInPmemHeap = new MemoryHeapPmem(masterHeap,MemoryHeapBase::NO_CACHING);
	 ...
	 if(fd>=0){
		...
		iYUVIn_phy =(uint8 *)region.offset;					
		SCI_TRACE_LOW("Mpeg4Encoder_OMX yuvin mem pmempool %x,%x,%x,%x\n",iYUVInPmemHeap->getHeapID(),iYUVInPmemHeap->base(),region.offset,region.len);
   ...
   SCI_TRACE_LOW("Mpeg4Encoder_OMX::Mp4EncInit:iSrcFrameRate %d,iTimeIncRes %d,iTickPerSrc %d\n",iSrcFrameRate,iTimeIncRes,iTickPerSrc);
    ...
    {
#if 0
	 sp<MemoryHeapBase> masterHeap = new MemoryHeapBase(PMEM_DRIVER,vidExtBufSize,MemoryHeapBase::NO_CACHING);
 	 iEncExtPmemHeap = new MemoryHeapPmem(masterHeap,MemoryHeapBase::NO_CACHING);
 	 int fd = iEncExtPmemHeap->getHeapID();
	 if(fd>=0){
	       	iEncExtPmemHeap->slap();
		masterHeap.clear();
	        struct pmem_region region;
	        ::ioctl(fd,PMEM_GET_PHYS,&region);
		iEncExtBuf_phy =(uint8 *)region.offset;					
		SCI_TRACE_LOW("Mpeg4Encoder_OMX ext mem pmempool %x,%x,%x,%x\n",iEncExtPmemHeap->getHeapID(),iEncExtPmemHeap->base(),region.offset,region.len);
...
#endif
	SCI_TRACE_LOW("aInBuffer %x, aInBuffer_phy %x,aInBuffer_size %d",aInBuffer,aInBuffer_phy,aInBufSize);
...
//SCI_TRACE_LOW("p_src_y u v%x ,%x,%x",vid_in.p_src_y ,vid_in.p_src_u,vid_in.p_src_v);
//SCI_TRACE_LOW("phy :p_src_y u v%x ,%x,%x",vid_in.p_src_y_phy ,vid_in.p_src_u_phy,vid_in.p_src_v_phy);
	int vsp_stat = vsp_acqure(iVsp_fd);
	if(vsp_stat)
	{
	    *aOutputLength = 0;
	...
	if(ret == MMENC_OK)
        ...
        if (status == PV_TRUE)
        {
            ...
            iNextModTime = (OMX_TICKS)((int32)( aInTimeStamp/1000)  + intervalms);
            iFrameTypeCounter++;
            if (Size > 0)
            {
