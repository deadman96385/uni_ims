					 ...
					 )
{
	...
	pix_read[3] = (uint8)((read_data >> 24) & 0xff);
	pix_read[2] = (uint8)((read_data >> 16) & 0xff);
	pix_read[1] = (uint8)((read_data >> 8 ) & 0xff);
	pix_read[0] = (uint8)((read_data >> 0 ) & 0xff);
}
...
//	uint8	pix_filter[8];
	...
	for (blk_id = 0; blk_id < 34; blk_id++)
	{
		...
		{
			if ((blk_id == 23) && (line_id == 4))
				printf ("");
#ifdef TV_OUT
#ifdef ONE_FRAME
			if(g_nFrame_dec==FRAME_X)
#endif
//			FPRINTF_DBK (g_fp_dbk_info_tv, "//frame_cnt=%d, mb_x=%d, mb_y=%d, blk_id=%d, line_id=%d\n", g_nFrame_dec, mb_col, mb_row, blk_id, line_id);
#endif
			/**********************************************************
		...
		//	if (blk_wr_x >= 0)
			...
			**********************************************************/
			...
			if(g_nFrame_dec==FRAME_X)
#endif
//			FPRINTF_DBK (g_fp_dbk_info_tv, "//edge_limit=%x, interior_diff_limit=%x, hevthr=%x, filter_type=%x\n", edge_limit, interior_diff_limit, hevthr, filter_type);
#endif
