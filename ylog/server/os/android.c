...
static struct ylog_keyword os_ylog_keyword_sroot[] =
{
	{"sroot", load_sroot},
	{NULL, NULL},
};
...
static int os_check_sroot(struct ydst_root *root, char *sdcard_path, char **historical_folder_root)
{
	if (root->sroot_flag && !root->sroot)
	{
		struct ylog_keyword *kw = os_ylog_keyword_sroot;
		parse_config(global_context->ylog_config_file, kw);
	}
	if (root->sroot)
	{
		strcpy(sdcard_path, root->sroot);
...
static int os_search_root_path(char *path, int len)
{
	...
	if (sroot == 1 && \
	        os_check_sdcard_online(sdcard_path, sizeof(sdcard_path)))
	{
		historical_folder_root = pos->historical_folder_root_last;
		pos->sdcard_online = 1;
	}
		...
		/* android/ */ {
			.ylog = {
				...
				},
				{
					...
					.mode = YLOG_READ_MODE_BLOCK | YLOG_READ_LEN_MIGHT_ZERO | YLOG_READ_MODE_BLOCK_RESTART_ALWAYS|YLOG_READ_MODE_BINARY,
					.restart_period = 2000,
					.id_token = "A4",
					.id_token_len = 2,
