...
/*****************************************************************************
 **	Name : 			Mp4_DivRoundnearest
 ** Description:	DivRoundnearest function. 
 ** Author:			Xiaowei Luo
 **	Note:
 *****************************************************************************/
int32 Mp4_DivRoundnearest(int32 i, int32 iDenom)
{
	if(i >= 0)
	{
 ...
 **	Note:
 *****************************************************************************/
LOCAL int16 *Mp4Dec_GetLeftBlkPred(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, 
					int32 blk_num, int32 *QpLeft, int32 *predAvail)
{
	...
	*predAvail = FALSE;
	...
	if(blk_num < 4)
	{
		pLeftPred = vop_mode_ptr->pLeftCoeff_sw + (blk_num>>1);// + (blk_num >> 1) * 8;
		if((blk_num & 1) == 0)
		{
			if(leftAvail)
			{
	...
	{	
		...
		pLeftPred = vop_mode_ptr->pLeftCoeff_sw + (blk_num - 2); // + (blk_num - 2) * 8;
	}
 ...
 **	Note:
 *****************************************************************************/
LOCAL int16 *Mp4Dec_GetTopBlkPred(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, 
 ...
 **	Note:
 *****************************************************************************/
LOCAL int32 Mp4Dec_GetLeftTopBlkPred(DEC_VOP_MODE_T *vop_mode_ptr, int32 blk_num)
{
	int32 pred;
 ...
 **	Note: 
					...
					3. inverse DC IQUANT;
 *****************************************************************************/
LOCAL void Mp4Dec_GetInverseDCACPred(DEC_MB_MODE_T *mb_mode_ptr, DEC_MB_BFR_T *mb_cache_ptr, 
...
{	
	...
	if(predAvail)
	{
		rgiCoefQ[0] += (int16)Mp4_DivRoundnearest(blkmPred[0], iDcScaler);	
	}else
	{
		rgiCoefQ[0] += (int16)Mp4_DivRoundnearest(iDefVal, iDcScaler); 	
	}
	...
	if(mb_mode_ptr->bACPrediction && predAvail)
	{
		...
		if(HORIZONTAL == mb_cache_ptr->preddir)
		{
			...
			}else
			{
				for(i = 1; i < 8; i++)
				{
					rgiCoefQ[i*8] += (int16)Mp4_DivRoundnearest(blkmPred[i] * iQpPred, iQP);
				}
			}
		}else
		{
			...
			}else
			{
				for(i = 1; i < 8; i++)
				{
					rgiCoefQ[i] += (int16)Mp4_DivRoundnearest(blkmPred[i] * iQpPred, iQP);
				}
			}
		}
	}
...
//	memset(pBlk, 0, 64 * sizeof(int16));	
	...
	if(!mb_cache_ptr->bCodeDcAsAc)
	{
		...
		else
		{
			...
			pBlk[0] = (int16)g_dec_dc_store[mb_num][iBlkIdx];
		}
	...
	#endif //DATA_PARTITION	
		{
			Mp4Dec_VlcDecIntraTCOEF(vop_mode_ptr, pBlk, iCoefStart);
		}
	...
	Mp4Dec_GetInverseDCACPred(mb_mode_ptr, mb_cache_ptr, iBlkIdx, pBlk, iDCScalar);
}
...
//#else
//	arm_memset32WordToZero(piDCTCoeff);
//#endif //ASM_MEMSET
	...
	if(mb_mode_ptr->CBP & (32 >> iBlkIdx))
	{
		Mp4Dec_VlcDecInterTCOEF_H263(vop_mode_ptr/*, piDCTCoeff*/, 1, bitstrm_ptr);
