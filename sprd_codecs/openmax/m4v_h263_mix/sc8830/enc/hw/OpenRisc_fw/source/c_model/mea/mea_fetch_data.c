...
void output_ref_mb(void)
{
	...
	if(g_vector_enable_flag&VECTOR_ENABLE_MEA)
	{
		...
		for(i = 0; i < 32; i++)
		{
			/*for(j = 0; j < 2; j++ )
			{
				fprintf_oneWord_hex(g_fp_mea_mca_mb_tv, pMea[j]);
			}
			pMea += 2;*/
			for(j=7; j >= 0; j-- )
			{
				fprintf(g_fp_mea_mca_mb_tv, "%02x", pMea8[j]);
			}
			pMea8 += 8;
			fprintf(g_fp_mea_mca_mb_tv, "\n");
		}
