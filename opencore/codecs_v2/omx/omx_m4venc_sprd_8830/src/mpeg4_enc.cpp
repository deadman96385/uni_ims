        ...
        OMX_VIDEO_PARAM_PROFILELEVELTYPE* aProfileLevel)
{
   ...
   OMX_MP4ENC_DEBUG("Mpeg4Encoder_OMX::Mp4EncInit:iSrcFrameRate %d,iTimeIncRes %d,iTickPerSrc %d\n",iSrcFrameRate,iTimeIncRes,iTickPerSrc);
    ...
    MMCodecBuffer 	InterMemBfr,ExtaMemBfr, StreamMemBfr;
    int fd, ret,phy_addr, buffer_size;

    buffer_size = MP4ENC_INTERNAL_BUFFER_SIZE;
    ...
    if(fd>=0){
        ...
        if(ret){
            OMX_MP4ENC_ERR("%s, %d: get_phy_addr_from_ion fail", __FUNCTION__, __LINE__);
        }

        iORBuf_v = (void *)iORbufferPmemHeap->base();
        InterMemBfr.common_buffer_ptr_phy = (void *)phy_addr;
    ...
    }else
    {
        ...
        OMX_MP4ENC_ERR ("%s: cannot malloc VID internal buff", __FUNCTION__);
        return OMX_ErrorInsufficientResources;	
    }

    buffer_size = encInfo.frame_width* encInfo.frame_height*3/2*2;
    iExtPmemHeap = new MemoryHeapIon(SPRD_ION_DEV,buffer_size,MemoryHeapBase::NO_CACHING, ION_HEAP_CARVEOUT_MASK);
    fd = iExtPmemHeap->getHeapID();
    if(fd>=0){
        ret = iExtPmemHeap->get_phy_addr_from_ion(&phy_addr, &buffer_size);
        if(ret) OMX_MP4ENC_ERR("iDecExtPmemHeap get_phy_addr_from_ion fail %d",ret);
		
	OMX_MP4ENC_DEBUG("Mpeg4Encoder_OMX ext mem pmempool %x,%x,%x,%x\n",iExtPmemHeap->getHeapID(),iExtPmemHeap->base(),phy_addr,buffer_size);

        iExtBuf_v = (void *) iExtPmemHeap->base();
        ExtaMemBfr.common_buffer_ptr = (uint8 *)iExtBuf_v;
        ExtaMemBfr.common_buffer_ptr_phy = (void *)phy_addr;
        ExtaMemBfr.size	= buffer_size;
    }else
    {
        iBufferAllocFail = OMX_TRUE;
    ...
    buffer_size = ONEFRAME_BITSTREAM_BFR_SIZE;
	...
	OMX_MP4ENC_DEBUG("Mpeg4Encoder_OMX ext mem pmempool %x,%x,%x,%x\n",iExtPmemHeap->getHeapID(),iStreamPmemHeap->base(),phy_addr,buffer_size);

        iStreamBuf_v = (void *) iExtPmemHeap->base();;
        iStreamBuf_p = (void *)phy_addr;       
        StreamMemBfr.common_buffer_ptr = (uint8 *)iStreamBuf_v;
        StreamMemBfr.common_buffer_ptr_phy = iStreamBuf_p;
        StreamMemBfr.size	= buffer_size;
    }else
    {
        iBufferAllocFail = OMX_TRUE;
    	OMX_MP4ENC_ERR("Mpeg4Encoder_OMX ext  mem pmempool  error\n");
    ...
    int status;
    MMEncRet InitRet =  MP4EncInit(&InterMemBfr, &ExtaMemBfr,&StreamMemBfr, &encInfo);
    if(InitRet == MMENC_OK)
        status = 	OMX_TRUE;
    else
	status = 	OMX_FALSE;		
