...
PVMFCommandId AndroidAudioOutput::DiscardData(PVMFTimestamp aTimestamp, const OsclAny* aContext)
{
    ...
    {
        //if (iOSSRequestQueue[i].iTimestamp < aTimestamp) //jgdu for streaming seek bug
        {
...
int AndroidAudioOutput::audout_thread_func()
{
    ...
    status_t ret = mAudioSink->open(iAudioSamplingRate, iAudioNumChannels, ((iAudioFormat == PVMF_MIME_PCM8) ? AUDIO_FORMAT_PCM_8_BIT: AUDIO_FORMAT_PCM_16_BIT), kNumOutputBuffers);
