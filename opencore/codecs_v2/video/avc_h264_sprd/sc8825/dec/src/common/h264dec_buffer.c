...
PUBLIC void H264Dec_init_img_buffer (DEC_IMAGE_PARAMS_T *img_ptr)
{
   ...
   if(total_mb_num < 100) { ////Modified by Leon.Li @20120925
        img_ptr->cmd_data_buf[0]= (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256 * 6, 4, HW_CACHABLE);
		img_ptr->cmd_info_buf[0] = (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256*2, 4, HW_CACHABLE);
		img_ptr->frame_bistrm_buf[0] = (uint8 *)H264Dec_ExtraMemAlloc (H264DEC_FRM_STRM_BUF_SIZE, 4, HW_CACHABLE);
		img_ptr->cmd_data_buf[1]= (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256 * 6, 4, HW_CACHABLE);
		img_ptr->cmd_info_buf[1] = (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256*2, 4, HW_CACHABLE);
		img_ptr->frame_bistrm_buf[1] = (uint8 *)H264Dec_ExtraMemAlloc (H264DEC_FRM_STRM_BUF_SIZE, 4, HW_CACHABLE);
   }
   else{

		img_ptr->cmd_data_buf[0]= (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256 * 3, 4, HW_CACHABLE);
		img_ptr->cmd_info_buf[0] = (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256*1, 4, HW_CACHABLE);
		img_ptr->frame_bistrm_buf[0] = (uint8 *)H264Dec_ExtraMemAlloc (H264DEC_FRM_STRM_BUF_SIZE, 4, HW_CACHABLE);
		img_ptr->cmd_data_buf[1]= (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256 * 3, 4, HW_CACHABLE);
		img_ptr->cmd_info_buf[1] = (uint32 *) H264Dec_ExtraMemAlloc(total_mb_num * 256*1, 4, HW_CACHABLE);
		img_ptr->frame_bistrm_buf[1] = (uint8 *)H264Dec_ExtraMemAlloc (H264DEC_FRM_STRM_BUF_SIZE, 4, HW_CACHABLE);
   }
...
PUBLIC void H264Dec_init_dpb (DEC_IMAGE_PARAMS_T *img_ptr, DEC_SPS_T *sps_ptr)
{
	...
	for (i = 0; i < MAX_REF_FRAME_NUMBER+1; i++)
	{
		...
		if (dpb_ptr->fs[i] == PNULL)
		{
			...
			dpb_ptr->fs[i]->frame->imgYAddr = (uint32)dpb_ptr->fs[i]->frame->imgY;  //y;//Modified by Leon.Li @20120925
			dpb_ptr->fs[i]->frame->imgUAddr = (uint32)dpb_ptr->fs[i]->frame->imgU;  //u;
			dpb_ptr->fs[i]->frame->imgVAddr = (uint32)dpb_ptr->fs[i]->frame->imgV;  //v;
		}
