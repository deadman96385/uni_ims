...
void Trace_after_intra_prediction_Blk8x8(uint8 *pred, uint32 width)
{
	if(g_trace_enable_flag&TRACE_ENABLE_IPRED)	
	{
		int i,j;
...
if(g_trace_enable_flag&TRACE_ENABLE_IPRED)	
{
	...
	{		
		for (jj = 0; jj < 8; jj++)
		{
			FPRINTF(g_fp_trace_ipred, "%02x", tmp_pred_Y[ii *MB_SIZE +7- jj]);
			FPRINTF(g_fp_mbc_ipred, "%02x", tmp_pred_Y[ii *MB_SIZE +7- jj]);
		}
		FPRINTF(g_fp_trace_ipred, "\n");
		FPRINTF(g_fp_mbc_ipred, "\n");
		for (jj = 0; jj < 8; jj++)
		{
			FPRINTF(g_fp_trace_ipred, "%02x", tmp_pred_Y[ii *MB_SIZE +15- jj]);
			FPRINTF(g_fp_mbc_ipred, "%02x", tmp_pred_Y[ii *MB_SIZE +15- jj]);
		}
		FPRINTF(g_fp_trace_ipred, "\n");
		FPRINTF(g_fp_mbc_ipred, "\n");
	...
	{		
		for (jj = 0; jj < BLOCK_SIZE; jj++)
		{
			FPRINTF(g_fp_trace_ipred, "%02x", tmp_pred_U[ii *BLOCK_SIZE +7- jj]);
			FPRINTF(g_fp_mbc_ipred, "%02x", tmp_pred_U[ii *BLOCK_SIZE +7- jj]);
		}
		FPRINTF(g_fp_trace_ipred, "\n");
		FPRINTF(g_fp_mbc_ipred, "\n");
		for (jj = 0; jj < BLOCK_SIZE; jj++)
		{
			FPRINTF(g_fp_trace_ipred, "%02x", tmp_pred_V[ii *BLOCK_SIZE +7- jj]);
			FPRINTF(g_fp_mbc_ipred, "%02x", tmp_pred_V[ii *BLOCK_SIZE +7- jj]);
		}
		FPRINTF(g_fp_trace_ipred, "\n");
		FPRINTF(g_fp_mbc_ipred, "\n");
	}	
