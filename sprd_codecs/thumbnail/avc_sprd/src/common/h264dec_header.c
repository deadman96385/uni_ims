...
//	DEC_IMAGE_PARAMS_T *img_ptr = img_ptr->g_image_ptr;
...
LOCAL void H264Dec_pred_weight_table (H264DecContext *img_ptr, DEC_SLICE_T *curr_slice_ptr)
{
	int32 i, j,list;
	DEC_BS_T *stream = img_ptr->bitstrm_ptr;
...
PUBLIC MMDecRet H264Dec_RestSliceHeader (H264DecContext *img_ptr, DEC_SLICE_T *curr_slice_ptr)
{
	...
	if (img_ptr->direct_spatial_mv_pred_flag == 1)
	{
		...
		if (img_ptr->VSP_used)
		{
			img_ptr->pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_spatial_hw;
			img_ptr->b8_mv_pred_func[0] = H264Dec_MC8x8_direct_spatial_hw;
		}else
#endif		
		{
			img_ptr->pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_spatial_sw;
	...
	}else
	{
		...
		if (img_ptr->VSP_used)
		{
			img_ptr->pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_temporal_hw;
			img_ptr->b8_mv_pred_func[0] = H264Dec_MC8x8_direct_temporal_hw;
		}else
#endif		
		{
			img_ptr->pred_skip_bslice = H264Dec_mv_prediction_skipped_bslice_temporal_sw;
