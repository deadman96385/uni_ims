...
LOCAL void H264Dec_mm_unmark_long_term_for_reference (H264DecObject *vo, int32 long_term_pic_num, DEC_DECODED_PICTURE_BUFFER_T *dpb_ptr)
{
    int32 i;
...
#if _MVC_
LOCAL DEC_STORABLE_PICTURE_T *get_inter_view_pic(H264DecObject *vo, int32 targetViewID, int32 listidx)
{
    unsigned i;
                             ...
                             int listidx)
{
    ...
    picLX = get_inter_view_pic(vo, targetViewID, listidx);
...
PUBLIC MMDecRet H264Dec_init_list (H264DecObject *vo, int32 curr_slice_type)
{
    ...
    if (curr_slice_type == P_SLICE)
    {
        ...
        if (curr_slice_ptr->svc_extension_flag == 0 && curr_slice_ptr->view_id != 0)
        {
            ...
            append_interview_list(vo, vo->g_dpb_layer[1],
                                  ...
                                  curr_view_id,
                                  curr_slice_ptr->anchor_pic_flag);
            for (i = 0; i < (curr_slice_ptr->listinterviewidx0); i++)
            {
    ...
    } else	//B-slice
    {
        ...
        if (curr_slice_ptr->svc_extension_flag == 0)
        {
            ...
            for (i = 0; i < (curr_slice_ptr->listinterviewidx1); i++)
            {
