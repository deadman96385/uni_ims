...
{\
    FPRINTF(g_fp_global_tv,"1,%08x,%08x   //%s\n",reg_addr,value,pstring);\
...
#define OR1200_READ_REG(reg_addr, pstring) *((int*)(reg_addr))
#define OR1200_READ_REG_POLL(reg_addr, msk_data, msked_data, pstring) \
{\
	int tmp;\
	tmp=(*((int*)(reg_addr)))&msk_data;\
    while(tmp != msked_data)\
	{\
      tmp=(*((int*)(reg_addr)))&msk_data;\
	}\
