...
**---------------------------------------------------------------------------*/
    ...
    {
#endif
PUBLIC MMDecRet Mp4Dec_frm_level_sync_hw_sw_pipeline (MP4DecObject *vd, MMDecInput *dec_input_ptr, MMDecOutput *dec_output_ptr, DEC_VOP_MODE_T *vop_mode_ptr)
{
	uint32 cmd;
	...
	if(mp4Handle->VSP_fluchCacheCb)
	{
		...
		//ret = (*VSP_fluchCacheCb)(g_user_data,(int *)s_CMD_mem_cache_bfr_ptr,(int *)((uint32)s_CMD_mem_cache_bfr_phy_ptr),s_CMD_mem_cache_size);
...
PUBLIC MMDecRet Mp4Dec_frm_level_sync_hw_sw_normal (MP4DecObject *vd, MMDecInput *dec_input_ptr, MMDecOutput *dec_output_ptr, DEC_VOP_MODE_T *vop_mode_ptr)
{
	uint32 cmd;
        MP4Handle *mp4Handle = vd->mp4Handle;
	MMDecRet ret = MMDEC_OK;
 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecIVOP_hw(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 pos_y, pos_x;
	int32 rsc_found;
	...
	{	
		...
		{
		#if _DEBUG_
			if((pos_y == 6)&&(pos_x == 9)&&(vop_mode_ptr->g_nFrame_dec == 53))
			{
				foo();
			}
		#endif //_DEBUG_
		...
		//	SCI_TRACE_LOW( "nframe: %d, Y: %d, X: %d\n", g_nFrame_dec, pos_y, pos_x);
			...
			/*vld*/
			Mp4Dec_DecIntraMBTexture_hw(vd, vop_mode_ptr, mb_mode_ptr);
 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecPVOP_hw(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 pos_y, pos_x;
	int32 rsc_found;
	...
	{	
		...
		{	
		...
		#endif //_DEBUG_
		#if _TRACE_			
			FPRINTF (g_fp_trace_fw, "\nnframe: %d, Y: %d, X: %d\n", vop_mode_ptr->g_nFrame_dec, pos_y, pos_x);
		#endif //_TRACE_		
			...
			{	
				...
				{
					Mp4Dec_StartMcaOneDir(vop_mode_ptr, mb_cache_ptr, mb_mode_ptr->mv);
					Mp4Dec_DecInterMBTexture_hw(vd, vop_mode_ptr, mb_mode_ptr);
				}
			}
 ...
 **	Note:
 *****************************************************************************/
PUBLIC MMDecRet Mp4Dec_DecBVOP_hw(MP4DecObject *vd, DEC_VOP_MODE_T *vop_mode_ptr)
{
	int32 pos_y, pos_x;
	...
	{	
		...
		{	
		#if _DEBUG_
			if((pos_y == 0)&&(pos_x == 7)&&(vop_mode_ptr->g_nFrame_dec == 2))
			{
				foo();
			}
		#endif //_DEBUG_	
			...
			/*vld*/
			Mp4Dec_DecInterMBTexture_hw(vd, vop_mode_ptr, mb_mode_bvop_ptr);
