...
typedef struct vsp_ahbm_reg_tag 
{
	...
	volatile uint32 CMD_INFO_BUF_ADDR;			//base address in SDRAM for command infor 
	volatile uint32 CMD_DATA_BUF_ADDR;			//base address in SDRAM for command data

	volatile uint32 CMD_DEBUG0;				//
	volatile uint32 CMD_DEBUG1;				//
}VSP_AHBM_REG_T;
