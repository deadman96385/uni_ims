...
void startPreviewInternal(ImsCameraClient *ptr)
{
    ...
    ALOGI("startPreviewInternal, previewRotationOffset %d previewRotationAngle %d",
...
void* cameraProcessThread(void* param __unused)
{
    ...
    while(1)
    {
        ...
        switch(camera_cmd)
        {
            ...
            rc  = cameraService->connectLegacy(
                    client,
                    cameraId,
            ...
            case SET_CAMERA_PARAM_CMD:
            {
                ...
                switch(setParams->type)
                {
                    ...
                    case SET_RESOLUTION:
                        ...
                        ALOGI("cameraProcessThread, width = %d, height = %d",
                                width,height);
                        break;
                    default:
                        break;
...
short imsCameraOpen(unsigned int cameraid)
{
    ...
    if ((err = pthread_create(&cameraLoopThread, NULL,
            cameraProcessThread, (void *) NULL)) < 0) {
...
short imsCameraSetPreviewSurface(sp<IGraphicBufferProducer> * surface)
{
    ...
    {
        ALOGI("imsCameraSetPreviewSurface failed, mPreviewNativeWindow.get() is null");
        return -1;
    }
...
void registerCameraCallbacks(ims_notify_callback notifyCb __unused)
{
    return ;
}
...
int ImsCameraClient::init(void* params __unused) {
    return 0;
}
