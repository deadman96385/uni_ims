...
#endif
{
	...
	if(VSP_JPEG != vid_standard)
	{
		...
		{
			fprintf (g_fp_dbk_mb_tv, "//frame=%d, mb_x=%d, mb_y=%d\n", g_nFrame_enc, mb_x, mb_y);
		}
		DBK_TraceMBInfor (
	...
	//	FPRINTF(fp_rdbk_trace_file,"output:\n");
			...
			if ((vid_standard == ITU_H263) || (vid_standard == VSP_MPEG4) || (vid_standard == FLV_H263) || (h264_fmo_mode == TRUE))  //mpeg4 output reconstructed frame
		{
			...
			for (j = 0; j < MB_SIZE; j++)
			{
				...
				if(g_vector_enable_flag&VECTOR_ENABLE_DBK)
				{
					fprintf (g_fp_dbk_mb_tv, "%08x%08x\n", frm_pix_ptr[1], frm_pix_ptr[0]);
					fprintf (g_fp_dbk_mb_tv, "%08x%08x\n", frm_pix_ptr[3], frm_pix_ptr[2]);
					fprintf (g_fp_dbk_mb_tv, "%016x\n", 0);
				}
				mb_pix_ptr  += 6;
			...
			for (uv = 0 ; uv < 2; uv++)
			{
				...
				{		
					...
					if(g_vector_enable_flag&VECTOR_ENABLE_DBK)
					{
						if(uv)
						{
							fprintf (g_fp_dbk_mb_tv, "%08x", *(vsp_dbk_out_bfr + V_OFFSET_BUF_C + 18 + j * 4 + 1));
							fprintf (g_fp_dbk_mb_tv, "%08x\n", *(vsp_dbk_out_bfr + V_OFFSET_BUF_C + 18 + j * 4));
						}
						else
						{
							fprintf (g_fp_dbk_mb_tv, "%08x", *(vsp_dbk_out_bfr + U_OFFSET_BUF_C + 18 + j * 4 + 1));
							fprintf (g_fp_dbk_mb_tv, "%08x\n", *(vsp_dbk_out_bfr + U_OFFSET_BUF_C + 18 + j * 4));
						}
						fprintf (g_fp_dbk_mb_tv, "%016x\n", 0);
					}
				}
					 ...
					 )//weihu
{
	   ...
	   qp_top=(dbk_para_buf[5]>>12)&0x3f;
	   qp_left=(dbk_para_buf[5]>>6)&0x3f;
	   qp_y=(dbk_para_buf[5])&0x3f;
	   ...
	   cmd = ((v_qp_offset & 0x1f) << 21)|((u_qp_offset & 0x1f) << 16) | ((LFAlphaC0Offset & 0x1f) << 8) | ((LFBetaOffset & 0x1f) << 0);
