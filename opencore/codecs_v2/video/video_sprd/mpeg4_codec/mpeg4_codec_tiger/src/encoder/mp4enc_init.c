...
PUBLIC void Mp4Enc_InitSession(VOL_MODE_T *pVol_mode, ENC_VOP_MODE_T *pVop_mode)
{
	...
	{
		if (pVop_mode->FrameWidth <= 352)
		{
			pVop_mode->mbline_num_slice = 1;
		}else if (pVop_mode->FrameWidth <= 720)
		{
			pVop_mode->mbline_num_slice = 2;
		}else //if (pVop_mode->FrameWidth <= 1280)
		{
			pVop_mode->mbline_num_slice = 4;
		}
	}
...
{	
	...
	{
		//pVop_mode->pYUVSrcFrame->imgU = (uint8 *)Mp4Enc_ExtraMemAlloc(size>>2);
		//pVop_mode->pYUVSrcFrame->imgV = (uint8 *)Mp4Enc_ExtraMemAlloc(size>>2);
		//@modified by leon 20120602
		pVop_mode->pYUVSrcFrame->imgU = (uint8 *)Mp4Enc_ExtraMemAlloc_64WordAlign(size);
		pVop_mode->pYUVSrcFrame->imgV = (uint8 *)Mp4Enc_ExtraMemAlloc_64WordAlign(size);
	}else
	{
...
//@modified by leon 20120525
	pVop_mode->pYUVSrcFrame->imgYAddr = (uint32)pVop_mode->pYUVSrcFrame->imgY ;//>> 2;
	pVop_mode->pYUVSrcFrame->imgUAddr = (uint32)pVop_mode->pYUVSrcFrame->imgU ;//>> 2;
	pVop_mode->pYUVSrcFrame->imgVAddr = (uint32)pVop_mode->pYUVSrcFrame->imgV ;//>> 2;



	pVop_mode->pYUVRecFrame->imgYAddr = (uint32)pVop_mode->pYUVRecFrame->imgY ;//>> 2;
	pVop_mode->pYUVRecFrame->imgUAddr = (uint32)pVop_mode->pYUVRecFrame->imgU;// >> 2;
	pVop_mode->pYUVRecFrame->imgVAddr = (uint32)pVop_mode->pYUVRecFrame->imgV ;//>> 2;

	pVop_mode->pYUVRefFrame->imgYAddr = (uint32)pVop_mode->pYUVRefFrame->imgY ;//>> 2;
	pVop_mode->pYUVRefFrame->imgUAddr = (uint32)pVop_mode->pYUVRefFrame->imgU;// >> 2;
	pVop_mode->pYUVRefFrame->imgVAddr = (uint32)pVop_mode->pYUVRefFrame->imgV ;//>> 2;
}
#endif
...
#endif
#if _CMODEL_
	pVop_mode->pYUVSrcFrame->imgYAddr = SRC_FRAME_Y_ADDR >> 2;
	pVop_mode->pYUVRecFrame->imgYAddr = REC_FRAME0_Y_ADDR >> 2;
	pVop_mode->pYUVRefFrame->imgYAddr = REC_FRAME1_Y_ADDR >> 2;
#endif
