...
void AddPredAndResidual (uint8 *pred, int16 *coeff, int32 width)
{
	...
	dct_8x8 = ((g_mbc_reg_ptr->MBC_CMD0 >>31)&0x1)&&(width==MBC_Y_SIZE);//weihu
	if(dct_8x8)
		blk_size=8;
	else
		blk_size=4;
...
if(g_trace_enable_flag&TRACE_ENABLE_IPRED)	
{
	...
	FPRINTF(g_fp_trace_ipred, "frame_cnt=%d, mb_x=%d, mb_y=%d\n",g_nFrame_dec_h264 , mb_x, mb_y);//jzy
	...
	if(g_trace_enable_flag&TRACE_ENABLE_IPRED)	
	{
		...
		}else if (mb_type == 5) //i8x8//weihu
		{
			...
			for (blkIdxInMB = 0; blkIdxInMB < 4; blkIdxInMB++)
			{
				...
				if (blkIdxInMB <2)
				{
					intrapred8x8mode = (g_mbc_reg_ptr->MBC_CMD1>>(28-blkIdxInMB*8))&0xf;
				}else
				{
					intrapred8x8mode = (g_mbc_reg_ptr->MBC_CMD2>>(28-blkIdxInMB*8+16))&0xf;
				}
				...
				intraPredLuma8x8[intrapred8x8mode] (blkIdxInMB, pred, top, left);
				...
				blk_coded = (g_mbc_reg_ptr->MBC_CMD0 >> (i*8+j*2)) & 0x01;
				
				if (blk_coded)
				{
					//add residual coef
					int16 *p4x4blk = coeff + g_blk_rec_ord_tbl[i*8+j*2] * 16;
					AddPredAndResidual (pred, p4x4blk, MBC_Y_SIZE);
				}
			}			
		}//ipred 8x8
