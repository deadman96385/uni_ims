...
static const char _stateName[] = "Service:Reset";
    ...
    FSM_Event_Ptr   event_ptr)
{
    ...
    switch (event_ptr->eventType) {
        ...
        case FSM_EVENT_TYPE_NEW_IP:
            ...
            /*
             * If we get a FAILURE here that because we were de-activating
             * and now we should re-attempt to log in.
             */
    ...
    FSM_Event_Ptr   event_ptr)
{
    ...
    if (OSAL_TRUE == _FSM_isServiceMasterIp(context_ptr)) {
        ...
        _FSM_sendServiceStateEvent(CSM_SERVICE_STATE_INACTIVE,
 ...
 */
const FSM_State _FSM_RESET_STATE = {
