...
PUBLIC void H264Dec_interpret_sps (H264DecObject *vo, DEC_SPS_T *sps_ptr)
{
    ...
    if (!sps_ptr->frame_mbs_only_flag)
    {
        ...
        {
            SPRD_CODEC_LOGW ("MBAFF is not supported!\n");
            return;
        }
    }
...
#if _MVC_
LOCAL MMDecRet InterpretSubsetSPS (H264DecObject *vo, int32 *curr_seq_set_id)
{
...
{
    subset_seq_parameter_set_rbsp_t *subset_sps;
    int32 curr_seq_set_id = 0;

    InterpretSubsetSPS (vo, &curr_seq_set_id);
...
LOCAL void H264Dec_interpret_pps (H264DecObject *vo, DEC_PPS_T *pps_ptr)
{
    ...
    {
        SPRD_CODEC_LOGE ("pic_parameter_set_id > 255 or pps_ptr->seq_parameter_set_id >31 !\n");
        vo->error_flag  |= ER_SREAM_ID;
        pps_ptr->valid = FALSE;
        return;
    }//weihu
...
void init_subset_sps_list(subset_seq_parameter_set_rbsp_t *subset_sps_list, int32 iSize)
{
    int32 i;
    ...
    for(i=0; i<iSize; i++)
    {
        subset_sps_list[i].sps.seq_parameter_set_id = (uint8)(-1);
...
void reset_subset_sps(subset_seq_parameter_set_rbsp_t *subset_sps)
{
    ...
    {
        subset_sps->sps.seq_parameter_set_id = (uint8)(-1);
