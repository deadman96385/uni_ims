...
LOCAL void H264Dec_decode_IPCM_MB(DEC_IMAGE_PARAMS_T *img_ptr, DEC_MB_INFO_T *mb_info_ptr, DEC_MB_CACHE_T *mb_cache_ptr, DEC_BS_T * bitstrm)
{
	...
	int32 i,j;
	int32 bit_offset;
	uint8 *I_PCM_value = mb_cache_ptr->pred_cache[0].pred_Y;
#ifdef _NEON_OPT_
	uint32x4_t v32x4;
#endif
	...
	for(j = 0; j < (16 + 4 + 4); j++)
	{
		...
		for(i = 0; i < 16; i += 4)
		{
			...
			I_PCM_value[i+3] =  (value)&0xFF;
		}
	#else
		value = READ_FLC(bitstrm, 32);
		v32x4 = vsetq_lane_u32(value, v32x4, 0);

		value = READ_FLC(bitstrm, 32);
		v32x4 = vsetq_lane_u32(value, v32x4, 1);

		value = READ_FLC(bitstrm, 32);
		v32x4 = vsetq_lane_u32(value, v32x4, 2);

		value = READ_FLC(bitstrm, 32);
		v32x4 = vsetq_lane_u32(value, v32x4, 3);

		vst1q_u32((uint32 *)I_PCM_value, v32x4);
	#endif
		I_PCM_value += 16;
	}
