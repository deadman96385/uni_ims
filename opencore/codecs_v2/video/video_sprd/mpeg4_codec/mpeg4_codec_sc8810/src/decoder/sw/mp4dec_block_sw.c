 ...
 **	Note:
 *****************************************************************************/
LOCAL int16 *Mp4Dec_GetLeftBlkPred(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, int32 blk_num, int32 *QpLeft, int32 *predAvail)
{
 ...
 **	Note:
 *****************************************************************************/
LOCAL int16 *Mp4Dec_GetTopBlkPred(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, int32 blk_num, int32 *QpTop, int32 *predAvail)
{
...
void Mp4Dec_H263IqIntraBlock(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr , DEC_MB_BFR_T *pMBCache,int16 * piDCTCoeff, char *nonCoeffPos, int32 nonCoeffNum)
{
	int32 i;
	int32 coeff;
	int32 index;
...
void Mp4Dec_MpegIqIntraBlock(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr ,DEC_MB_BFR_T *pMBCache, int16 *piDCTCoeff, char *nonCoeffPos, int32 nonCoeffNum)
{
	int32 i;
	int32 coeff;
	int32 index;
	...
	char *piQuantizerMatrix = vop_mode_ptr->IntraQuantizerMatrix;
...
/*****************************************************************************
 **	Name : 			Mp4Dec_GetIntraBlkTCoef_Mp4_sw
 ** Description:	Get blk_num of current intra MB's TCoef, Mp4.
 ** Author:			Xiaowei Luo
 **	Note:			DeScan and IQ done by SW.
 *****************************************************************************/
PUBLIC void Mp4Dec_GetIntraBlkTCoef_Mp4_sw (DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, int16 *coef_block, int32 iBlkIdx)
{
	int32 iCoefStart = 0;
	DEC_MB_BFR_T *mb_cache_ptr = vop_mode_ptr->mb_cache_ptr;
	...
	#if 0//def _MP4CODEC_DATA_PARTITION_	
		else
		{
			int32 mb_x = vop_mode_ptr->mb_x;
			...
			coef_block[0] = (int16)g_dec_dc_store[mb_num][iBlkIdx];
		}
	...
	if(mb_mode_ptr->CBP & (32 >> iBlkIdx))
	{
		...
		{			
			Mp4Dec_RvlcIntraTCOEF(vop_mode_ptr, coef_block, iCoefStart);		
		}else
	#endif //DATA_PARTITION	
		{
		   	nonCoeffNum = Mp4Dec_VlcDecIntraTCOEF_sw(vop_mode_ptr, coef_block, iCoefStart,nonCoeffPos);
	   	}
	}
	...
	{
		Mp4Dec_H263IqIntraBlock(vop_mode_ptr,mb_mode_ptr, mb_cache_ptr, coef_block, nonCoeffPos, nonCoeffNum);
	}
}

/*****************************************************************************
 **	Name : 			Mp4Dec_GetIntraBlkTCoef_H263_sw
 ** Description:	Get blk_num of current intra MB's TCoef, H263.
 ** Author:			Xiaowei Luo
 **	Note:
 *****************************************************************************/
PUBLIC void Mp4Dec_GetIntraBlkTCoef_H263_sw(DEC_VOP_MODE_T *vop_mode_ptr, DEC_MB_MODE_T *mb_mode_ptr, int16 *coef_block, int32 iBlkIdx)
{
	int32 DCCoeff;
 ...
 // coef_block[0] = IClip(-2048,2047, coef_block[0] );
	...
	if(mb_mode_ptr->CBP & (32 >> iBlkIdx))
	{
		Mp4Dec_VlcDecInterTCOEF_H263_sw(vop_mode_ptr, coef_block, mb_mode_ptr->StepSize, bitstrm_ptr);
	}
}
/**---------------------------------------------------------------------------*
**                         Compiler Flag                                      *
